...
After updating fern definitions, run `cd fern && fern check` to validate. The CI pipeline runs `fern check` and treats errors as build failures. Common pitfalls:
  - Enum values with spaces need an explicit `name` property for code generation (e.g. `value: Inactive Visible` requires `name: InactiveVisible`)
  - Endpoint paths must not conflict with other endpoints in the same service
...
- Add underscore before any global variable name, i.e. _Item
