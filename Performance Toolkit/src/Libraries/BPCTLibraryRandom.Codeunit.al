...
#pragma warning disable AL0756
        exit(CalcDate(StrSubstNo(DatePattern, Delta / Abs(Delta) * RandInt(Abs(Delta))), WorkDate()));
#pragma warning restore AL0756
    ...
    begin
        ...
        if Range = 0 then
            exit(FromDate);
#pragma warning disable AL0756
        exit(CalcDate(StrSubstNo(DatePattern, Range / Abs(Range) * RandInt(Range)), FromDate));
#pragma warning restore AL0756
    end;
    ...
    begin
        ...
        if FromRange >= ToRange then
            exit(FromDate);
        exit(CalcDate(StrSubstNo(DatePattern, RandIntInRange(FromRange, ToRange)), FromDate));
    end;
