codeunit 88008 "NPR BCPT POS DS Total Disc Amt" implements "BCPT Test Param. Provider"
{
    SingleInstance = true;
    ...
    begin
        ...
        CreateDirectSalesWithTotalDiscountAmount();
    end;
    ...
    local procedure CreateDirectSalesWithTotalDiscountAmount()
    var
        i: Integer;
    begin
        for i := 1 to NoOfSales do
            CreateDirectSaleWithTotalDiscountAmount();
    end;

    local procedure CreateDirectSaleWithTotalDiscountAmount()
    var
        AmountToPay, TotalDiscoutAmount : Decimal;
    begin
        StartSale();
        AmountToPay := CreateLinesPerSale();
        TotalDiscoutAmount := AmountToPay * 0.1;
        SetLineDiscountPctABS(TotalDiscoutAmount);
        AmountToPay := AmountToPay - TotalDiscoutAmount;
        PaySale(AmountToPay);
    end;
    ...
    local procedure SetLineDiscountPctABS(TotalDiscoutAmount: Decimal)
    var
        PresetMultiLineDiscTarget: Option Auto,"Positive Only","Negative Only",All,Ask;
    begin
        BCPTTestContext.StartScenario('Set Total Discout Amount');
        POSMockLibrary.SetLineDiscountPctABS(POSSession, PresetMultiLineDiscTarget::"Positive Only", false, TotalDiscoutAmount);
        BCPTTestContext.EndScenario('Set Total Discout Amount');
        BCPTTestContext.UserWait();
    end;
