page 6184776 "NPR Setup DE Fiscal"
{
    Caption = 'Setup DE Fiscalization';
    Extensible = false;
    ...
    layout
    {
        area(Content)
        {
            ...
            group(IntroStep)
            {
                ...
                group(Welcome)
                {
                    Caption = 'Welcome to DE Fiscalization Setup';
                    Visible = IntroStepVisible;
                    group(Group18)
                    {
                        Caption = '';
                        InstructionalText = 'This essential step ensures your business adheres to Germany fiscal regulations. Enable DE fiscalization and set up related settings for a comprehensive and compliant financial foundation.';
                    }
                }
                group("Let's go!")
                {
            ...
            group(EnableFiscalStep)
            {
                ...
                group(EnableDEFiscal)
                {
                    Caption = 'Enable DE Fiscalization';
                    Editable = true;
                    ShowCaption = false;
                    part(DEEnableFiscalStep; "NPR DE Enable Fiscal Step")
                    {
                        ApplicationArea = NPRRetail;
                        Caption = 'Enabling DE Fiscalization in the setup is essential for the effective operation of fiscalization.';
                    }
                }
            }
            ...
            group(Finish)
            {
                ...
                {
                    Caption = '';
                    InstructionalText = 'Unable to complete DE Fiscalization Setup. Please ensure all required fields, including enabling DE fiscalization are filled correctly.';
    ...
    begin
        CurrPage.DEEnableFiscalStep.Page.CopyToTemp();
    ...
    begin
        DataPopulated := CurrPage.DEEnableFiscalStep.Page.IsDataPopulated();
    end;

    local procedure FinishAction();
    begin
        CurrPage.DEEnableFiscalStep.Page.CreateFiscalSetupData();
