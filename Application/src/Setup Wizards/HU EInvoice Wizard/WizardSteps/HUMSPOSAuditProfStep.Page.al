page 6150840 "NPR HU MS POS Audit Prof. Step"
{
    Caption = 'HU MS POS Audit Profile Setup';
    PageType = ListPart;
    ...
    internal procedure HUMSPOSAuditProfileDataToCreate(): Boolean
    begin
        exit(CheckIsDataSet());
    end;
    ...
    begin
        if not Rec.FindSet() then
            exit;
        repeat
            if (Rec."Audit Handler" = HUAuditMgt.HandlerCode()) and (Rec."Audit Log Enabled" = true) then
                exit(true);
        until Rec.Next() = 0;
    end;
}
