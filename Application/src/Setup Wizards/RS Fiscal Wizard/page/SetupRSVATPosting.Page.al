page 6151446 "NPR Setup RS VAT Posting"
{
    Caption = 'Setup RS VAT Posting Setup';
    Extensible = false;
    ...
    layout
    {
        area(Content)
        {
            ...
            group(IntroStep)
            {
                ...
                group(Welcome)
                {
                    Caption = 'Welcome to RS VAT Posting Setup';
                    Visible = IntroStepVisible;
                    group(Group18)
                    {
                        Caption = '';
            ...
            group(SetupVATPostingStep)
            {
                ...
                group(VATPostingStep)
                {
                    ShowCaption = false;
                    Editable = true;
                    part(RSVATPostingSetupStep; "NPR RS VAT Posting Step")
                    {
                        ApplicationArea = NPRRetail;
                        Caption = 'Add VAT Product and VAT Business Posting Group, then select the corresponding mapping from the dropdown menu to establish precise configurations.';
                    }
                }
            }
            // Finish Step
            group(Finish)
            {
                ...
                {
                    Caption = ' ';
                    InstructionalText = 'VAT Posting Setup Incomplete: It seems no combinations have been entered into the Mapping record. Please revisit the previous steps and ensure you have entered the necessary VAT Posting Setup combinations before proceeding.';
    ...
    begin
        CurrPage.RSVATPostingSetupStep.Page.CopyRealToTemp();
    ...
    begin
        RSPOSAuditDataToCreate := CurrPage.RSVATPostingSetupStep.Page.RSVATPostingSetupMappingDataToCreate();
    end;

    local procedure FinishAction();
    begin
        CurrPage.RSVATPostingSetupStep.Page.CreateVATPostingMappingData();
