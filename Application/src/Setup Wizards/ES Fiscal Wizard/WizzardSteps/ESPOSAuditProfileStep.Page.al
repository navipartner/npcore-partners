page 6184767 "NPR ES POS Audit Profile Step"
{
    Caption = 'ES POS Audit Profile Setup';
    Extensible = false;
    PageType = ListPart;
    ...
    layout
    {
        area(Content)
        {
            repeater(Group)
            {
                ...
                {
                    ToolTip = 'Create additional logs, usually for VES.';
                    ApplicationArea = NPRRetail;
                }
    ...
    begin
        ...
        repeat
            if (Rec."Audit Handler" = ESAuditMgt.HandlerCode()) and Rec."Audit Log Enabled" and Rec."Require Item Return Reason" then
                exit(true);
        until Rec.Next() = 0;
