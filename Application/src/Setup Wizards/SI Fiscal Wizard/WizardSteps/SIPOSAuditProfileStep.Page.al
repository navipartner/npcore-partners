page 6151610 "NPR SI POS Audit Profile Step"
{
    Caption = 'SI POS Audit Profile Setup';
    Extensible = false;
    PageType = ListPart;
    ...
    internal procedure SIPOSAuditProfileDataToCreate(): Boolean
    begin
        exit(CheckIsDataSet());
    end;
    ...
    begin
        if not Rec.FindSet() then
            exit;
        repeat
            if (Rec."Audit Handler" = SIAuditMgt.HandlerCode()) and (Rec."Audit Log Enabled" = true) then
                exit(true);
        until Rec.Next() = 0;
    end;
}