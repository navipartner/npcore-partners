page 6184806 "NPR SE POS Audit Profile Step"
{
    Caption = 'SE POS Audit Profile Setup';
    Extensible = false;
    PageType = ListPart;
    ...
    internal procedure SEPOSAuditProfileDataToCreate(): Boolean
    begin
        exit(CheckIsDataSet());
    end;
    ...
    var
        SECCWrapper: Codeunit "NPR CleanCash XCCSP Protocol";
    begin
        if not Rec.FindSet() then
            exit;
        repeat
            if (Rec."Audit Handler" = UpperCase(SECCWrapper.HandlerCode())) and (Rec."Audit Log Enabled" = true) then
                exit(true);
        until Rec.Next() = 0;
    end;
}