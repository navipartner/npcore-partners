xmlport 6151080 "NPR ExRv Vouchers"
{
    ...
    schema
    {
        ...
        {
            MaxOccurs = Once;
            tableelement(tempexrvvoucher; "NPR ExRv Voucher")
            {
                MinOccurs = Zero;
                XmlName = 'voucher';
                UseTemporary = true;
    ...
    procedure GetSourceTable(var TempExRvVoucherCopy: Record "NPR ExRv Voucher" temporary)
    begin
        TempExRvVoucherCopy.Copy(TempExRvVoucher, true);
    end;

    procedure SetSourceTable(var NewTempExRvVoucher: Record "NPR ExRv Voucher" temporary)
    begin
