page 6014633 "NPR Retail Download Major Tom"
{
    PageType = NavigatePage;
    ApplicationArea = All;
    UsageCategory = Administration;
    Caption = 'Download Major Tom setup from Azure';
        ...
        area(Content)
        {
            ...
            field("Setup File"; SetupFiles)
            {
                ...
                begin
                    ...
                    rapidstartBaseDataMgt.GetAllPackagesInBlobStorage(BaseUri + '/majortomsetup/?restype=container&comp=list'
            ...
            action(ActionFinish)
            {
                ...
                begin
                    ...
                    foreach SetupFile in SetupFileList do begin
                        CompleteUri := BaseUri + '/majortomsetup/' + SetupFile
                            + '?sv=2019-10-10&ss=b&srt=co&sp=rlx&se=2050-06-23T00:45:22Z&st=2020-06-22T16:45:22Z&spr=https&sig=' + Secret;

                        if not (ManagedDependencyMgt.DownloadFileFromAzureBlobToUserDevice(CompleteUri, 'Download Major Tom', '', 'EXE File (*.exe)|*.exe', SetupFile, ReasonPhrase)) then
                            Error(ReasonPhrase);
                    end;
