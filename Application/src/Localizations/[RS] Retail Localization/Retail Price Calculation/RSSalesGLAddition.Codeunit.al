codeunit 6151094 "NPR RS Sales GL Addition"
{
    ...
    begin
        ...
        repeat
            ...
            NivelationLines.Validate("New Price", RSRLocalizationMgt.RoundAmountToCurrencyRounding(((TempNivSalesInvLines.GetLineAmountInclVAT() - TempNivSalesInvLines."Inv. Discount Amount")
                                                                                                    / TempNivSalesInvLines.Quantity), SalesInvoiceHeader."Currency Code"));
    ...
    begin
        if PriceListLine."Price Includes VAT" then begin
            if (TempSalesInvoiceLine.GetLineAmountInclVAT() - TempSalesInvoiceLine."Inv. Discount Amount") = Round(PriceListLine."Unit Price" * TempSalesInvoiceLine.Quantity) then
                exit;
        end else
            if (TempSalesInvoiceLine.GetLineAmountExclVAT() - TempSalesInvoiceLine."Inv. Discount Amount") = Round(PriceListLine."Unit Price" * TempSalesInvoiceLine.Quantity) then
                exit;
    ...
    begin
        ...
        if TempApplicationItemLedgerEntry.IsEmpty() then begin
            if Abs(StdValueEntry."Cost per Unit") > 0 then begin
                CorrectStdValueEntry(StdCorrectionValueEntry, StdValueEntry, SalesInvoiceHeader);
                InsertCOGSCorrectionValueEntry(SumOfCOGSCostPerUnit, SumOfCOGSCostAmtAct, StdValueEntry, SalesInvoiceHeader, Abs(StdValueEntry."Cost per Unit"), TempSalesInvoiceLine.Quantity);
            end;
            exit;
        end;
        ...
        if Abs(StdValueEntry."Cost per Unit") > 0 then
            if QtyNeeded > 0 then
                InsertCOGSCorrectionValueEntry(SumOfCOGSCostPerUnit, SumOfCOGSCostAmtAct, StdValueEntry, SalesInvoiceHeader, Abs(StdValueEntry."Cost per Unit"), QtyNeeded)
            else
                if SumOfCOGSCostPerUnit = 0 then
                    InsertCOGSCorrectionValueEntry(SumOfCOGSCostPerUnit, SumOfCOGSCostAmtAct, StdValueEntry, SalesInvoiceHeader, Abs(StdValueEntry."Cost per Unit"), TempSalesInvoiceLine.Quantity);
    end;
    ...
    begin
        ...
        if QtyTakenFromEntry <= 0 then
            exit;

        ApplCostPerUnit := RSRLocalizationMgt.CalculateAppliedCostPerUnit(ApplValueEntry);
        if ApplCostPerUnit = 0 then
            exit;

        InsertCOGSCorrectionValueEntry(SumOfCOGSCostPerUnit, SumOfCOGSCostAmtAct, StdValueEntry, SalesInvoiceHeader, ApplCostPerUnit, QtyTakenFromEntry);
        RSRLocalizationMgt.SubRetValueEntryMappingRemainingQty(RSRetValueEntryMapp, QtyTakenFromEntry);
        QtyNeeded -= QtyTakenFromEntry;
    end;
    ...
    begin
        ...
        DiscountPerUnit := (TempSalesInvoiceLine."Line Discount Amount" / TempSalesInvoiceLine.Quantity) + (TempSalesInvoiceLine."Inv. Discount Amount" / TempSalesInvoiceLine.Quantity);
    ...
    begin
        CalculatedLineAmount := TempSalesInvoiceLine.GetLineAmountInclVAT() - TempSalesInvoiceLine."Inv. Discount Amount";
