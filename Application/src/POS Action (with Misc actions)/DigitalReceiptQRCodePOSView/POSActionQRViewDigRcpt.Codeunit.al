...
'let main=async({workflow:l})=>{debugger;let{qrCodeText:e,timeoutIntervalSec:t}=await l.respond();if(e){let i=await popup.open({size:{width:"450px",height:"500px"},noScroll:!0,isSupportedOnMobile:!0,ui:[{id:"title",type:"label",caption:"Scan your receipt",style:{fontWeight:"bold",textAlign:"center",fontSize:"20px"}},{id:"html",type:"html",html:"<div style=''text-align: center; margin-top: 30px''><img src=''data:image/png;base64,"+e+"'' width=''300'' height=''300''/></div>",className:"qrcode-dialog-html"}],buttons:[{id:"button_1",caption:"Close",enabled:!0,click:()=>{i.close({close:!0})},style:{textAlign:"center"}}]});t&&setTimeout(function(){i.close()},t*1e3)}};'
        );
    end;
}
