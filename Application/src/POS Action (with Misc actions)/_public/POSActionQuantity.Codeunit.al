...
'let main=async({workflow:a,context:n,popup:i,parameters:t,captions:e})=>{let r,o=runtime.getData("BUILTIN_SALELINE");if(!o._current)return;let u=parseFloat(o._current[12]);if(t.Constraint==t.Constraint["Positive Quantity Only"]&&u<0){i.error(e.MustBePositive);return}if(t.Constraint==t.Constraint["Negative Quantity Only"]&&u>0){i.error(e.MustBeNegative);return}if(n.PromptQuantity=t.InputType==0?await i.numpad({caption:e.QtyCaption,value:u}):t.InputType==1?t.ChangeToQuantity:t.InputType==2?u+t.IncrementQuantity:null,!n.PromptQuantity)return;if(t.MaxQuantityAllowed&&t.MaxQuantityAllowed!=0&&Math.abs(n.PromptQuantity)>t.MaxQuantityAllowed){i.error(e.CannotExceedMaxQty+" "+t.MaxQuantityAllowed);return}if(t.TakePhoto&&await a.respond("TakePhoto"),t.PromptUnitPriceOnNegativeInput&&(t.NegativeInput?n.PromptQuantity>0:n.PromptQuantity<0)&&(n.PromptUnitPrice=await i.numpad({caption:e.PriceCaption,value:o._current[15]}),!n.PromptUnitPrice))return;(t.NegativeInput?n.PromptQuantity>0:n.PromptQuantity<0)?r=await a.respond("AskForReturnReason"):(n.PromptForReason=!1,r="");let{postWorkflows:y}=await a.respond("ChangeQty",r);await processWorkflows(y)};async function processWorkflows(a){if(!!a)for(const[n,{mainParameters:i,customParameters:t}]of Object.entries(a))await workflow.run(n,{context:{customParameters:t},parameters:i})}'
          );
    end;
