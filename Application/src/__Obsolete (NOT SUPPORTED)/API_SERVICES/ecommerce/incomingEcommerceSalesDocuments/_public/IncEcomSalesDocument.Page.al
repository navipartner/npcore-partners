...
page 6185060 "NPR Inc Ecom Sales Document"
{
    PageType = Document;
    UsageCategory = None;
    SourceTable = "NPR Inc Ecom Sales Header";
    Caption = 'Incoming Ecommerce Sales Document (Legacy)';
    InsertAllowed = false;
    ModifyAllowed = false;
    ObsoleteState = Pending;
    ObsoleteTag = '2025-10-26';
    ObsoleteReason = 'Replaced with NPR Ecom Sales Document';
    layout
    {
        area(Content)
        {
            ...
            part(SalesLines; "NPR Inc Ecom Sales Doc Sub")
            {
                ApplicationArea = NPRRetail;
                SubPageLink = "External Document No." = field("External No."), "Document Type" = field("Document Type");
                UpdatePropagation = Both;
            }
            part(PaymentLines; "NPR Inc Ecom Sales Doc Pmt Sub")
            {
                ApplicationArea = NPRRetail;
                SubPageLink = "External Document No." = field("External No."), "Document Type" = field("Document Type");
                UpdatePropagation = Both;
        ...
        area(factboxes)
        {
            part(IncEcomDocFactBox; "NPR Inc Ecom Doc FactBox")
            {
    ...
    actions
    {
        area(Processing)
        {
            action(Process)
            {
                ...
                trigger OnAction()
                var
                    IncEcomSalesDocConfirm: Codeunit "NPR Inc Ecom Sales Doc Confirm";
                begin
                    IncEcomSalesDocConfirm.SetShowError(true);
                    IncEcomSalesDocConfirm.SetUpdateRetryCount(false);
                    IncEcomSalesDocConfirm.Run(Rec);
                end;
            }
            ...
            action(RelatedSalesDocuments)
            {
                ...
                begin
                    IncEcomSalesDocUtils.OpenRelatedSalesDocumentsFromEcomDoc(Rec);
                end;
            }
        }
        area(Promoted)
    ...
    begin
        if Rec."API Version Date" = IncEcomSalesDocImplV2.GetApiVersion() then begin
