codeunit 6060081 "NPR MCS Rec. Service API"
{
    ...
    var
        RecServiceAPI: DotNet NPRNetRecommendationServiceAPI;
        ConfirmDeleteModel: Label 'Delete model %1?';
        ErrorModelExists: Label 'Model already exists in Azure';
        ModelInfo: DotNet NPRNetModelInfo;
        ModelInfoList: DotNet NPRNetModelInfoList;
        CatalogImportStats: DotNet NPRNetCatalogImportStats;
        UsageImportStats: DotNet NPRNetUsageImportStats;
        BuildRequestInfo: DotNet NPRNetBuildRequestInfo;
        BuildParameters: DotNet NPRNetBuildParameters;
        RandomSplitterParameters: DotNet NPRNetRandomSplitterParameters;
        RecommendationBuildParameters: DotNet NPRNetRecommendationBuildParameters;
        SplitterStrategy: DotNet NPRNetSplitterStrategy;
        BuildType: DotNet NPRNetBuildType;
        OperationId: Text;
        OperationStatus: DotNet NPRNetOperationStatus;
        OperationInfo: DotNet NPRNetOperationInfo;
        RecommendedItemSetInfoList: DotNet NPRNetRecommendedItemSetInfoList;
        RecommendedItemSetInfo: DotNet NPRNetRecommendedItemSetInfo;
        RecommendedItemInfo: DotNet NPRNetRecommendedItemInfo;
        UpdateActiveBuildInfo: DotNet NPRNetUpdateActiveBuildInfo;
    ...
    begin
        ...
        RecServiceAPI := RecServiceAPI.RecommendationServiceAPI(CognitivityAPISetup.GetAPIKey1(), 'https://westus.api.cognitive.microsoft.com/recommendations/v4.0');
    ...
    procedure CreateModel(var MCSRecommendationsModel: Record "NPR MCS Recomm. Model")
    begin
        if not InitializeClient then
            exit;
    ...
    procedure UploadCatalog(var MCSRecommendationsModel: Record "NPR MCS Recomm. Model"; CatalogFilePath: Text)
    begin
        if not InitializeClient then
            exit;
    ...
    procedure UploadUsageData(var MCSRecommendationsModel: Record "NPR MCS Recomm. Model"; UsageFilePath: Text)
    begin
        if not InitializeClient then
            exit;
    ...
    procedure CreateRecommendationsBuild(var MCSRecommendationsModel: Record "NPR MCS Recomm. Model"; WaitForOperationCompletion: Boolean)
    var
        BuildId: BigInteger;
        ...
        if WaitForOperationCompletion then begin
            ...
            while (OperationIsRunning) do begin
                ...
                if (OperationInfo.Status = Format(OperationStatus.Succeeded)) then begin
                    ...
                    if ((OperationInfo.Status = Format(OperationStatus.Failed)) or (OperationInfo.Status = Format(OperationStatus.Cancelled))) then
                        OperationIsRunning := false;
    ...
    local procedure BuildModel(var MCSRecommendationsModel: Record "NPR MCS Recomm. Model")
    var
        BuildId: BigInteger;
    begin
        if not InitializeClient then
    ...
    procedure DeleteModel(var MCSRecommendationsModel: Record "NPR MCS Recomm. Model")
    begin
        if not InitializeClient then
            exit;
    ...
    procedure SetActiveBuild(var MCSRecommendationsModel: Record "NPR MCS Recomm. Model")
    begin
        if not InitializeClient then
            exit;
    ...
    procedure GetRecommendations(var MCSRecommendationsModel: Record "NPR MCS Recomm. Model"; ItemNo: Code[20]; ShowStatusDialog: Boolean)
    begin
        if not InitializeClient then
            exit;
    ...
    procedure GetRecommendationsLines(var TempMCSRecommendationsLine: Record "NPR MCS Recommendations Line" temporary; ShowStatusDialog: Boolean)
    var
        MCSRecommendationsModel: Record "NPR MCS Recomm. Model";
    begin
        if not InitializeClient then
    ...
    procedure CreateRecommendationsModel(var Rec: Record "NPR MCS Recomm. Model"; CatalogFilePath: Text; UsageFilePath: Text; WaitForOperationCompletion: Boolean)
    begin
        CreateModel(Rec);
    ...
    local procedure OnBeforeDeleteModel(var Rec: Record "NPR MCS Recomm. Model"; RunTrigger: Boolean)
    var
        RecRef: RecordRef;
