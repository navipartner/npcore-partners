page 6150846 "NPR NPCamera"
{
    ...
    begin
        if (NPCameraProf.Get('DEFAULT')) then
            SetConfJson(NPCameraProf."Pixel X", NPCameraProf."Pixel Y", NPCameraProf."Quality Value", Format(NPCameraProf."File Type"))
        else
            //Pixel values will find closest possilbe ot ideal 30000 is to get max resolution.
            SetConfJson(30000, 30000, 0.5, 'jpeg');
...
#pragma warning restore AL0749
    ...
    /// <param name="ImageStream">Will insert the image bytes into the stream.</param>
    /// <returns>Boolean indicating if it has a picture</returns>
    internal procedure TakePicture(ImageStream: InStream; "Pixel X": Integer; "Pixel Y": Integer; "Quality Value": Decimal; "File Type": Text): Boolean
    begin
        SetConfJson("Pixel X", "Pixel Y", "Quality Value", "File Type");
        if (("Quality Value" < 0.0) or ("Quality Value" > 1.00)) then Error('Quality needs to be between 0.0-1.0');
        if (not (("File Type".ToUpper() = 'PNG') or ("File Type".ToUpper() = 'JPEG'))) then Error('File type needs to be either PNG or JPEG');
    ...
    local procedure SetConfJson(pixelX: Integer; pixelY: Integer; quality: Decimal; imageType: Text);
    begin
        ProfileJson.Add('pixelX', pixelX);
        ProfileJson.Add('pixelY', pixelY);
        ProfileJson.Add('qualityVal', quality);
    ...
    var
        ...
        CamErrorLabel: Label 'Camera can''t be accessed, make sure it has permission and is not beeing used by another process.';
