    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                {
                    ToolTip = 'Specifies the default number of guests, when a new waiter pad is created at the restaurant. <Default> means that the value is going to be inherited from restaurant setup. Please note that the value may be overriden for each seating location.';
                    ApplicationArea = NPRRetail;
                }
            }
            group("Kitchen Integration")
            {
                ...
                group(Print)
                {
                    ...
                    {
                        ToolTip = 'Specifies whether quantity updates for items included in a cancelled POS sale should be sent (printed) to kitchen. Typically, in this scenario, requests for items with zero quantity will be printed to kitchen after the sale is cancelled. <Default> means that the value is going to be inherited from restaurant setup.';
                        ApplicationArea = NPRRetail;
                    }
                }
                group(KDS)
                {
                    ...
                    {
                        ApplicationArea = NPRRetail;
                        ToolTip = 'Specifies when kitchen requests are marked as Served. "Manual" requires a restaurant employee to mark requests as served. "When Production is Finished" automatically marks requests as served once all kitchen stations finish production, skipping the "Ready for Serving" step.';
                    }
