codeunit 6184672 "NPR DK SAF-T Cash Export Mgt."
{
    Access = Internal;
    ...
    local procedure StartExport(var SAFTExportHeader: Record "NPR DK SAF-T Cash Exp. Header")
    var
    ...
    internal procedure DeleteExport(var SAFTExportHeader: Record "NPR DK SAF-T Cash Exp. Header")
    var
    ...
    internal procedure RestartTaskOnExportLine(var SAFTExportLine: Record "NPR DK SAF-T Cash Export Line")
    var
    ...
    internal procedure UpdateExportStatus(var SAFTExportHeader: Record "NPR DK SAF-T Cash Exp. Header")
    var
        SAFTExportLine: Record "NPR DK SAF-T Cash Export Line";
        TypeHelper: Codeunit "Type Helper";
        Status: Enum "NPR DK SAF-T Cash Exp. Status";
        TotalCount: Integer;
    begin
    ...
    internal procedure StartExportLinesNotStartedYet(SAFTExportHeader: Record "NPR DK SAF-T Cash Exp. Header")
    var
        SAFTExportLine: Record "NPR DK SAF-T Cash Export Line";
        RunThisLine: Boolean;
        NotBefore: DateTime;
    ...
    internal procedure ShowActivityLog(SAFTExportLine: Record "NPR DK SAF-T Cash Export Line")
    var
    ...
    internal procedure ShowErrorOnExportLine(SAFTExportLine: Record "NPR DK SAF-T Cash Export Line")
    var
        ActivityLog: Record "Activity Log";
        Stream: InStream;
        ErrorMessage: Text;
    ...
    internal procedure LogSuccess(SAFTExportLine: Record "NPR DK SAF-T Cash Export Line")
    var
        ActivityLog: Record "Activity Log";
    begin
    ...
    internal procedure LogError(SAFTExportLine: Record "NPR DK SAF-T Cash Export Line")
    var
        ActivityLog: Record "Activity Log";
        ErrorMessage: Text;
    begin
    ...
    local procedure LogState(SAFTExportLine: Record "NPR DK SAF-T Cash Export Line"; Description: Text[250])
    var
        ActivityLog: Record "Activity Log";
    begin
    ...
    local procedure StartExportLines(SAFTExportHeader: Record "NPR DK SAF-T Cash Exp. Header")
    var
        SAFTExportLine: Record "NPR DK SAF-T Cash Export Line";
        NotBefore: DateTime;
        NoOfJobs: Integer;
    ...
    local procedure RunGenerateSAFTFileOnSingleLine(var SAFTExportLine: Record "NPR DK SAF-T Cash Export Line"; var NoOfJobs: Integer; var NotBefore: DateTime; SAFTExportHeader: Record "NPR DK SAF-T Cash Exp. Header")
    var
            ...
            if DoNotScheduleTask then
                ...
                SAFTExportLine."Task ID" :=
                    TaskScheduler.CreateTask(
                        Codeunit::"NPR Generate SAF-T Cash File", Codeunit::"NPR DK SAF-T Exp. Err. Handler", true, CompanyName(),
    ...
    local procedure PrepareForExport(var SAFTExportHeader: Record "NPR DK SAF-T Cash Exp. Header"): Boolean
    var
        ErrorMessageHandler: Codeunit "Error Message Handler";
        ErrorMessageManagement: Codeunit "Error Message Management";
        SAFTExportCheck: Codeunit "NPR DK SAF-T Cash Export Check";
    begin
    ...
    local procedure CreateExportLines(SAFTExportHeader: Record "NPR DK SAF-T Cash Exp. Header")
    var
    ...
    local procedure InsertSAFTExportLine(var SAFTExportLine: Record "NPR DK SAF-T Cash Export Line"; var LineNo: Integer; SAFTExportHeader: Record "NPR DK SAF-T Cash Exp. Header"; MasterData: Boolean; Description: Text; StartingDate: Date; EndingDate: Date)
    begin
        SAFTExportLine.Init();
    ...
    local procedure RemoveExportLines(var SAFTExportHeader: Record "NPR DK SAF-T Cash Exp. Header")
    var
        SAFTExportLine: Record "NPR DK SAF-T Cash Export Line";
    begin
        SAFTExportLine.SetRange(ID, SAFTExportHeader.ID);
    ...
    local procedure RemoveExportLine(var SAFTExportLine: Record "NPR DK SAF-T Cash Export Line")
    begin
        CancelTask(SAFTExportLine);
        SAFTExportLine.Delete(true);
    end;

    local procedure CancelTask(SAFTExportLine: Record "NPR DK SAF-T Cash Export Line")
    var
    ...
    internal procedure GenerateZipFileWithCheck(SAFTExportHeader: Record "NPR DK SAF-T Cash Exp. Header")
    var
    ...
    internal procedure GenerateZipFile(SAFTExportHeader: Record "NPR DK SAF-T Cash Exp. Header")
    begin
        if SAFTExportHeader.Status <> SAFTExportHeader.Status::Completed then
            exit;
    ...
    internal procedure BuildZipFilesWithAllRelatedXmlFiles(SAFTExportHeader: Record "NPR DK SAF-T Cash Exp. Header")
    var
        CompanyInformation: Record "Company Information";
        SAFTExportLine: Record "NPR DK SAF-T Cash Export Line";
        SAFTXMLHelper: Codeunit "NPR DK SAF-T XML Helper";
    ...
    internal procedure GenerateZipFileFromSavedFiles(SAFTExportHeader: Record "NPR DK SAF-T Cash Exp. Header")
    begin
    ...
    internal procedure DownloadZipFileFromExportHeader(SAFTExportHeader: Record "NPR DK SAF-T Cash Exp. Header")
    var
        SAFTExportFile: Record "NPR DK SAF-T Cash Export Zip";
        ZipFileInStream: InStream;
        FileName: Text;
    begin
        SAFTExportFile.SetRange("Export ID", SAFTExportHeader.ID);
        SAFTExportFile.SetAutoCalcFields("SAF-T File");
    ...
    internal procedure DownloadExportFile(SAFTExportFile: Record "NPR DK SAF-T Cash Export Zip")
    var
        ZipFileInStream: InStream;
        FileName: Text;
    begin
    ...
    internal procedure ZipMultipleXMLFilesInServerFolder(var SAFTExportHeader: Record "NPR DK SAF-T Cash Exp. Header")
    var
    ...
    begin
        ...
        SAFTCashExportFile.SetAutoCalcFields("SAF-T File");
    ...
    local procedure SaveZipOfMultipleXMLFiles(SAFTExportHeader: Record "NPR DK SAF-T Cash Exp. Header")
    var
        SAFTCashExportFile: Record "NPR DK SAF-T Cash Export File";
        DataCompression: Codeunit "Data Compression";
        EntryFileInStream: InStream;
    ...
    local procedure SaveZipFile(var DataCompression: Codeunit "Data Compression"; var FilesHandled: Integer; SAFTExportHeader: Record "NPR DK SAF-T Cash Exp. Header")
    var
        SAFTExportFile: Record "NPR DK SAF-T Cash Export Zip";
    ...
    local procedure ExportZipFile(var DataCompression: Codeunit "Data Compression"; var FilesHandled: Integer; var ZipNo: Integer; SAFTExportHeader: Record "NPR DK SAF-T Cash Exp. Header")
    var
        ZipTempBlob: Codeunit "Temp Blob";
        SAFTCashExportZip: Record "NPR DK SAF-T Cash Export Zip";
    ...
    internal procedure CheckNoFilesInFolder(SAFTExportHeader: Record "NPR DK SAF-T Cash Exp. Header")
    var
        SAFTCashExportFile: Record "NPR DK SAF-T Cash Export File";
        ErrorMessageManagement: Codeunit "Error Message Management";
    begin
    ...
    internal procedure ClearExportFiles(SAFTExportHeader: Record "NPR DK SAF-T Cash Exp. Header")
    var
        SAFTExportFile: Record "NPR DK SAF-T Cash Export Zip";
    begin
        SAFTExportFile.SetRange("Export ID", SAFTExportHeader.ID);
    ...
    local procedure InitExportFile(var SAFTExportFile: Record "NPR DK SAF-T Cash Export Zip"; SAFTExportHeader: Record "NPR DK SAF-T Cash Exp. Header")
    var
        FileNo: Integer;
    begin
    ...
    internal procedure InitExportFile(var SAFTExportFile: Record "NPR DK SAF-T Cash Export File"; ExportHeaderID: Integer)
    var
        FileNo: Integer;
    begin
        SAFTExportFile.Reset();
    ...
    internal procedure ExportFilesExist(SAFTExportHeader: Record "NPR DK SAF-T Cash Exp. Header"): Boolean
    var
        SAFTExportFile: Record "NPR DK SAF-T Cash Export Zip";
    begin
        SAFTExportFile.SetRange("Export ID", SAFTExportHeader.ID);
    ...
    //     exit(DKFiscalizationSetup."Not Applicable VAT Code");
    // end;

    local procedure IsSessionActive(SAFTExportLine: Record "NPR DK SAF-T Cash Export Line"): Boolean
    var
    ...
    local procedure IsSessionLoggedOff(SAFTExportLine: Record "NPR DK SAF-T Cash Export Line"): Boolean
    var
        SAFTExportHeader: Record "NPR DK SAF-T Cash Exp. Header";
    ...
    local procedure CheckStatus(Status: Enum "NPR DK SAF-T Cash Exp. Status"; Question: Text): Boolean
    var
        SAFTExportHeader: Record "NPR DK SAF-T Cash Exp. Header";
        StatusMessage: Text;
    begin
    ...
    local procedure CheckLineStatusForRestart(var SAFTExportLine: Record "NPR DK SAF-T Cash Export Line"): Boolean;
    begin
