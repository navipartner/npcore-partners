table 6150864 "NPR RS EI Aux Purch. CrMem Hdr"
{
    Access = Internal;
    Caption = 'RS EI Aux Purchase Cr. Memo Header';
    DataClassification = CustomerContent;

    fields
    {
        field(1; "Purch. Cr. Memo Hdr. SystemId"; Guid)
        {
            Caption = 'Purchase Cr. Memo Header SystemId';
            TableRelation = "Purch. Cr. Memo Hdr.".SystemId;
            DataClassification = CustomerContent;
        }
    ...
    keys
    {
        key(PK; "Purch. Cr. Memo Hdr. SystemId")
        {
            Clustered = true;
        }
    }
    ...
    internal procedure ReadRSEIAuxPurchCrMemHdrFields(PurchCrMemoHdr: Record "Purch. Cr. Memo Hdr.")
    var
        RSEInvoiceMgt: Codeunit "NPR RS E-Invoice Mgt.";
    begin
        if not RSEInvoiceMgt.IsRSEInvoiceEnabled() then
            exit;
        if not Rec.Get(PurchCrMemoHdr.SystemId) then begin
            Rec.Init();
            Rec."Purch. Cr. Memo Hdr. SystemId" := PurchCrMemoHdr.SystemId;
        end;
    end;

    internal procedure SaveRSEIAuxPurchCrMemoHdrFields()
    begin
        if not Insert() then
            Modify();
    end;

    internal procedure SetRSEIAuxPurchCrMemoHdrInvoiceStatus(DocumentNo: Code[20]; RSEInvoiceStatus: Enum "NPR RS E-Invoice Status")
    var
        PurchCrMemoHdr: Record "Purch. Cr. Memo Hdr.";
    begin
        if not PurchCrMemoHdr.Get(DocumentNo) then
            exit;
        ReadRSEIAuxPurchCrMemHdrFields(PurchCrMemoHdr);
        "NPR RS EI Invoice Status" := RSEInvoiceStatus;
        SaveRSEIAuxPurchCrMemoHdrFields();
    end;
#endif
}