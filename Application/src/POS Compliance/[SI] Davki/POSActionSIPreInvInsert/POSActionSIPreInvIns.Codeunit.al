codeunit 6184559 "NPR POS Action: SIPreInv Ins." implements "NPR IPOS Workflow"
{
    ...
    internal procedure InputAuditPreInvoiceNumbersTest(SetNumberNo: Text; SerialNumberNo: Text; POSSale: Record "NPR POS Sale")
    var
        SIPOSSale: Record "NPR SI POS Sale";
    begin
        SIPOSSale."POS Sale SystemId" := POSSale.SystemId;
        SIPOSSale."SI Set Number" := CopyStr(SetNumberNo, 1, MaxStrLen(SIPOSSale."SI Set Number"));
        SIPOSSale."SI Serial Number" := CopyStr(SerialNumberNo, 1, MaxStrLen(SIPOSSale."SI Serial Number"));
        if not SIPOSSale.Insert() then
            SIPOSSale.Modify();
    end;
