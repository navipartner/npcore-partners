codeunit 6248729 "NPR DE Fiscal Thermal Print"
{
    Access = Internal;
    ...
    begin
        ...
        Printer.ProcessBuffer(Codeunit::"NPR DE Fiscal Thermal Print", Enum::"NPR Line Printer Device"::Epson, TempPrinterDeviceSettings);
    end;
    ...
    begin
        ...
        if AuditLogFound then
            AddStornoHeaderIfCancellation(DEPOSAuditLogAuxInfo);

        AddReceiptInformation(POSEntry);

        if not AuditLogFound then begin
            AddTSEOutageWarning();
            AddPapercutCommand();
            exit;
        end;
    ...
    var
        ...
        TSEOutageLine1Lbl: Label 'TSE-AUSFALL', Locked = true;
        TSEOutageLine2Lbl: Label 'Beleg nicht TSE-signiert', Locked = true;
    begin
        Printer.SetPadChar('*');
        Printer.AddLine('', 0);
        Printer.SetPadChar(' ');
        Printer.SetFont(B21FontLbl);
        Printer.AddLine(TSEOutageLine1Lbl, 1);
        Printer.SetFont(A11FontLbl);
        Printer.AddLine(TSEOutageLine2Lbl, 1);
        Printer.SetPadChar('*');
        Printer.AddLine('', 0);
        Printer.SetPadChar(' ');
    end;

    local procedure AddStornoHeaderIfCancellation(DEPOSAuditLogAuxInfo: Record "NPR DE POS Audit Log Aux. Info")
    var
        B21FontLbl: Label 'B21', Locked = true;
        A11FontLbl: Label 'A11', Locked = true;
        StornoLbl: Label 'STORNO', Locked = true;
    begin
        if DEPOSAuditLogAuxInfo."Fiskaly Transaction Type" <> DEPOSAuditLogAuxInfo."Fiskaly Transaction Type"::CANCELLATION then
            exit;
        Printer.SetFont(B21FontLbl);
        Printer.AddLine(StornoLbl, 1);
        Printer.SetFont(A11FontLbl);
    end;

    local procedure AddFiskalyTextBlock(DEPOSAuditLogAuxInfo: Record "NPR DE POS Audit Log Aux. Info")
    var
        TransactionNoLbl: Label 'Transaktions-Nr.: %1', Locked = true;
        StartTimeLbl: Label 'Beginn: %1', Locked = true;
        FinishTimeLbl: Label 'Ende: %1', Locked = true;
        ISOTimeFormatTok: Label '<Year4>-<Month,2>-<Day,2>T<Hours24,2>:<Minutes,2>:<Seconds,2>', Locked = true;
    begin
        if DEPOSAuditLogAuxInfo."Transaction Number" <> 0 then
            Printer.AddLine(StrSubstNo(TransactionNoLbl, DEPOSAuditLogAuxInfo."Transaction Number"), 1);
        if DEPOSAuditLogAuxInfo."Start Time" <> 0DT then
            Printer.AddLine(StrSubstNo(StartTimeLbl, Format(DEPOSAuditLogAuxInfo."Start Time", 0, ISOTimeFormatTok)), 1);
        if DEPOSAuditLogAuxInfo."Finish Time" <> 0DT then
            Printer.AddLine(StrSubstNo(FinishTimeLbl, Format(DEPOSAuditLogAuxInfo."Finish Time", 0, ISOTimeFormatTok)), 1);
    end;

    local procedure AddFiskalyQRCode(QRData: Text)
    var
    ...
    begin
        ...
        Printer.AddBarcode(QRFontLbl, QRData, 5, true, 5);
        Printer.SetFont(A11FontLbl);
    end;
    ...
    var
        ...
        QRFontLbl: Label 'QR', Locked = true;
        PhoneNoLbl: Label 'Tel.: ', Locked = true;
        VATRegistrationNoLbl: Label 'USt-IdNr.: ', Locked = true;
        EMailLbl: Label 'E-Mail: ', Locked = true;
        HomePageLbl: Label 'Webseite: ', Locked = true;
        CopyLbl: Label '*** Zweitbeleg ***', Locked = true;
        DescriptionLbl: Label 'Beschreibung', Locked = true;
        QuantityLbl: Label 'Menge', Locked = true;
        AmountLbl: Label 'Betrag', Locked = true;
        LineDiscountLbl: Label 'Rabatt', Locked = true;
        TotalLbl: Label 'Summe', Locked = true;
        VATRateHeaderLbl: Label 'MwSt %', Locked = true;
        VATBaseHeaderLbl: Label 'Netto', Locked = true;
        VATAmountHeaderLbl: Label 'MwSt', Locked = true;
        VATGrossHeaderLbl: Label 'Brutto', Locked = true;
        ReceiptNoLbl: Label 'Beleg-Nr: %1', Locked = true;
        POSUnitLbl: Label 'Kasse: %1', Locked = true;
        SalespersonLbl: Label 'Bediener: %1 / %2', Locked = true;
        DEDateFormatTok: Label '<Day,2>.<Month,2>.<Year4>', Locked = true;
        DETimeFormatTok: Label '<Hours24,2>:<Minutes,2>:<Seconds,2>', Locked = true;
    begin
        Printer.SetFont(A11FontLbl);
        ...
        // Sale Lines
        POSEntrySalesLine.SetLoadFields("No.", Description, Type, "Variant Code", Quantity, "Unit Price", "Amount Incl. VAT", "Line Discount Amount Incl. VAT");
        ...
        // VAT breakdown per rate (DE / KassenSichV compliance)
        POSEntryTaxLine.SetLoadFields("Tax %", "Tax Base Amount", "Tax Amount", "Amount Including Tax");
        POSEntryTaxLine.SetRange("POS Entry No.", POSEntry."Entry No.");
        POSEntryTaxLine.SetFilter("Tax Amount", '<>%1', 0);
        if POSEntryTaxLine.FindSet() then begin
            Printer.SetFourColumnDistribution(0.18, 0.27, 0.27, 0.28);
            Printer.SetBold(true);
            Printer.AddTextField(1, 0, VATRateHeaderLbl);
            Printer.AddTextField(2, 2, VATBaseHeaderLbl);
            Printer.AddTextField(3, 2, VATAmountHeaderLbl);
            Printer.AddTextField(4, 2, VATGrossHeaderLbl);
            Printer.SetBold(false);
            repeat
                Printer.AddTextField(1, 0, Format(POSEntryTaxLine."Tax %", 0, '<Precision,1:1><Standard Format,2>') + '%');
                Printer.AddTextField(2, 2, Format(POSEntryTaxLine."Tax Base Amount", 0, '<Precision,2:2><Standard Format,2>') + '€');
                Printer.AddTextField(3, 2, Format(POSEntryTaxLine."Tax Amount", 0, '<Precision,2:2><Standard Format,2>') + '€');
                Printer.AddTextField(4, 2, Format(POSEntryTaxLine."Amount Including Tax", 0, '<Precision,2:2><Standard Format,2>') + '€');
        ...
        // Total line
        Printer.SetBold(true);
        Printer.AddTextField(1, 0, TotalLbl);
        Printer.AddTextField(2, 0, '');
        ...
        // Receipt number / POS Unit / Salesperson (DE labels)
        Printer.AddLine(StrSubstNo(ReceiptNoLbl, POSEntry."Document No."), 1);
        Printer.AddLine(StrSubstNo(POSUnitLbl, POSEntry."POS Unit No."), 1);
        if SalespersonPurchaser.Get(POSEntry."Salesperson Code") then
            Printer.AddLine(StrSubstNo(SalespersonLbl, SalespersonPurchaser.Code, SalespersonPurchaser.Name), 1);

        // Receipt date and time info (German format: DD.MM.YYYY HH:MM:SS)
        Printer.AddLine(Format(POSEntry."Entry Date", 0, DEDateFormatTok) + ' ' + Format(POSEntry."Ending Time", 0, DETimeFormatTok), 1);
    end;
}
