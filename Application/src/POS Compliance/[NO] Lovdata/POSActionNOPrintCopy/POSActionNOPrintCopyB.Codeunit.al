codeunit 6248439 "NPR POS Action: NO Print CopyB"
{
    Access = Internal;
    ...
    begin
        ...
        if PrintPhysicalReceipts then
            PrintEntry(POSEntry, SettingOption in [SettingOption::"Choose Receipt Large", SettingOption::"Last Receipt Large"]);
    ...
    local procedure PrintEntry(POSEntry: Record "NPR POS Entry"; Large: Boolean)
    var
        ...
        RecRef: RecordRef;
        IsReprint: Boolean;
        ReprintNotAllowedErrMsg: Label 'Additional reprints are not allowed for current sale (%1 %2).', Comment = '%1 = POS Entry No. field caption, %2 = POS Entry No.';
    begin
        ...
        if (IsReprint) and (POSEntryOutputLog.Count() > 1) then
