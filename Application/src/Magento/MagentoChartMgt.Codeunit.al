codeunit 6151480 "NPR Magento Chart Mgt."
{
    ...
    var
        Date: Record Date;
        PlaceHolder1NextLbl: Label '<C%1+7%1>', Locked = true;
        PlaceHolder1PreviousLbl: Label '<C%1-7%1>', Locked = true;
        PlaceHolder2Lbl: Label '<-%1%2>', Locked = true;
    begin
        case Period of
            Period::Next:
                begin
                    Enddate := CalcDate(StrSubstNo(PlaceHolder1NextLbl, BusChartBuf.GetPeriodLength()), BusChartBuf."Period Filter End Date");
                end;
            Period::Previous:
                begin
                    Enddate := CalcDate(StrSubstNo(PlaceHolder1PreviousLbl, BusChartBuf.GetPeriodLength()), BusChartBuf."Period Filter End Date");
                end;
        end;
        ...
        Date.SetFilter("Period Start", '%1..', CalcDate(StrSubstNo(PlaceHolder2Lbl, 7, BusChartBuf.GetPeriodLength()), Enddate));
