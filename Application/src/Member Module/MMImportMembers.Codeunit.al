codeunit 6060132 "NPR MM Import Members"
{
    ...
    begin
        ...
        case UpperCase(dateMask) of
            'YYYYMMDD':
                if (not Evaluate(rDate, StrSubstNo(PlaceHolder3Lbl, CopyStr(fieldValue, 1, 4), CopyStr(fieldValue, 5, 2), CopyStr(fieldValue, 7, 2)), 9)) then
                    Error(INVALID_DATE, fieldValue, fieldCaptionName, GLineCount, dateMask);

            'YYYY-MM-DD':
                if (not Evaluate(rDate, StrSubstNo(PlaceHolder3Lbl, CopyStr(fieldValue, 1, 4), CopyStr(fieldValue, 6, 2), CopyStr(fieldValue, 9, 2)), 9)) then
                    Error(INVALID_DATE, fieldValue, fieldCaptionName, GLineCount, dateMask);
