        ...
        case FunctionId of
            ...
            1:
                begin
                    ...
                    MemberArrival.AddToastMemberScannedData(MemberCardEntryNo, Response);
                    POSActionMemberMgtWF3B.AddSelectMembershipToastHeadline(Response);
                end;
            2:
...
'let main=async({workflow:s,context:i,popup:r,captions:a,parameters:e})=>{e.Function<0&&(e.Function=e.Function["Member Arrival"]);let u=await s.respond("CheckMemberInitialized");u!=null&&u.memberExternalCardNo!=null&&u.memberExternalCardNo!==""&&(i.memberCardInput=u.memberExternalCardNo),e.DoNotSuggestTrackedMemberCard&&(i.memberCardInput="");let g=a.DialogTitle.substitute(e.Function);if(e.DefaultInputValue.length==0&&e.DialogPrompt<=e.DialogPrompt["Member Card Number"]&&(i.memberCardInput=await r.input({caption:a.MemberCardPrompt,title:a.windowTitle,value:i.memberCardInput}),i.memberCardInput===null))return;if(e.DefaultInputValue.length>0&&(i.memberCardInput=e.DefaultInputValue),e.Function>=e.Function["Regret Membership Entry"]&&e.Function<=e.Function["Cancel Membership"]){let n=await s.respond("GetMembershipAlterationLookup");if(i.memberCardInput=n.cardnumber,n.data?.length==0){await r.error({title:a.windowTitle,caption:n.notFoundMessage});return}let l=data.createArrayDriver(n.data),c=data.createDataSource(l);c.loadAll=!1;let o=await r.lookup({title:n.title,configuration:{className:"custom-lookup",styleSheet:"",layout:n.layout,result:m=>m?m.map(b=>b?b.itemno:null):null},source:c});if(o===null||o.length===0)return;i.itemNumber=o[0].itemno}let t=await s.respond("DoManageMembership");if(e.Function==e.Function["View Membership Entry"]){let n=data.createArrayDriver(t.data),l=data.createDataSource(n),c=await r.lookup({title:t.title,configuration:{className:"custom-lookup",styleSheet:"",layout:t.layout},source:l})}const d=e.ToastMessageTimer!==null&&e.ToastMessageTimer!==void 0&&e.ToastMessageTimer!==0?e.ToastMessageTimer:5;if(t.MemberScanned&&d>0){const n=e.Function==e.Function["Select Membership"];toast.memberScanned({memberImg:t.MemberScanned.ImageDataUrl,memberName:t.MemberScanned.Name,validForAdmission:n?void 0:t.MemberScanned.Valid,hideAfter:d,memberExpiry:n?void 0:t.MemberScanned.ExpiryDate,memberHeadline:t.MemberScanned.Headline,content:[{caption:t.MemberScanned.MembershipCodeCaption,value:t.MemberScanned.MembershipCodeDescription}]})}const M=(e.ToastSuccessMessageTimer??0)!==0?e.ToastSuccessMessageTimer:5;if(t&&(t.success===!0||!t.error)){let n=a.SuccessMessage;switch(Number(e.Function)){case 11:{n=a.CancelAutoRenewSuccess;break}case 12:{if(t.success===!1)return;n=a.TerminateSubscriptionSuccess;break}case 0:case 1:case 2:case 3:case 4:case 5:case 6:case 7:case 8:case 9:case 10:return}toast.success(n,{title:a.SuccessTitle,hideAfter:M})}};'
        )
    end;
