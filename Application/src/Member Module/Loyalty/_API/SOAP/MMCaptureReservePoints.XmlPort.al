xmlport 6014416 "NPR MM CaptureReservePoints"
{
    ...
    schema
    {
        textelement(CaptureReservePoints)
        {
            MaxOccurs = Once;
            textelement(Request)
            {
                ...
                textelement(CaptureReservation)
                {
                    MaxOccurs = Once;
                    tableelement(TmpRegisterCaptureRequest; "NPR MM Reg. Sales Buffer")
                    {
                        MaxOccurs = Once;
                        XmlName = 'Line';
                        UseTemporary = true;
                        fieldattribute(Type; TmpRegisterCaptureRequest.Type)
                        {
                        }
                        fieldattribute(AuthorizationCode; TmpRegisterCaptureRequest."Authorization Code")
                        {
                        }
                    }
                }
    ...
    internal procedure GetRequest(var TmpAuthorization: Record "NPR MM Loy. LedgerEntry (Srvr)" temporary; var TmpCaptureLine: Record "NPR MM Reg. Sales Buffer" temporary)
    begin
        ...
        if (TmpRegisterCaptureRequest.FindSet()) then begin
            repeat
                TmpCaptureLine.TransferFields(TmpRegisterCaptureRequest, true);
                TmpCaptureLine.Insert();
            until (TmpRegisterCaptureRequest.Next() = 0);
        end;
    end;
    ...
    begin
        ...
        if (not TmpPoints.FindFirst()) then begin
            SetErrorResponse('Capture Reservation Failed.', '-1098');
            exit;
        end;
