codeunit 6014662 "NPR Stock-Take Manager"
{
    // 
    ...
    var
        ...
        Varerec: Record Item;
        Licens: Codeunit "NPR License Information";
    ...
    procedure CheckConfigurationCode(CurrentConfigurationCode: Code[20]; var StockTakeWorksheet: Record "NPR Stock-Take Worksheet")
    begin
        //-NPR4.16
    ...
    procedure SetConfigurationCode(CurrentConfigurationCode: Code[20]; var StockTakeWorksheet: Record "NPR Stock-Take Worksheet")
    begin
        //-NPR4.16
    ...
    procedure LookupConfigurationCode(var CurrentConfigurationCode: Code[20]; var StockTakeWorksheet: Record "NPR Stock-Take Worksheet")
    var
    ...
    procedure CheckWorksheetName(CurrentWorksheetName: Code[20]; var StockTakeWorksheet: Record "NPR Stock-Take Worksheet")
    var
        NPPhysInvJnlBatch2: Record "NPR Stock-Take Worksheet";
    begin
        //-NPR4.16
    ...
    procedure SetWorksheetName(CurrentWorksheetName: Code[20]; var StockTakeWorksheet: Record "NPR Stock-Take Worksheet")
    begin
        //-NPR4.16
        StockTakeWorksheet.FilterGroup := 2;
    ...
    procedure LookupWorksheetName(var CurrentWorksheetName: Code[20]; var StockTakeWorksheet: Record "NPR Stock-Take Worksheet")
    var
        StockTakeWorksheet2: Record "NPR Stock-Take Worksheet";
    begin
        //-NPR4.16
        Commit;
    ...
    procedure TranslateBarcode(var StockTakeWorksheetLine: Record "NPR Stock-Take Worksheet Line")
    begin
        //-NPR4.16
    ...
    begin
        ...
        //-NPR4.21
        if (StrLen(Barcode) <= MaxStrLen(Item."NPR Label Barcode")) then begin
            Item.SetRange("NPR Label Barcode", Barcode);
    ...
    procedure AssignItemCost(var StockTakeWorksheetLine: Record "NPR Stock-Take Worksheet Line")
    var
        StockTakeConfig: Record "NPR Stock-Take Configuration";
    begin
        //-NPR4.16
    ...
    begin
        ...
        case SuggestedUnitCostSource of
            StockTakeConfig."Suggested Unit Cost Source"::UNIT_COST:
                exit(Item."Unit Cost");
            StockTakeConfig."Suggested Unit Cost Source"::LAST_DIRECT_COST:
                exit(Item."Last Direct Cost");
            StockTakeConfig."Suggested Unit Cost Source"::STANDARD_COST:
    ...
    procedure TransferDimensions(StockTakeWorksheetLine: Record "NPR Stock-Take Worksheet Line"; var ItemJnlLine: Record "Item Journal Line")
    var
    ...
    procedure CreateDefaultDim(var StockTakeWorksheetLine: Record "NPR Stock-Take Worksheet Line")
    begin
        //-NPR4.16
        CreateDim(StockTakeWorksheetLine,
                   DATABASE::"NPR Stock-Take Configuration", StockTakeWorksheetLine."Stock-Take Config Code",
                   DATABASE::Item, StockTakeWorksheetLine."Item No.",
                   DATABASE::"Salesperson/Purchaser", '');
        //+NPR4.16
    end;

    procedure CreateDim(var StockTakeWorksheetLine: Record "NPR Stock-Take Worksheet Line"; Type1: Integer; No1: Code[20]; Type2: Integer; No2: Code[20]; Type3: Integer; No3: Code[20])
    var
    ...
    procedure ValidateShortcutDimCode(var StockTakeWorksheetLine: Record "NPR Stock-Take Worksheet Line"; FieldNumber: Integer; var ShortcutDimCode: Code[20]; DimensionSetID: Integer)
    var
        DimMgt: Codeunit DimensionManagement;
    begin
        //-NPR4.16
    ...
    procedure ImportPreHandler(StockTakeWorksheet: Record "NPR Stock-Take Worksheet")
    var
        StockTakeConfig: Record "NPR Stock-Take Configuration";
    ...
    begin
        ...
        with StockTakeConfig do begin
            ...
            case "Session Based Loading" of
                "Session Based Loading"::APPEND:
                    ; // do nothing
                "Session Based Loading"::APPEND_UNTIL_TRANSFERRED:
    ...
    procedure ImportPostHandler(StockTakeWorksheet: Record "NPR Stock-Take Worksheet")
    var
        StockTakeConfig: Record "NPR Stock-Take Configuration";
        StockTakeWorksheetLine: Record "NPR Stock-Take Worksheet Line";
    ...
    procedure AggregateBatch(StockTakeWorksheet: Record "NPR Stock-Take Worksheet")
    var
        tmpWorksheetLine: Record "NPR Stock-Take Worksheet Line" temporary;
    ...
    procedure TransferToItemInvJnl(StockTakeWorksheet: Record "NPR Stock-Take Worksheet"; TransferAction: Option; PostingDate: Date)
    var
        StockTakeWorksheet2: Record "NPR Stock-Take Worksheet";
        StockTakeConfig: Record "NPR Stock-Take Configuration";
    ...
    procedure TransferPreHandler(StockTakeWorksheet: Record "NPR Stock-Take Worksheet")
    var
        StockTakeConfig: Record "NPR Stock-Take Configuration";
        WorksheetLine: Record "NPR Stock-Take Worksheet Line";
        WorksheetLine2: Record "NPR Stock-Take Worksheet Line";
    ...
    begin
        ...
        if (NotTranslatedCount <> 0) then begin
            case StockTakeConfig."Barcode Not Accepted" of
                ...
                StockTakeConfig."Barcode Not Accepted"::IGNORE:
                    ; // Do nothing
                else
                    Error('Unsupported option in TransferPreHandler ()');
            end;
        ...
        if (NotTranslatedCount <> 0) then begin
            case StockTakeConfig."Blocked Item" of
                ...
                StockTakeConfig."Blocked Item"::IGNORE:
                    ; // Do nothing
    ...
    procedure TransferHandler(StockTakeWorksheet: Record "NPR Stock-Take Worksheet"; FilterType: Option JOURNAL,BATCH,BOTH; var ItemJnlLine: Record "Item Journal Line"; PostingDate: Date)
    var
        StockTakeConfig: Record "NPR Stock-Take Configuration";
        Item: Record Item;
    ...
    procedure ApplyCountToItemInvJnl(var ItemJnlLine: Record "Item Journal Line"; var tmpWorksheetLine: Record "NPR Stock-Take Worksheet Line" temporary; ItemsInScopeNotCounted: Integer; SuppressNotCounted: Boolean; StockTakeConfigCode: Code[20])
    var
        Item: Record Item;
        StockTakeConfig: Record "NPR Stock-Take Configuration";
        WorksheetLine: Record "NPR Stock-Take Worksheet Line";
        StockTakeWorksheet: Record "NPR Stock-Take Worksheet";
        tmpWorksheetLine2: Record "NPR Stock-Take Worksheet Line" temporary;
    ...
    procedure AdjustForStockTake(ItemsInScopeNotCounted: Integer; SuppressNotCounted: Boolean; StockTakeConfig: Record "NPR Stock-Take Configuration"; var ItemJnlLine: Record "Item Journal Line") MarkForDelete: Boolean
    begin
    ...
    procedure AdjustForAdjustment(ItemsInScopeNotCounted: Integer; SuppressNotCounted: Boolean; StockTakeConfig: Record "NPR Stock-Take Configuration"; var ItemJnlLine: Record "Item Journal Line") MarkForDelete: Boolean
    begin
    ...
    procedure AdjustForPurchase(ItemsInScopeNotCounted: Integer; SuppressNotCounted: Boolean; StockTakeConfig: Record "NPR Stock-Take Configuration"; var ItemJnlLine: Record "Item Journal Line") MarkForDelete: Boolean
    begin
    ...
    procedure TransferPostHandler(StockTakeWorksheet: Record "NPR Stock-Take Worksheet"; var ItemJnlLine: Record "Item Journal Line"; TransferAction: Option; var ResponseText: Text)
    var
        WorksheetLine: Record "NPR Stock-Take Worksheet Line";
        StockTakeConfig: Record "NPR Stock-Take Configuration";
        ItemJnlMgt: Codeunit ItemJnlManagement;
    begin
        ...
        case TransferAction of
            StockTakeConfig."Transfer Action"::TRANSFER:
                ; // Done
    ...
    procedure GetItemJnlLine(StockTakeWorksheet: Record "NPR Stock-Take Worksheet"; var ItemJnlLine: Record "Item Journal Line"; WithCheck: Boolean)
    var
        StockTakeConfig: Record "NPR Stock-Take Configuration";
        ItemJnlBatch: Record "Item Journal Batch";
    ...
    procedure CreateItemJnlLine(StockTakeWorksheet: Record "NPR Stock-Take Worksheet"; FilterType: Option; CountingMethod: Option; ItemJnlLine: Record "Item Journal Line"; var tmpWorksheetLine: Record "NPR Stock-Take Worksheet Line" temporary; PostingDate: Date)
    var
        StockTakeConfig: Record "NPR Stock-Take Configuration";
        Item: Record Item;
        CalcQtyOnHand: Report "NPR Retail Calc. Inv.";
        ItemsNotOnInventory: Boolean;
        tmpAdjmtWorksheetLine: Record "NPR Stock-Take Worksheet Line" temporary;
        tmpCompressedWorksheet: Record "NPR Stock-Take Worksheet Line" temporary;
    ...
    procedure CreateAdhocStockTakeLine(StockTakeConfigCode: Code[20]; ItemJnlLine: Record "Item Journal Line"; var tmpWorksheetLine: Record "NPR Stock-Take Worksheet Line" temporary; PostingDate: Date)
    var
        StockTakeConfig: Record "NPR Stock-Take Configuration";
        CalcQtyOnHand: Report "NPR Retail Calc. Inv.";
        Item: Record Item;
    ...
    procedure CreateAdhocItemJnlLine(StockTakeConfigCode: Code[20]; ItemJnlLine: Record "Item Journal Line"; var tmpWorksheetLine: Record "NPR Stock-Take Worksheet Line" temporary; PostingDate: Date; WithCompress: Boolean; Distinct: Boolean)
    var
        StockTakeConfig: Record "NPR Stock-Take Configuration";
        LineNo: Integer;
        tmpAggrWorksheetLine: Record "NPR Stock-Take Worksheet Line" temporary;
        ItemJnlLineCopy: Record "Item Journal Line";
    begin
        //-NPR4.16
        StockTakeConfig.Get(StockTakeConfigCode);
    ...
    procedure GetItemInventory(var Item: Record Item; var tmpAdjmtWorksheetLine: Record "NPR Stock-Take Worksheet Line" temporary; CountingMethod: Option)
    var
        LineNo: Integer;
        ItemVariant: Record "Item Variant";
        StockTakeConfig: Record "NPR Stock-Take Configuration";
    ...
    procedure SetItemFilter(StockTakeWorksheet: Record "NPR Stock-Take Worksheet"; FilterType: Option JOURNAL,BATCH,BOTH; var Item: Record Item)
    var
        StockTakeConfig: Record "NPR Stock-Take Configuration";
        FilterString: array[5] of Text[200];
    begin
        ...
        if (FilterString[1] <> '') then
            Item.SetFilter("NPR Item Group", FilterString[1]);
    ...
    procedure GetConfigurationFilterValues(StockTakeWorksheet: Record "NPR Stock-Take Worksheet"; var FilterString: array[5] of Text[200])
    var
        StockTakeConfig: Record "NPR Stock-Take Configuration";
    begin
        //-NPR4.16
    ...
    procedure GetWorksheetFilterValues(StockTakeWorksheet: Record "NPR Stock-Take Worksheet"; var FilterString: array[5] of Text[200])
    begin
        //-NPR4.16
    ...
    procedure RetailPrint(var StockTakeWorksheetLine: Record "NPR Stock-Take Worksheet Line")
    var
    ...
    begin
        ...
        Commit;
        PAGE.RunModal(PAGE::"NPR Retail Journal Print", RetailJournalLine);
        RetailJournalLine.DeleteAll;
        //+NPR5.46 [329899]
    end;

    local procedure FillRetailJournalLine(SelectionID: Code[40]; var StockTakeWorksheetLine: Record "NPR Stock-Take Worksheet Line"; var RetailJournalLine: Record "NPR Retail Journal Line")
    var
        LineCount: Integer;
    begin
    ...
    procedure AreaTopUp(var TopUpToWorksheet: Record "NPR Stock-Take Worksheet")
    var
        TopUpFromWorksheet: Record "NPR Stock-Take Worksheet";
        TopUpResultWorksheet: Record "NPR Stock-Take Worksheet";
        LineLeft: Record "NPR Stock-Take Worksheet Line";
        LineRight: Record "NPR Stock-Take Worksheet Line";
        TmpWorksheetLine: Record "NPR Stock-Take Worksheet Line" temporary;
        WorksheetLine: Record "NPR Stock-Take Worksheet Line";
        RetailJournalHeader: Record "NPR Retail Journal Header";
        RetailJournalLine: Record "NPR Retail Journal Line";
        StockTakeWorksheetsPage: Page "NPR Stock-Take Worksheets";
        PageAction: Action;
    ...
    local procedure OnAfterTranslateBarcode(var StockTakeWorksheetLine: Record "NPR Stock-Take Worksheet Line")
    begin
        //-NPR5.55 [412140]
        //+NPR5.55 [412140]
    end;
