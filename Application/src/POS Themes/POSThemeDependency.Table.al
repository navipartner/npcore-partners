table 6150722 "NPR POS Theme Dependency"
{
    ...
    fields
    {
        ...
        field(13; "Target Code"; Code[20])
        {
            Caption = 'Target Code';
            DataClassification = CustomerContent;
            TableRelation = IF ("Target Type" = CONST(View)) "NPR POS View";
        ...
        field(31; "Dependency Code"; Code[10])
        {
            ...
            DataClassification = CustomerContent;
            TableRelation = IF ("Dependency Type" = FILTER(Logo | Background)) "NPR Web Client Dependency".Code WHERE(Type = CONST(DataUri))
            ELSE
            IF ("Dependency Type" = CONST(Stylesheet)) "NPR Web Client Dependency".Code WHERE(Type = CONST(CSS))
            ELSE
            IF ("Dependency Type" = CONST(JavaScript)) "NPR Web Client Dependency".Code WHERE(Type = CONST(JavaScript));
