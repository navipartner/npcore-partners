codeunit 6151366 "NPR CS UI SH To SO List"
{
    ...
    var
        ...
        Text013: Label 'Please scan a valid Sales Order No.';
        Text014: Label 'No Shipment lines found for Sales Order No. %1';
        Text015: Label 'No Shipment or Sales Document is found with doc. no. filter\\ %1.';
    ...
    begin
        ...
        if Barcode <> '' then begin
            if StrLen(Barcode) > MaxStrLen(SalesHeader."No.") then
                Error(Text011);
            SalesHeader.SetRange("Document Type", SalesHeader."Document Type"::Order);
            SalesHeader.SetRange("No.", Barcode);
            if SalesHeader.FindFirst then begin
                ...
                if not WarehouseShipmentLine.FindSet then
                    Error(Text014, SalesHeader."No.");

                TmpWhseShipmentHeader.Get(WarehouseShipmentLine."No.");
