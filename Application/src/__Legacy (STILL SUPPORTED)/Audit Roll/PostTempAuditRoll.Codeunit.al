codeunit 6014414 "NPR Post Temp Audit Roll"
{
    Permissions = TableData "NPR Audit Roll" = rimd;

    trigger OnRun()
    var
        RevRulle: Record "NPR Audit Roll";
    begin
    ...
    var
        Dummy: Record "NPR Audit Roll Posting" temporary;
        ...
        AltVarenummer: Record "NPR Alternative No.";
        Debitorpost: Record "Cust. Ledger Entry" temporary;
        RevisionUdbetaling: Record "NPR Audit Roll Posting" temporary;
        RetailSetup: Record "NPR Retail Setup";
        Debitorpost1: Record "Cust. Ledger Entry";
        Vare: Record Item;
        Kasse: Record "NPR Register";
        EkspBogfVarepost: Record "NPR Sale POS";
    ...
    procedure PostSales(var TempPost: Record "NPR Audit Roll Posting" temporary)
    var
    ...
    procedure PostRegisterMovements(var TempPost: Record "NPR Audit Roll Posting" temporary)
    var
        CurrentPost: Record "NPR Audit Roll Posting" temporary;
    begin
        RetailSetup.Get;
    ...
    procedure PostRegisterMovementsPrPost(var RevRulle: Record "NPR Audit Roll Posting" temporary)
    var
        Betalingsvalg: Record "NPR Payment Type POS";
    begin
    ...
    procedure MovementEntries(Kontonummer: Code[20]; Amount2: Decimal; Kassenr: Code[10]; Kontotype: Integer; "Global Dimension 1": Code[20]; ForceDesc: Text[50]; "Posting Date": Date; var TempPost: Record "NPR Audit Roll Posting" temporary)
    var
    ...
    procedure PostTodaysGLEntries(var TempPost: Record "NPR Audit Roll Posting" temporary)
    var
    ...
    procedure PostTodaysItemEntries(var TempPost: Record "NPR Audit Roll Posting" temporary)
    begin
        RetailSetup.Get;
        Clear(Counter);
    ...
    procedure PosterFremValDifferencer(var Rec: Record "NPR Audit Roll Posting" temporary)
    var
        DiffTekst: Label 'Cash register %1, Difference %2 %3';
        AuditRoll: Record "NPR Audit Roll";
        Period: Record "NPR Period";
        PeriodLine: Record "NPR Period Line";
    ...
    procedure PosterVarekladde(var RevRulle: Record "NPR Audit Roll Posting" temporary; Straks: Boolean; Bogfdate: Date): Boolean
    var
    ...
    begin
        ...
        with RevRulle do begin
            ...
            VarekldLinie."NPR Document Time" := "Closing Time";
            ...
            VarekldLinie."NPR Discount Type" := "Discount Type";
            VarekldLinie."NPR Discount Code" := CopyStr("Discount Code", 1, 10);
            VarekldLinie."NPR Vendor No." := Vendor;
            VarekldLinie."NPR Item Group No." := "Item Group";
            VarekldLinie."NPR Register Number" := "Register No.";
            ...
            VarekldLinie."NPR Group Sale" := Vare."NPR Group sale";
    ...
    procedure PosterDebitorIndbetaling(var RevRulle: Record "NPR Audit Roll Posting" temporary)
    var
        Kasse: Record "NPR Register";
    ...
    procedure PosterKasseAfslutning(var Rec: Record "NPR Audit Roll Posting" temporary)
    var
    ...
    procedure PostedInvoiceDocumentNo(var TempPost: Record "NPR Audit Roll Posting" temporary): Code[20]
    var
    ...
    procedure StraksBogfVarePostFraAfslEksp(var Ekspedition: Record "NPR Sale POS"; StraksBogf: Boolean; PostParam: Boolean)
    begin
    ...
    procedure RunPost(var Rec: Record "NPR Audit Roll Posting" temporary)
    var
        BetValgRec: Record "NPR Payment Type POS";
        POSUnit: Record "NPR POS Unit";
        XBonNr: Code[20];
        ChangeSum: Decimal;
        ChangeDep: Code[20];
            ...
            if not DoNotPost then begin
                ...
                if FindSet then
                    repeat
                        ...
                        Counter += 1;
                        if "No." = POSSetup.RoundingAccount(true) then begin
                    ...
                    until Next = 0;
                if ChangeSum <> 0 then
                    MovementEntries(POSSetup.RoundingAccount(true), ChangeSum, "Register No.", AccountType::"G/L", ChangeDep, '', BogfDate, Rec);
    ...
    procedure RunPostItemLedger(var Rec: Record "NPR Audit Roll Posting" temporary)
    var
        XBonNr: Code[20];
        t002: Label 'Posting of audit roll is not possible in offline mode';
        POSAuditRollIntegration: Codeunit "NPR POS-Audit Roll Integration";
    ...
    procedure RunTest(var Rec: Record "NPR Audit Roll Posting" temporary)
    var
    ...
    procedure RunTransfer(var TempPost: Record "NPR Audit Roll Posting" temporary; var Revisionsrulle: Record "NPR Audit Roll"): Integer
    begin
    ...
    procedure RunTransferItemLedger(var TempPost: Record "NPR Audit Roll Posting" temporary; var Revisionsrulle: Record "NPR Audit Roll"): Integer
    begin
        if WindowIsOpen then
    ...
    procedure RemoveSuspendedPayouts(var Rulle: Record "NPR Audit Roll Posting" temporary)
    var
        nCount: Integer;
        nTotal: Integer;
        Linie: Record "NPR Audit Roll Posting" temporary;
    begin
    ...
    procedure RunUpdateChanges(var TempPost: Record "NPR Audit Roll Posting" temporary)
    begin
        if WindowIsOpen then
    ...
    procedure PostDayClearing(var TempAuditRoll: Record "NPR Audit Roll Posting" temporary; var PaymentType: Record "NPR Payment Type POS"; Amount: Decimal)
    begin
    ...
    local procedure OnAfterRunPostItemLedger(var Rec: Record "NPR Audit Roll Posting")
    begin
    end;
}
