codeunit 6151200 "NPR NpCs Imp. Sales Doc." implements "NPR Nc Import List IProcess"
{
    ...
    begin
        ...
        if WebServiceAuthHelper.GetApiPassword(NpCsStore."API Password Key") <> NpXmlDomMgt.GetElementText(Element, 'from_store/service_password', 250, true) then
    ...
    begin
        ...
        NpCsDocument."Enable NP Email" := NpXmlDomMgt.GetElementBoolean(Element, 'notification/enable_np_email', false);
        NpCsDocument."NP E-mail Template (Pending)" := CopyStr(NpXmlDomMgt.GetElementCode(Element, 'notification/np_email_template_pending', MaxStrLen(NpCsDocument."NP E-mail Template (Pending)"), false), 1, MaxStrLen(NpCsDocument."NP E-mail Template (Pending)"));
        NpCsDocument."NP E-mail Template (Confirmed)" := CopyStr(NpXmlDomMgt.GetElementCode(Element, 'notification/np_email_template_confirmed', MaxStrLen(NpCsDocument."NP E-mail Template (Confirmed)"), false), 1, MaxStrLen(NpCsDocument."NP E-mail Template (Confirmed)"));
        NpCsDocument."NP E-mail Template (Rejected)" := CopyStr(NpXmlDomMgt.GetElementCode(Element, 'notification/np_email_template_rejected', MaxStrLen(NpCsDocument."NP E-mail Template (Rejected)"), false), 1, MaxStrLen(NpCsDocument."NP E-mail Template (Rejected)"));
        NpCsDocument."NP E-mail Template (Expired)" := CopyStr(NpXmlDomMgt.GetElementCode(Element, 'notification/np_email_template_expired', MaxStrLen(NpCsDocument."NP E-mail Template (Expired)"), false), 1, MaxStrLen(NpCsDocument."NP E-mail Template (Expired)"));
