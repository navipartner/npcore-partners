    ...
    begin
        ...
        if not POSSaleTaxLine.FindLine(POSSaleTax, TaxType, false) then begin
            ...
            if FeatureFlagsManagement.IsEnabled('fullyDiscountedPOSLineNetsToZero') then begin
                POSSaleTaxLine."Amount Excl. Tax" := POSSaleTaxCalc.CalcAmountAfterDiscount(POSSaleTaxLine."Unit Price Excl. Tax" * POSSaleTaxLine.Quantity, POSSaleTaxLine."Discount Amount" + POSSaleTaxLine."Invoice Disc. Amount", Currency."Amount Rounding Precision");
                POSSaleTaxLine."Line Amount" := POSSaleTaxCalc.CalcAmountAfterDiscount(POSSaleTaxLine."Unit Price Excl. Tax" * POSSaleTaxLine.Quantity, POSSaleTaxLine."Discount Amount", Currency."Amount Rounding Precision");
            end else begin
