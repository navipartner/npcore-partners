...
'let main=async({workflow:r,context:u,captions:a,parameters:n})=>{if(u.quantity=1,n.PromptQuantity&&(u.quantity=await popup.numpad(a.VoucherQuantity)),!u.quantity||u.quantity>100||u.quantity<1){await popup.error(a.InvalidQuantity);return}if(u.amount=await popup.numpad(a.VoucherAmount),!u.amount||u.amount<0){await popup.error(a.InvalidAmount);return}if(u.discountPct=0,n.PromptDiscountPct&&(u.discountPct=await popup.numpad(a.VoucherDiscount)),!u.discountPct||u.discountPct<0||u.discountPct>100){await popup.error(a.InvalidDiscount);return}for(i=1;i<u.quantity+1;i++){debugger;u.voucherNumber=i;let{workflowName:p,integrationRequest:t}=await r.respond("PrepareGiftCardLoad");if(!t.synchronousRequest){let{success:o}=await r.run(p,{context:{request:t,amount:u.amount}});if(!o)return}u.discountPct!==0&&(u.eftEntryNo=t.EntryNo,await r.respond("InsertDiscountLine"))}};'
        );
