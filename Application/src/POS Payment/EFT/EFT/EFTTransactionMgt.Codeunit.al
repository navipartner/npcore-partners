codeunit 6184473 "NPR EFT Transaction Mgt."
{
    ...
    procedure DispatchHwcEftDeviceResponse(HwcName: Text; HwcType: Text; HwcRequest: JsonObject; HwcResponse: JsonObject; Result: JsonObject)
    var
        ...
        JToken: JsonToken;
    begin
        EftInterface.OnHwcEftDeviceResponse(HwcName, HWcType, HwcRequest, HwcResponse, Result, Handled);
        if (not Handled) then
            EftInterface.OnHwcEftDeviceResponseNotHandled(HwcName, HWcType);

        HwcRequest.Get('EntryNo', JToken); // Required property in Hardware Connector request
    ...
    procedure EftPaymentHandleResponse(EftTransactionRequest: Record "NPR EFT Transaction Request"; POSSession: Codeunit "NPR POS Session")
    var
        EFTInterface: Codeunit "NPR EFT Interface";
    begin
