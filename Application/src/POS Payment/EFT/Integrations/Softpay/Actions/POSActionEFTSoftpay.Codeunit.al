...
'let main=async r=>{const{context:a,workflow:s,popup:n}=r;if(window.parent.jsBridge==null||navigator.userAgent.indexOf("Android")==-1)return await n.error("You can only use Softpay on Android devices.","Device error"),{success:!1};let e={SoftpayAction:a.request.SoftpayAction,Step:null,RequestID:null,Amount:a.request.Amount,Currency:a.request.Currency,IntegratorID:a.request.IntegratorID,IntegratorCredentials:a.request.IntegratorCredentials.split(""),SoftpayUsername:a.request.SoftpayUsername,SoftpayPassword:a.request.SoftpayPassword.split("")},t=null;try{switch(e.SoftpayAction){case"Refund":case"Payment":var i=await SendMPOSAsync(e);if(t=await s.respond("IDRecieved",{SoftpayResponse:i,SoftpayRequest:e}),t.success){e.RequestID=i.RequestID;let o=await SendMPOSAsync(e);t=await s.respond("TransactionFinished",{SoftpayResponse:o,SoftpayRequest:e})}break;case"GetTransaction":e.RequestID=a.request.RequestID;var p=await SendMPOSAsync(e);t=await s.respond("TransactionFinished",{SoftpayResponse:p,SoftpayRequest:e});break;default:t=await s.respond("Failed",{ErrorMessage:"Command: "+e.SoftpayAction+" Is not supported",SoftpayRequest:e})}}catch(o){t=await s.respond("Failed",{ErrorMessage:o.message,SoftpayRequest:e})}return t.success||await n.error(t.errorcaption,t.errortitle),{success:t.succes,endSale:t.endSale}},step=1;async function SendMPOSAsync(r){let a=window.parent.jsBridge;return r.Step=step++,JSON.parse(await a.SoftpayProtocol(JSON.stringify(r)))}'
        );
    end;
}
