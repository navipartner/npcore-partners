const main = async (obj) => {
  ...
  if (
    window.parent.jsBridge == null ||
    navigator.userAgent.indexOf("Android") == -1
  ) {
    await popup.error(
      "You can only use Softpay on Android devices.",
      "Device error"
    );
    return { success: false };
  }
  if (
    window.parent.jsBridge.SoftpayProtocol == null ||
    window.parent.jsBridge.SoftpayProtocol === undefined
  ) {
    await popup.error(
      "Softpay integration not found. Either you are using an outdated version of the Mobile App or the device is not supported.",
      "Device error"
    );
    return { success: false };
  }
  ...
  try {
    switch (request.SoftpayAction) {
      ...
      case "Payment":
        ...
        bc = await workflow.respond("IDRecieved", {
          SoftpayResponse: resultWithReqId,
          SoftpayRequest: request,
        });
        if (bc.success) {
          request.RequestID = resultWithReqId.RequestID;
          const paymentResponse = await SendMPOSAsync(request);
          bc = await workflow.respond("TransactionFinished", {
            SoftpayResponse: paymentResponse,
            SoftpayRequest: request,
          });
        }
        break;
      case "GetTransaction":
        ...
        bc = await workflow.respond("TransactionFinished", {
          SoftpayResponse: GetTransactionReq,
          SoftpayRequest: request,
        });
        break;
      default:
        bc = await workflow.respond("Failed", {
          ErrorMessage:
            "Command: " + request.SoftpayAction + " Is not supported",
          SoftpayRequest: request,
  ...
  if (!bc.success) await popup.error(bc.errorcaption, bc.errortitle);
...
async function SendMPOSAsync(request) {
  const mpos = window.parent.jsBridge;
  request.Step = step++;
