...
'const main=async({workflow:s,context:e,popup:o,captions:n})=>{if(!window.top.jsBridge.AdyenProtocol){o.error("The Tap to pay integration only works for Android devices.","Invalid Platform");return}e.EntryNo=e.request.EntryNo,e.PaymentSetupCode=e.request.PaymentSetupCode,e.IsLiveEnvironment=e.request.IsLiveEnvironment;try{if(await s.respond("PrepareRequest"),e.IsLookup){const i=await Lookup(e);return i.Success?(await s.respond("TerminalApiResponse",{FoundResponse:i.FoundCachedResponse,InstallationId:i.InstallationId,TerminalApiResult:JSON.stringify(i.TerminalApiResponse)}),{success:e.success,tryEndSale:e.success}):(await s.respond("Error",{Error:i.Error}),{success:!1,tryEndSale:!1})}let r=await IsBoarded(e);if(!r.Success)return await s.respond("Error",{Error:r.Error}),{success:!1,tryEndSale:!1};const a=r.IsBoardedResponse;if(e.InstallationId=a.InstallationId,!a.Boarded){e.BoardingRequestToken=a.BoardingRequestToken;const i=await Board(e);if(!i.Success)return await s.respond("Error",{Error:i.Error}),{success:!1,tryEndSale:!1}}if(r=await TerminalAPIRequest(e),!r.Success)return await s.respond("Error",{Error:r.Error}),{success:!1,tryEndSale:!1};await s.respond("TerminalApiResponse",{TerminalApiResult:JSON.stringify(r.TerminalApiResponse)})}catch(r){let a;r.ALError&&r.ALError.originalMessage?a=r.ALError.originalMessage:(r.message?a=r.message:a=r,o.error(a,"Error in Tap to Pay flow")),await s.respond("Error",{Error:a})}return{success:e.success,tryEndSale:e.success}};function IsBoarded(s){return new Promise(async(e,o)=>{const n=JSON.stringify({RequestType:"IsBoarded",IsLiveEnvironment:s.IsLiveEnvironment}),r=await window.top.jsBridge.AdyenProtocol(n);e(JSON.parse(r))})}function Board(s){return new Promise(async(e,o)=>{try{await workflow.respond("GetBoardingToken");const n=JSON.stringify({RequestType:"BoardWithToken",IsLiveEnvironment:s.IsLiveEnvironment,BoardingToken:s.boardingTokenBase64}),r=await window.top.jsBridge.AdyenProtocol(n);e(JSON.parse(r))}catch(n){o(n)}})}function Lookup(s){return new Promise(async(e,o)=>{const n=JSON.stringify({RequestType:"CachedLookup",CahcedLookupServiceId:s.LookupReference,EncDetailsJson:s.derivedKeyMaterial}),r=await window.top.jsBridge.AdyenProtocol(n);e(JSON.parse(r))})}function TerminalAPIRequest(s){return new Promise(async(e,o)=>{const n=JSON.stringify({RequestType:"TerminalApiRequest",IntegrationType:"TapToPay",TerminalApiSaletoPoiRequestJson:s.terminalApiReq,IsLiveEnvironment:s.IsLiveEnvironment,EncDetailsJson:s.derivedKeyMaterial}),r=await window.top.jsBridge.AdyenProtocol(n);e(JSON.parse(r))})}'
);
    end;
}
