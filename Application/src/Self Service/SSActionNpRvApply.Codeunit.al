codeunit 6151292 "NPR SS Action: NpRv Apply"
{
    ...
    var
        ...
        Handled: Boolean;
    begin
        Context.SetScopeParameters(true);
        VoucherTypeCode := UpperCase(Context.GetString('VoucherTypeCode', true));

        Context.SetScopeRoot(true);
        ReferenceNo := UpperCase(Context.GetString('VourchRefNo', true));
        if ReferenceNo = '' then
            exit;
