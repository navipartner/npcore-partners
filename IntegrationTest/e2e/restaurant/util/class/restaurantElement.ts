...
export class RestaurantElement extends RestaurantLayout {
  ...
  async add(locationName: string): Promise<void> {
    ...
    await this.page
      .frameLocator("iframe")
      .locator(
        `div.seating-setup-locations__location > span:has-text('${locationName}')`
      )
      .click();
    ...
    await this.page
      .frameLocator("iframe")
      .locator(
        "div.seating-setup-menu__buttons > div.seating-setup-menu-button"
      )
    ...
    switch (this.elementName) {
      case ElementType.Wall:
        await expect(
          this.page
            .frameLocator("iframe")
            .locator(`div.seating--${this.elementName.toLowerCase()}`)
        ).toBeVisible();
        break;
      default:
        await expect(
  ...
  async rename(...args: string[]): Promise<void> {
    if (args.length === 3) {
      ...
      await this.page
        .frameLocator("iframe")
        .locator(
          "div.seating-setup-context-menu > div.seating-setup-menu-button > span"
        )
      ...
      const byCaptionPromise = this.page
        .frameLocator("iframe")
        .locator(`div.seating-floor .${this.elementName.toLowerCase()}`)
...
export class RestaurantConfigElement extends RestaurantElement {
  async config(
    ...
    const byCaptionPromise1 = this.page
      .frameLocator("iframe")
      .locator(`div.restaurant__container .${this.elementName.toLowerCase()}`)
      .filter({ hasText: caption });
