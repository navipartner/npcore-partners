...
export const addItemToSale = async (page: Page, itemName: string) => {
  const frame = getFrame(page);
  await frame.getByRole('button', { name: 'Test Items' }).click();
  await frame.getByText('Open Popup Menu', { exact: true }).click();
  await frame.getByRole('button', { name: itemName }).click();
  await frame.getByText('Run ADD_ITEM_TO_SALE').click();
};
