...
export const restaurantSelectLayoutType = async (
  ...
  if (shouldAuthenticate) {
    if (isModern) {
      await page
        ...
        .fill(process.env.E2E_RESTAURANT_USERNAME ?? "");
      await page
        .getByLabel("Password:")
        .fill(process.env.E2E_RESTAURANT_PASSWORD ?? "");
    } else {
      await page
        .getByLabel("User name:")
        .fill(process.env.E2E_OLD_RESTAURANT_USERNAME ?? "");
      await page
        .getByLabel("Password:")
        .fill(process.env.E2E_OLD_RESTAURANT_PASSWORD ?? "");
    }
  ...
  const popupOldLocator = page
    .frameLocator('iframe[title="Main Content"]')
    .getByText(/Caution: Your program license expires in \d+ days\./);
  if (await popupOldLocator.isVisible().catch(() => false)) {
    await page
      .frameLocator('iframe[title="Main Content"]')
  ...
  const popupNewLocator = page
    .frameLocator('iframe[title="undefined"]')
    .getByText(/Caution: Your program license expires in \d+ days\./);
  if (await popupNewLocator.isVisible().catch(() => false)) {
    await page
      .frameLocator('iframe[title="undefined"]')
      .getByRole("button", { name: "OK" })
      .click();
  ...
  const restaurantViewLayoutDropdownHiddenExists =
    (await restaurantViewLayoutDropdownHidden.count()) > 0;
  ...
  const restaurantViewLayoutDropdown = page
    .frameLocator("iframe")
    .getByLabel("Restaurant View Layout");
  const restaurantViewLayoutDropdownExists =
    (await restaurantViewLayoutDropdown.count()) > 0;

  if (restaurantViewLayoutDropdownExists) {
    await restaurantViewLayoutDropdown.scrollIntoViewIfNeeded({
      timeout: 5000,
    });
    if (isModern) {
      await page
