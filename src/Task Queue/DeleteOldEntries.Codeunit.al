codeunit 6059911 "NPR Delete Old Entries"
{
    ...
    procedure WriteLogAndCommit(TaskLine: Record "NPR Task Line"; TableName: Text[60])
    begin
        TaskLine.AddMessageLine2OutputLog(StrSubstNo(Text003, NoOfEntries, TableName));
        Commit;
    end;

    procedure DeleteCreditCardLog(TaskLine: Record "NPR Task Line"; CompanyToDelete: Text[50]; LastDateToDelete: Date)
    var
        CreditCardTrans: Record "NPR EFT Receipt";
        RecRef: RecordRef;
    begin
    ...
    procedure DeleteTaskQueueLog(TaskLine: Record "NPR Task Line"; CompanyToDelete: Text[50])
    var
                ...
                if TaskLine2.FindSet then
                    repeat
                        ...
                        if TaskLine2."Delete Log After" <> 0 then begin
                            ...
                            NoOfRowsToDeleteAllowed := 0;
                            Counter := 0;
                            NoOfRowsToDeleteAllowed := GetNoOfRowsToDelete(CompanyToDelete, DATABASE::"NPR Task Log (Task)", NoOfEntries);
                            if (OriginalMaxSizeToDelete <> 0) then begin
                                if (NoOfRowsToDeleteAllowed <> 0) then begin
                                    NoOfEntries := NoOfRowsToDeleteAllowed;
                            ...
                            NoOfRowsToDeleteAllowed := 0;
                            Counter := 0;
                            NoOfRowsToDeleteAllowed := GetNoOfRowsToDelete(CompanyToDelete, DATABASE::"NPR Task Output Log", NoOfEntries);
                            if (OriginalMaxSizeToDelete <> 0) then begin
                                if (NoOfRowsToDeleteAllowed <> 0) then begin
                                    NoOfEntries := NoOfRowsToDeleteAllowed;
    ...
    procedure DeleteDataLog(TaskLine: Record "NPR Task Line"; CompanyToDelete: Text[50])
    var
    ...
    begin
        ...
        if DataLogSetup.FindSet then
            repeat
                ...
                NoOfRowsToDeleteAllowed := 0;
                Counter := 0;
                NoOfRowsToDeleteAllowed := GetNoOfRowsToDelete(CompanyToDelete, DATABASE::"NPR Data Log Field", NoOfEntries);
                if (OriginalMaxSizeToDelete <> 0) then begin
                    if (NoOfRowsToDeleteAllowed <> 0) then begin
                        NoOfEntries := NoOfRowsToDeleteAllowed;
                ...
                NoOfRowsToDeleteAllowed := 0;
                Counter := 0;
                NoOfRowsToDeleteAllowed := GetNoOfRowsToDelete(CompanyToDelete, DATABASE::"NPR Data Log Record", NoOfEntries);
                if (OriginalMaxSizeToDelete <> 0) then begin
                    if (NoOfRowsToDeleteAllowed <> 0) then begin
                        NoOfEntries := NoOfRowsToDeleteAllowed;
    ...
    procedure DeleteIDSPackages(TaskLine: Record "NPR Task Line"; CompanyToDelete: Text[50]; LastDateToDelete: Date)
    var
        RecRef: RecordRef;
        FRef: FieldRef;
        FRef2: FieldRef;
    ...
    procedure DeleteByRecRef(TaskLine: Record "NPR Task Line"; CompanyToDelete: Text[50]; LastDateToDelete: Date; TableNo: Integer; DateFilterFieldNo: Integer; ExecuteDeleteTrigger: Boolean)
    var
        RecRef: RecordRef;
        FRef: FieldRef;
    begin
        ...
        case UpperCase(Format(FRef.Type)) of
            'DATE':
                FRef.SetFilter('<%1', LastDateToDelete);
            'DATETIME':
                FRef.SetFilter('<%1', CreateDateTime(LastDateToDelete, 0T));
        end;
    ...
    procedure BackupAuditRoll(TaskLine: Record "NPR Task Line"; CompanyToBackup: Text[50]; LastDateToBackup: Date)
    var
        AuditRoll: Record "NPR Audit Roll";
        AuditRollBck: Record "NPR Audit Roll Backup";
        LastDate: Date;
    ...
    begin
        ...
        if AuditRoll.FindFirst then
            repeat
                ...
                //-NPR5.53 [326663]
                NoOfRowsToDeleteAllowed := GetNoOfRowsToDelete(CompanyToBackup, DATABASE::"NPR Audit Roll", NoOfEntries);
                if NoOfRowsToDeleteAllowed <> 0 then begin
                    //+NPR5.53 [326663]
                    if AuditRoll.FindSet(true, false) then
                        repeat
