codeunit 6150719 "NPR POS Action Management"
{
    ...
    procedure IsValidActionConfiguration(POSSession: Codeunit "NPR POS Session"; ActionObject: DotNet NPRNetAction; Source: Text; var ErrorText: Text; RaiseEvent: Boolean): Boolean
    var
        WorkflowAction: DotNet NPRNetWorkflowAction;
    begin
        case ActionObject.Type of
            WorkflowAction.WorkflowAction().Type:
                exit(CheckWorkflowActionConfiguration(ActionObject, Source, ErrorText, RaiseEvent, POSSession));
        end;

        exit(true);
    end;

    local procedure CheckWorkflowActionConfiguration(ActionObject: DotNet NPRNetWorkflowAction; Source: Text; var ErrorText: Text; RaiseEvent: Boolean; POSSession: Codeunit "NPR POS Session"): Boolean
    var
        POSAction: Record "NPR POS Action";
        Parameter: Record "NPR POS Action Parameter";
        Param: DotNet NPRNetKeyValuePair_Of_T_U;
        Value: Text;
        Severity: Integer;
    begin
        // Invalid situations
    ...
    local procedure ReportError(ActionObject: DotNet NPRNetAction; Source: Text; "Action": Text; ErrorText: Text; Severity: Integer; RaiseEvent: Boolean)
    begin
