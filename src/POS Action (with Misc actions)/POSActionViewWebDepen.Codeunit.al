codeunit 6150672 "NPR POSAction: View Web Depen."
{
    ...
    local procedure OnLookupCssCode(var POSParameterValue: Record "NPR POS Parameter Value"; Handled: Boolean)
    var
    ...
    begin
        ...
        if PAGE.RunModal(PAGE::"NPR Web Client Dependencies", WebClientDependency) = ACTION::LookupOK then
            POSParameterValue.Value := WebClientDependency.Code;
    end;

    [EventSubscriber(ObjectType::Table, 6150705, 'OnLookupValue', '', true, true)]
    local procedure OnLookupHtmlCode(var POSParameterValue: Record "NPR POS Parameter Value"; Handled: Boolean)
    var
        WebClientDependency: Record "NPR Web Client Dependency";
    begin
        if POSParameterValue."Action Code" <> ActionCode() then
    ...
    local procedure OnActionViewWebDependency(JSON: Codeunit "NPR POS JSON Management"; FrontEnd: Codeunit "NPR POS Front End Management")
    begin
    ...
    local procedure CreateUserInterface(JSON: Codeunit "NPR POS JSON Management")
    begin
        Model := Model.Model();
    ...
    local procedure InitCss(JSON: Codeunit "NPR POS JSON Management") Css: Text
    var
        WebClientDependency: Record "NPR Web Client Dependency";
    ...
    local procedure InitHtml(JSON: Codeunit "NPR POS JSON Management") Html: Text
    var
        WebClientDependency: Record "NPR Web Client Dependency";
        StreamReader: DotNet NPRNetStreamReader;
        InStr: InStream;
