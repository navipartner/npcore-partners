codeunit 6014441 "NPR Event Subscriber (Item)"
{
    ...
    begin
        ...
        with Rec do begin
            ...
            //-NPR5.27 [256157]
            if RetailSetup."Item Group on Creation" and ("No." = '') and ("NPR Item Group" = '') then begin
                if PAGE.RunModal(PAGE::"NPR Item Group Tree", ItemGroup) = ACTION::LookupOK then begin
                    "NPR Item Group" := ItemGroup."No.";
    ...
    begin
        ...
        with Rec do begin
            "Last Date Modified" := Today;
            "NPR Primary Key Length" := StrLen(Rec."No."); //this line shouldn't be needed as primary key length can't be changed in modify trigger
        end;
    end;
    ...
    begin
        //-NPR5.30 [265190]
        with Rec do
            if StrLen("NPR Label Barcode") > 0 then begin
                if BarcodeLibrary.TranslateBarcodeToItemVariant("NPR Label Barcode", ItemNo, VariantCode, ResolvingTable, false) then
                    if (ItemNo = "No.") and (ResolvingTable in [DATABASE::"NPR Alternative No.", DATABASE::"Item Cross Reference"]) then
                        exit;
                Error(Error_LabelBarcode, "NPR Label Barcode", AltNo.TableCaption, ItemCrossRef.TableCaption);
            end;
        //+NPR5.30 [265190]
    end;
    ...
    begin
        with Rec do begin
            ...
            if (CopyStr("No.", 1, 1) = '*') and ("No." <> '**') then begin
                ...
                if ("NPR Item Group" = '') and RetailSetup."Item Group on Creation" then
                ...
                if RetailSetup."Itemgroup Pre No. Serie" <> '' then begin
                    ...
                    if RetailSetup."EAN No. at 1 star" then
                        "No." := Utility.CreateEAN("NPR Item Group" + InternNo, '')
                    else
                        "No." := "NPR Item Group" + InternNo;
                end else begin
