codeunit 6014453 "NPR POS Sales Price Calc. Mgt."
{
    ...
    local procedure OnFindItemPrice(POSUnit: Record "NPR POS Unit"; SalePOS: Record "NPR Sale POS"; var SaleLinePOS: Record "NPR Sale Line POS"; var Handled: Boolean)
    begin
        //-NPR5.45 [323705]
        //+NPR5.45 [323705]
    end;
    ...
    local procedure FindBestRetailPrice(POSUnit: Record "NPR POS Unit"; SalePOS: Record "NPR Sale POS"; var SaleLinePOS: Record "NPR Sale Line POS"; var Handled: Boolean)
    begin
        //-NPR5.45 [323705]
        if POSUnit."Item Price Codeunit ID" <> CODEUNIT::"NPR POS Sales Price Calc. Mgt." then
            exit;
    ...
    begin
        ...
        with SaleLinePOS do begin
            ...
            if Item."NPR Group sale" or "Custom Price" then begin
            ...
            if Item.Get("No.") and Item."NPR Explode BOM auto" then
                "Unit Price" := 0
            else
    ...
    procedure SalesLineLineDiscExists(SalePOS: Record "NPR Sale POS"; var SalesLine: Record "NPR Sale Line POS"; ShowAll: Boolean): Boolean
    var
        Customer: Record Customer;
    begin
        if SalePOS."Customer Type" = SalePOS."Customer Type"::Ord then
