codeunit 6184501 "NPR CleanCash Comm."
{
    ...
    var
        CleanCashServerBridge: Codeunit "NPR CleanCash Server Bridge";
    ...
    var
        ...
        CleanCashAuditRoll: Record "NPR CleanCash Audit Roll";
        CleanCashAuditRoll2: Record "NPR CleanCash Audit Roll";
    ...
    local procedure SendReceiptToApplication(OrganisationNumber: Text[10]; PosId: Text[16]; DateTime: Text[12]; ReceiptNo: Text[30]; ReceiptType: DotNet NPRNetCommunicationReceipt; ReceiptTotal: Text[30]; NegativeTotal: Text[30]; Vat: array[4] of Text[30]; ConnectionString: Text[100]; MultiOrganizationIDPerPOS: Boolean; ShowErrorMessage: Boolean) IsSucces: Boolean
    var
        ...
        CCCommResult: DotNet NPRNetCommunicationResult;
    ...
    begin
        ...
        if RunLocal then begin
            // TODO: CTRLUPGRADE - old stargate v1 proxy
            ERROR('CTRLUPGRADE');
            /*
            CleanCashProxy.InitializeProtocol();
    ...
    var
        CleanCashErrorList: Record "NPR CleanCash Error List";
    begin
        ...
        CleanCashErrorList."Object No." := CODEUNIT::"NPR CleanCash Comm.";
    ...
    local procedure SetVatArray(var Vats: array[4] of Text[30]; CleanCashAuditRoll: Record "NPR CleanCash Audit Roll")
    begin
    ...
    local procedure IsCCSucces(CCCommResult: DotNet NPRNetCommunicationResult; EventResponse: Option NotSet,FailOpen,FailCheckStatusEX,FailRegisterPos,FailCheckStatus,FailStartReceipt," FailSendReceiptEX"," FailSendReceipt",ReceiptSend): Boolean
    begin
        //-NPR5.31
        //CleanCashProxy.GetCCCommunicationResult(CCCommResult);
        //EventResponse := CleanCashProxy.GetEventResponse;
    ...
    local procedure CheckEnumCommResult(EventResponse: Option NotSet,FailOpen,FailCheckStatusEX,FailRegisterPos,FailCheckStatus,FailStartReceipt,FailSendReceiptEX,FailSendReceipt,ReceiptSend; EnumCommResult: DotNet NPRNetCommunicationResult): Boolean
    var
        EnumType: Text[30];
    ...
    var
        EnumStatus: DotNet NPRNetCommunicationStatus;
        EnumType: Text[30];
        StatusList: Text[100];
    ...
    var
        CleanCashProxy: Codeunit "NPR CleanCash Proxy";
    begin
        //-NPR5.31
        if RunLocal then
    ...
    local procedure GetCCCommunicationStatus(var EnumStatus: DotNet NPRNetCommunicationStatus)
    var
        CleanCashProxy: Codeunit "NPR CleanCash Proxy";
    begin
        //-NPR5.31
