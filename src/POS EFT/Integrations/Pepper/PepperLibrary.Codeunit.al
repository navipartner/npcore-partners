// TODO: CTRLUPGRADE - quite some obsolete invocations in here - INVESTIGATE
codeunit 6184480 "NPR Pepper Library"
{
    ...
    begin
        ...
        case Choice of
            1:
                EftOpen('1', false);
            2:
                Error('not available now.');
            //-NPR5.35 [284379]
            //3 : EftPayment ('SALESTICKET1', 1753.67, 'EUR', '1234567890');
            3:
                EftPayment('SALESTICKET1', 1753.67, 0, 'EUR', '1234567890');
            //+NPR5.35 [284379]
            4:
                EftAuxFunction('SALESTICKET1', AuxFunctions::TICKETREPRINT);
            5:
                EftClose('SALESTICKET1', true);
        end;
    end;
    ...
    var
        ...
        // TODO: CTRLUPGRADE - references old Stargate v1 functionality
    ...
    procedure SetPepperConfiguration(ParPepperTerminal: Record "NPR Pepper Terminal"; ParPepperInstance: Record "NPR Pepper Instance"; ParPepperConfiguration: Record "NPR Pepper Config."; ParPepperVersion: Record "NPR Pepper Version")
    begin
    ...
    var
        ResultCode: Integer;
        PepperClientInstall: Codeunit "NPR Pepper File Mgmt. Func.";
    ...
    begin
        ...
        if PepperVersion."Codeunit Install" = 0 then
            PepperVersion."Codeunit Install" := CODEUNIT::"NPR Pepper File Mgmt. Func.";

        // TODO: CTRLUPGRADE - obsolete code
        ERROR('CTRLUPGRADE');
        /*
    ...
    var
        ...
        // TODO: CTRLUPGRADE - obsolete protocol codeunit, INVESTIGATE!! This one is not commented out, because it may contain some legit code, but most likely this is just old Stargate v1 stuff
        PepperBeginWorkshift: Codeunit "NPR Pepper Begin Workshift";
        ResultCode: Integer;
        OptionInt: Integer;
        Recovered: Boolean;
        MaxAttempts: Integer;
    ...
    begin
        ...
        // TODO: CTRLUPGRADE - obsolete invocation
        ERROR('CTRLUPGRADE');
        /*
        ...
        */
        ...
        if PepperVersion."Codeunit Begin Workshift" = 0 then
            PepperVersion."Codeunit Begin Workshift" := CODEUNIT::"NPR Pepper Begin Workshift";
    ...
    var
        PepperEndWorkshift: Codeunit "NPR Pepper End Workshift";
        ResultCode: Integer;
    begin
        TransactionStarted := CurrentDateTime;
        TestConfig(2);
        ...
        if PepperVersion."Codeunit End Workshift" = 0 then
            PepperVersion."Codeunit End Workshift" := CODEUNIT::"NPR Pepper End Workshift";

        // TODO: CTRLUPGRADE - obsolete
        ERROR('CTRLUPGRADE');
        /*
    ...
    var
        PepperTrxTransaction: Codeunit "NPR Pepper Trx Transaction";
    begin
        //-NPR5.35 [284379]
    ...
    begin
        ...
        if PepperVersion."Codeunit Transaction" = 0 then
            PepperVersion."Codeunit Transaction" := CODEUNIT::"NPR Pepper Trx Transaction";
    ...
    begin
        ...
        case TinaFunction of
            1:
                AuxFunction := AuxFunction::TINAACTIVATION;
            2:
                AuxFunction := AuxFunction::TINADEACTIVATION;
            3:
                AuxFunction := AuxFunction::TINAQUERY;
        end;
    ...
    var
        PepperAuxFunctions: Codeunit "NPR Pepper Aux Functions";
        ResultCode: Integer;
    ...
    var
        EFTTransactionRequestToReverse: Record "NPR EFT Transaction Request";
    begin
    ...
    procedure GetEFTTransactionRequest(var VarEFTTransactionRequest: Record "NPR EFT Transaction Request")
    begin
    ...
    procedure GetEFTRecoveryTransactionRequest(var VarEFTTransactionRequest: Record "NPR EFT Transaction Request")
    begin
    ...
    var
        ...
        EFTTransactionComment: Record "NPR EFT Transact. Req. Comment";
        TransactionType: Record "NPR Pepper EFT Trx Type";
        EntryNo: Integer;
        I: Integer;
        RcptText: Text;
        OStream: OutStream;
    ...
    var
        LocPepperTransactionRequest: Record "NPR EFT Transaction Request";
    begin
    ...
    var
        EFTIntegrationType: Record "NPR EFT Integration Type";
    begin
        //-NPR5.30 [263458]
