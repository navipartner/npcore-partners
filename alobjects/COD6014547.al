codeunit 6014547 "RP Matrix Print Mgt."
{
    // Matrix Print Mgt.
    ...
    begin
        case DecimalRounding of
          DecimalRounding::"2" : UpdateField(X,Y,0,Align,0,0,'',Format(Decimal,0,'<Precision,2:2><Standard Format,2>'));
          DecimalRounding::"3" : UpdateField(X,Y,0,Align,0,0,'',Format(Decimal,0,'<Precision,3:3><Standard Format,2>'));
          DecimalRounding::"4" : UpdateField(X,Y,0,Align,0,0,'',Format(Decimal,0,'<Precision,4:4><Standard Format,2>'));
          DecimalRounding::"5" : UpdateField(X,Y,0,Align,0,0,'',Format(Decimal,0,'<Precision,5:5><Standard Format,2>'));
        end;
    end;
    ...
    begin
        ...
        if TemplateHeader."Print Processing Object ID" = 0 then
          ...
          case TemplateHeader."Print Processing Object Type" of
            TemplateHeader."Print Processing Object Type"::Codeunit : CODEUNIT.Run(TemplateHeader."Print Processing Object ID", Variant);
            TemplateHeader."Print Processing Object Type"::Report : REPORT.Run(TemplateHeader."Print Processing Object ID", GuiAllowed, false, Variant);
          end;
    ...
    local procedure ProcessLayout(DataJoinBuffer: Codeunit "RP Data Join Buffer Mgt.";DataItems: Record "RP Data Items";TemplateHeader: Record "RP Template Header")
    var
    ...
    local procedure EvaluateFields(var TemplateLine: Record "RP Template Line";var DataJoinBuffer: Codeunit "RP Data Join Buffer Mgt."): Text[30]
    var
        Field1Val: Text[250];
        Field2Val: Text[250];
        Field1Dec: Decimal;
    ...
    begin
        ...
        end else if TemplateLine."Data Item Record No." > 0 then begin
    ...
    local procedure MergeField(var TemplateLine: Record "RP Template Line";var DataJoinBuffer: Codeunit "RP Data Join Buffer Mgt.")
    var
        Font: Text[30];
        RecID: RecordID;
        Handled: Boolean;
    ...
    begin
        ...
        case true of
          TemplateLine.Field > 0 : TemplateLine."Processing Value" := GetFieldValue(TemplateLine, DataJoinBuffer);
          TemplateLine.Attribute <> '' : TemplateLine."Processing Value" := GetAttributeValue(TemplateLine, DataJoinBuffer);
        end;
    ...
    local procedure GetFieldValue(TemplateLine: Record "RP Template Line";DataJoinBuffer: Codeunit "RP Data Join Buffer Mgt."): Text
    begin
        //-NPR5.46 [314067]
        if TemplateLine."Field 2" = 0 then
          ...
          else if TemplateLine."Data Item Record No." > 0 then
    ...
    local procedure GetRecID(TemplateLine: Record "RP Template Line";DataJoinBuffer: Codeunit "RP Data Join Buffer Mgt.";var RecordID: RecordID)
    begin
        //-NPR5.46 [314067]
        if TemplateLine."Root Record No." > 0 then
    ...
    local procedure GetAttributeValue(TemplateLine: Record "RP Template Line";DataJoinBuffer: Codeunit "RP Data Join Buffer Mgt."): Text
    var
    ...
    [IntegrationEvent(false, false)]
    local procedure OnSendPrintJob(TemplateCode: Text;CodeunitId: Integer;ReportId: Integer;var Printer: Codeunit "RP Matrix Printer Interface";NoOfPrints: Integer)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforePrintMatrix(var RecRef: RecordRef;TemplateHeader: Record "RP Template Header";var Skip: Boolean)
    begin
    end;
