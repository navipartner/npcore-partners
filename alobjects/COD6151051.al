codeunit 6151051 "Distribution Mgmt"
{
    ...
    procedure CreateDistributionItem(DistributionId: Integer;ItemHierarchy: Record "Item Hierarchy";DistributionGroups: Record "Distribution Group")
    var
        ReplenishmentDemandLine: Record "Retai Repl. Demand Line";
    ...
    begin
        ...
        case DistributionHeaders."Distribution Type" of
          DistributionHeaders."Distribution Type"::Blocked: begin
            Error(BlockedForDistribution,DistributionHeaders."Distribution Group");
          end;
          DistributionHeaders."Distribution Type"::Manual: begin
          ...
          DistributionHeaders."Distribution Type"::Automatic: begin
            Message('No support for automatic distribution now!');
          end;
        end;
    end;

    procedure CreateDistributionDocuments(var DistributionHeaders: Record "Distribution Headers")
    var
        Item: Record Item;
        DistributionLines: Record "Distribution Lines";
    ...
    begin
        ...
        if DistributionLines.FindSet then begin
          repeat
            ...
            PurchaseLine.SetRange("Retail Replenisment No.",0);
            ...
            if PurchaseLine.FindSet(true) then begin
              repeat
                if QtyToDist > 0 then begin
                  ...
                  PurchaseLine."Retail Replenisment No." := DistributionLines."Distribution Id";
                  PurchaseLine.Modify;
                end;
    ...
    procedure CreateTransfOrders(var DistributionLines: Record "Distribution Lines";TransferQuantity: Decimal)
    var
        ReplenSetup: Record "Retail Replenishment Setup";
    ...
    begin
        ...
        DistributionGroups.TestField("Warehouse Location");
            ...
            TransferLine."Retail Replenisment No." := DistributionLines."Distribution Id";
