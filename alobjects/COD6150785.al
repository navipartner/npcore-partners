codeunit 6150785 "PoC: CUST_DEBITSALE"
{
    ...
    begin
        ...
        case WorkflowStep of
            'CUST_SELECT':
                AfterStep_SelectCustomer(Context, POSSession, FrontEnd);
            'SET_REFERENCE':
                AfterStep_SetReference(Context, POSSession, FrontEnd);
            'SET_EXTDOCNO':
                AfterStep_SetExtDocNo(Context, POSSession, FrontEnd);
            'COMPLETE':
                AfterStep_Complete(Context, POSSession, FrontEnd);
            else
    ...
    local procedure AfterStep_SelectCustomer(Context: JsonObject; POSSession: Codeunit "POS Session"; FrontEnd: Codeunit "POS Front End Management")
    var
        JSON: Codeunit "POS JSON Management";
    begin
    end;

    local procedure AfterStep_SetReference(Context: JsonObject; POSSession: Codeunit "POS Session"; FrontEnd: Codeunit "POS Front End Management")
    begin
    end;

    local procedure AfterStep_SetExtDocNo(Context: JsonObject; POSSession: Codeunit "POS Session"; FrontEnd: Codeunit "POS Front End Management")
    begin
    end;

    local procedure AfterStep_Complete(Context: JsonObject; POSSession: Codeunit "POS Session"; FrontEnd: Codeunit "POS Front End Management")
    var
        JSON: Codeunit "POS JSON Management";
