codeunit 6150822 "POS Action - Conv Touch2Trans"
{
    ...
    begin
        ...
        if GridPosition <> GridPosition::DontCare then
            TouchScreenMenuLines.SetRange("Grid Position", GridPosition);
        if TouchScreenMenuLines.FindSet then
            repeat
    ...
    begin
            ...
            TSMenuLines."Line Type"::Item:
                begin
                    if TSMenuLines.Type in [TSMenuLines.Type::"Payment Form", TSMenuLines.Type::"Payment Functions", TSMenuLines.Type::PaymentType] then begin
                        ...
                        if TSMenuLines."Filter No." <> '' then begin
                            POSMenuButton.Validate("Action Type", POSMenuButton."Action Type"::Action);
                            POSMenuButton.Validate("Action Code", 'ITEM');
                            POSMenuButton.Modify(true);
                            SetActionParameter(POSMenuButton, 'itemNo', TSMenuLines."Filter No.", false);
        ...
        case TSMenuLines."Button Styling" of
            TSMenuLines."Button Styling"::Green:
                POSMenuButton."Background Color" := 'green';
            TSMenuLines."Button Styling"::Red:
                POSMenuButton."Background Color" := 'red';
            TSMenuLines."Button Styling"::"Dark Red":
                POSMenuButton."Background Color" := 'dark-red';
            TSMenuLines."Button Styling"::Grey:
                POSMenuButton."Background Color" := 'gray';
            TSMenuLines."Button Styling"::Purple:
                POSMenuButton."Background Color" := 'purple';
            TSMenuLines."Button Styling"::Indigo:
                POSMenuButton."Background Color" := 'indigo';
            TSMenuLines."Button Styling"::Yellow:
                POSMenuButton."Background Color" := 'yellow';
            TSMenuLines."Button Styling"::Orange:
                POSMenuButton."Background Color" := 'orange';
            TSMenuLines."Button Styling"::White:
                POSMenuButton."Background Color" := 'white';
        end;
    ...
    begin
        case TSMenuLines."Filter No." of
            //Popup Menus
            'FUNCTIONS_DISCOUNT':
                SetPopupMenu(POSMenuButton, 'DISCOUNT');
            'PRINTS':
                SetPopupMenu(POSMenuButton, 'PRINTS');
            'CUSTOMER':
                SetPopupMenu(POSMenuButton, 'CUSTOMER');
            'FUNCTIONS_ITEM':
                SetPopupMenu(POSMenuButton, 'ITEMS'); //ITEMFUNCTIONS

            'SALE-TOP':
                SetPopupMenu(POSMenuButton, 'SALE-TOP');
            'FUNCTIONS_SALE':
                SetPopupMenu(POSMenuButton, 'MORE_SALE_FUNC');
            'RETURN':
                SetPopupMenu(POSMenuButton, 'RETURN');
            'STATISTICS':
                SetPopupMenu(POSMenuButton, 'STATISTICS');
            'DISCOUNT SCHEMES':
                SetPopupMenu(POSMenuButton, 'DISCOUNT_SCHEMES');
            'SALE-BOTTOM':
                SetPopupMenu(POSMenuButton, 'SALE-BOTTOM');

                //Payment Types
            'TERMINAL_PAY':
                SetPaymentType(POSMenuButton, 'T');
                //-NPR5.38 [297943]
            ...
            //Simple Actions
            'DELETE_LINE':
                SetSimpleLineAction(POSMenuButton, 'DELETE_POS_LINE');
            'CANCEL_SALE':
                SetSimpleLineAction(POSMenuButton, 'CANCEL_POS_SALE');
            'Zoom Sales Line':
                SetSimpleLineAction(POSMenuButton, 'ZOOM');
            'COMMENT_INSERT':
                SetSimpleLineAction(POSMenuButton, 'INSERT_COMMENT');
            'REGISTER_CHANGE':
                SetSimpleLineAction(POSMenuButton, 'SWITCH_REGISTER');
            'BALANCE_REGISTER':
                SetSimpleLineAction(POSMenuButton, 'BALANCE_V1');
            'SALE_REVERSE':
                SetSimpleLineAction(POSMenuButton, 'REVERSE_SALE');
            'CREDITVOUCHER_CREATE':
                SetSimpleLineAction(POSMenuButton, 'CREDIT_GIFTVOUCHER');
            'REGISTER_OPEN':
                SetSimpleLineAction(POSMenuButton, 'OPEN_CASH_DRAWER');


                //Complex Actions
            'LINE_AMOUNT':
                SetComplexLineAction(POSMenuButton, 'DISCOUNT', 'DiscountType', 'LineAmount', false);
            'LINE_DISCOUNT_AMOUNT':
                SetComplexLineAction(POSMenuButton, 'DISCOUNT', 'DiscountType', 'LineDiscountAmount', false);
            'LINE_DISCOUNTPCT_ABS':
                SetComplexLineAction(POSMenuButton, 'DISCOUNT', 'DiscountType', 'LineDiscountPercentABS', false);
            'LINE_DISCOUNTPCT_REL':
                SetComplexLineAction(POSMenuButton, 'DISCOUNT', 'DiscountType', 'LineDiscountPercentREL', false);
            'LINE_UNITPRICE':
                SetComplexLineAction(POSMenuButton, 'DISCOUNT', 'DiscountType', 'LineUnitPrice', false);
            'TOTAL_AMOUNT':
                SetComplexLineAction(POSMenuButton, 'DISCOUNT', 'DiscountType', 'TotalAmount', false);
            'TOTAL_DISCOUNT':
                SetComplexLineAction(POSMenuButton, 'DISCOUNT', 'DiscountType', 'TotalDiscountAmount', false);
            'TOTAL_DISCOUNTPCT_ABS':
                SetComplexLineAction(POSMenuButton, 'DISCOUNT', 'DiscountType', 'DiscountPercentABS', false);
            'TOTAL_DISCOUNTPCT_REL':
                SetComplexLineAction(POSMenuButton, 'DISCOUNT', 'DiscountType', 'DiscountPercentREL', false);
            'NPORDER_GET':
                SetComplexLineAction(POSMenuButton, 'CUSTOMERINFO', 'CustomerType', 'NPOrderGet', false);
            'NPORDER_SEND':
                SetComplexLineAction(POSMenuButton, 'CUSTOMERINFO', 'CustomerType', 'NPOrderSend', false);
            'OUT_PAYMENT':
                SetComplexLineAction(POSMenuButton, 'PAYIN_PAYOUT', 'Pay Option', 'Payout', false);
            'SEND_TO_ORDER / SALESORDER':
                SetComplexLineAction(POSMenuButton, 'CREATE_SALESORDER', 'InitNewSaleOnDone', 'SAND', false);
            'CUSTOMER REPAIR':
                SetComplexLineAction(POSMenuButton, 'CUSTOMERINFO', 'CustomerType', 'RepairSend', false);
            'REGISTER_LOCK':
                SetComplexLineAction(POSMenuButton, 'CHANGE_VIEW', 'ViewType', 'Locked', false);
            'SALE_GIFTVOUCHER':
                SetComplexLineAction(POSMenuButton, 'SALE_GIFTVOUCHER', 'DiscountType', 'Amount', false);
            'TURNOVER_REPORT':
                SetComplexLineAction(POSMenuButton, 'SALES_STATISTICS', 'Statistic Type', 'Statistics', false);
            'TURNOVER_SALE':
                SetComplexLineAction(POSMenuButton, 'SALES_STATISTICS', 'Statistic Type', 'Sale', false);
            'TURNOVER_STATS':
                SetComplexLineAction(POSMenuButton, 'SALES_STATISTICS', 'Statistic Type', 'Report', false);
            'GOTO_PAYMENT':
                SetComplexLineAction(POSMenuButton, 'CHANGE_VIEW', 'ViewType', 'Payment', false);
                //'CUSTOMER_CRM' :              SetComplexLineAction(POSMenuButton, '','Type','CustomerCRM', false);
            'CUSTOMER_ILE':
                SetComplexLineAction(POSMenuButton, 'CUSTOMERINFO', 'Type', 'CustomerILE', false);
            'CUSTOMER_INFO':
                SetComplexLineAction(POSMenuButton, 'CUSTOMERINFO', 'Type', 'CustomerInfo', false);
            'CUSTOMER_PAY':
                SetComplexLineAction(POSMenuButton, 'RECEIVABLES', 'Type', 'InvoiceCustomer', false);
            'INSERT_PAYMENT':
                SetComplexLineAction(POSMenuButton, 'RECEIVABLES', 'Type', 'ApplyPaymentToInvoices', false);
            'INSERT_PAYMENT_CASH':
                SetComplexLineAction(POSMenuButton, 'RECEIVABLES', 'Type', 'DepositAmount', false);
            'DEBIT_INFO':
                SetComplexLineAction(POSMenuButton, 'CUSTOMERINFO', 'Type', 'DebitInfo', false);
            'PRINT_EXCHLABEL_ALL':
                SetComplexLineAction(POSMenuButton, 'PRINT_EXCH_LABEL', 'Setting', 'All Lines', false);
            'PRINT_EXCHLABEL_LINE_ALL':
                SetComplexLineAction(POSMenuButton, 'PRINT_EXCH_LABEL', 'Setting', 'Line Quantity', false);
            'PRINT_EXCHLABEL_LINE_ONE':
                SetComplexLineAction(POSMenuButton, 'PRINT_EXCH_LABEL', 'Setting', 'Single', false);
            'PRINT_EXCHLABEL_PACKAGE':
                SetComplexLineAction(POSMenuButton, 'PRINT_EXCH_LABEL', 'Setting', 'Package', false);
            'PRINT_ITEM_LABEL':
                SetComplexLineAction(POSMenuButton, 'PRINT_ITEM', 'PrintType', 'Price', false);
            'PRINT_LAST_RECEIPT':
                SetComplexLineAction(POSMenuButton, 'PRINT_RECEIPT', 'Setting', 'Last Receipt', false);
            'PRINT_LAST_RECEIPT_A4':
                SetComplexLineAction(POSMenuButton, 'PRINT_RECEIPT', 'Setting', 'Last Receipt Large', false);
            'TERMINAL_OPENSHIFT':
                SetComplexLineAction(POSMenuButton, 'XXX_TERMINAL', 'auxCommand', 'Ticket Reprint', false);
            'TERMINAL_ENDOFDAY':
                SetComplexLineAction(POSMenuButton, 'XXX_TERMINAL', 'auxCommand', 'Ticket Reprint', false);
            'Reprint':
                SetComplexLineAction(POSMenuButton, 'XXX_TERMINAL', 'auxCommand', 'Ticket Reprint', false);
            'TERMINAL_AUX':
                SetComplexLineAction(POSMenuButton, 'XXX_TERMINAL', 'auxCommand', 'StrMenu', false);
            'TERMINAL_INSTALL':
                SetComplexLineAction(POSMenuButton, 'XXX_TERMINAL', 'auxCommand', 'Ticket Reprint', false);
            'TERMINAL_CANCEL':
                SetComplexLineAction(POSMenuButton, 'XXX_TERMINAL', 'auxCommand', 'StrMenu', false);
            'TERMINAL_OFFLINE':
                SetComplexLineAction(POSMenuButton, 'XXX_TERMINAL', 'auxCommand', 'StrMenu', false);
            'TAX_FREE':
                SetComplexLineAction(POSMenuButton, 'TAX_FREE', 'Setting', 'Toggle', false);
            'Audit Roll View':
                SetComplexLineAction(POSMenuButton, '', 'PageId', '6014432', false);
            'GOTO_SALE':
                SetComplexLineAction(POSMenuButton, 'CHANGE_VIEW', 'ViewType', 'Sale', false);
            'QUANTITY_POS':
                SetComplexLineAction(POSMenuButton, 'Quantity', 'Constraint', 'Positive Quantity Only', false);
            'QUANTITY_NEG':
                SetComplexLineAction(POSMenuButton, 'Quantity', 'Constraint', 'No Constraint', false); //JDH
            'GET_SAVED_SALE':
                SetComplexLineAction(POSMenuButton, 'PARK_SALE', 'Function', 'Retrieve Parked Sale', false);
            'GET_SALE':
                SetComplexLineAction(POSMenuButton, 'PARK_SALE', 'Function', 'Retrieve Parked Sale', false);
            'SALE_SAVE':
                SetComplexLineAction(POSMenuButton, 'PARK_SALE', 'Function', 'Park Sale', false);
            'LOOKUP':
                SetComplexLineAction(POSMenuButton, 'LOOKUP', 'LookupType', 'Item', false);
            'CUSTOMER_STD':
                SetComplexLineAction(POSMenuButton, 'CUSTOMERINFO', 'CustomerType', 'CustomerSTD', false);
            'TM_SCAN_TICKET':
                begin
                    ...
                    case FirstParameter of
                        'ADMITTED_COUNT':
                            SetComplexLineAction(POSMenuButton, 'TM_TICKETMGMT', 'Function', 'Admission Count', false);
                        'ARRIVAL':
                            SetComplexLineAction(POSMenuButton, 'TM_TICKETMGMT', 'Function', 'Register Arrival', true);
                    end;
                end;
            'MM_SCAN_CARD':
                begin
                    case TSMenuLines.Parametre of
                        'SET_MEMBERNUMBER':
                            SetComplexLineAction(POSMenuButton, 'MM_MEMBERMGT', 'Function', 'Select Membership', false);
                        'MEMBER_ARRIVAL':
                            SetSimpleLineAction(POSMenuButton, 'MM_MEMBER_ARRIVAL');
                            //'EDIT_MEMBERINFO' :
                        'RENEW_MEMBERSHIP':
                            SetComplexLineAction(POSMenuButton, 'MM_MEMBERMGT', 'Function', 'Renew Membership', false);
                        'UPGRADE_MEMBERSHIP':
                            SetComplexLineAction(POSMenuButton, 'MM_MEMBERMGT', 'Function', 'Upgrade Membership', false);
                    end;
                end;
