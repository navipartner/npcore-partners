codeunit 6060118 "TM Admission Sch. Management"
{
    ...
    var
        ...
        TmpAdmissionScheduleEntry: Record "TM Admission Schedule Entry" temporary;
    ...
    local procedure GenerateScheduleEntry(AdmissionScheduleLines: Record "TM Admission Schedule Lines";Regenerate: Boolean;GenerateForDate: Date;var TmpAdmissionScheduleEntry: Record "TM Admission Schedule Entry" temporary)
    var
        Admission: Record "TM Admission";
    ...
    begin
        ...
        if (CreateEntryThisPeriod) then begin
          ...
          case (DateRecord."Period No.") of
            1 : if (Schedule.Monday) then AddTimeEntry (GenerateForDate, Admission, Schedule, TmpAdmissionScheduleEntry);
            2 : if (Schedule.Tuesday) then AddTimeEntry (GenerateForDate, Admission, Schedule, TmpAdmissionScheduleEntry);
            3 : if (Schedule.Wednesday) then AddTimeEntry (GenerateForDate, Admission, Schedule, TmpAdmissionScheduleEntry);
            4 : if (Schedule.Thursday) then AddTimeEntry (GenerateForDate, Admission, Schedule, TmpAdmissionScheduleEntry);
            5 : if (Schedule.Friday) then AddTimeEntry (GenerateForDate, Admission, Schedule, TmpAdmissionScheduleEntry);
            6 : if (Schedule.Saturday) then AddTimeEntry (GenerateForDate, Admission, Schedule, TmpAdmissionScheduleEntry);
            7 : if (Schedule.Sunday) then AddTimeEntry (GenerateForDate, Admission, Schedule, TmpAdmissionScheduleEntry);
          end;
        end;
    ...
    local procedure CalculateScheduleDateRange(AdmissionScheduleLines: Record "TM Admission Schedule Lines";ReferenceDate: Date;MaxEndDate: Date;IgnoreGenerationDate: Boolean;var GenerateFromDate: Date;var GenerateUntilDate: Date) RefDateIsInRange: Boolean
    var
        Admission: Record "TM Admission";
        Schedule: Record "TM Admission Schedule";
        DateRecord: Record Date;
    ...
    local procedure AddTimeEntry(StartFromDate: Date;Admission: Record "TM Admission";Schedule: Record "TM Admission Schedule";var TmpAdmissionScheduleEntry: Record "TM Admission Schedule Entry" temporary)
    var
        ExistingAdmissionScheduleEntry: Record "TM Admission Schedule Entry";
        NewAdmissionScheduleEntry: Record "TM Admission Schedule Entry";
        AdmissionScheduleLines: Record "TM Admission Schedule Lines";
        DuplicateAdmissionScheduleEntry: Record "TM Admission Schedule Entry";
    ...
    begin
        ...
        with TmpAdmissionScheduleEntry do begin
          ...
          if ((Admission."Admission Base Calendar Code" <> '') and ("Admission Is" = "Admission Is"::OPEN)) then begin
            NonWorking := CalendarManagement.CheckCustomizedDateStatus (CustomizedCalendarChange."Source Type"::Location, "Admission Code", '', Admission."Admission Base Calendar Code", StartFromDate, CalendarDesc);
            if (NonWorking) then
          ...
          if ((Schedule."Admission Base Calendar Code" <> '') and ("Admission Is" = "Admission Is"::OPEN)) then begin
            NonWorking := CalendarManagement.CheckCustomizedDateStatus (CustomizedCalendarChange."Source Type"::Location, '', "Schedule Code", Schedule."Admission Base Calendar Code", StartFromDate, CalendarDesc);
            if (NonWorking) then
              "Admission Is" := Schedule."Admission Is"::CLOSED;
          end;

          if ((AdmissionScheduleLines."Admission Base Calendar Code" <> '') and ("Admission Is" = "Admission Is"::OPEN)) then begin
            NonWorking := CalendarManagement.CheckCustomizedDateStatus (CustomizedCalendarChange."Source Type"::Location, "Admission Code", "Schedule Code", AdmissionScheduleLines."Admission Base Calendar Code", StartFromDate, CalendarDesc);
            if (NonWorking) then
              "Admission Is" := Schedule."Admission Is"::CLOSED;
          end;
          //+TM1.28 [303925]
    ...
    local procedure CompareScheduleEntries(AdmissionCode: Code[20];ReferenceDate: Date;var TmpAdmissionScheduleEntry: Record "TM Admission Schedule Entry" temporary): Boolean
    var
        AdmissionScheduleEntry: Record "TM Admission Schedule Entry";
        PreviousAdmissionScheduleEntry: Record "TM Admission Schedule Entry";
        UniqKey: Text;
    begin
    ...
    local procedure StoreScheduleEntries(AdmissionCode: Code[20];ReferenceDate: Date;var TmpAdmissionScheduleEntry: Record "TM Admission Schedule Entry" temporary)
    var
        AdmissionScheduleEntry: Record "TM Admission Schedule Entry";
        PreviousAdmissionScheduleEntry: Record "TM Admission Schedule Entry";
        UniqKey: Text;
    ...
    local procedure GetIdentifyingString(AdmissionScheduleEntry: Record "TM Admission Schedule Entry"): Text
    begin
    ...
    local procedure CheckExists(TmpAdmissionScheduleEntry: Record "TM Admission Schedule Entry" temporary): Boolean
    var
        AdmissionScheduleEntry: Record "TM Admission Schedule Entry";
    begin
    ...
    local procedure CancelExisting(TmpAdmissionScheduleEntry: Record "TM Admission Schedule Entry" temporary) ExternalEntryNo: Integer
    var
        AdmissionScheduleEntry: Record "TM Admission Schedule Entry";
    begin
    ...
    local procedure IsIdentical(AdmissionScheduleEntry1: Record "TM Admission Schedule Entry";AdmissionScheduleEntry2: Record "TM Admission Schedule Entry"): Boolean
    begin
    ...
    var
        AdmissionScheduleEntry: Record "TM Admission Schedule Entry";
        OriginalScheduleEntry: Record "TM Admission Schedule Entry";
        RescheduleToScheduleEntry: Record "TM Admission Schedule Entry";
        ...
        TmpTicketParticipantWks: Record "TM Ticket Participant Wks." temporary;
        TmpTicketParticipantWks2: Record "TM Ticket Participant Wks." temporary;
        Admission: Record "TM Admission";
