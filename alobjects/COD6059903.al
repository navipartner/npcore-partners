codeunit 6059903 "Task Queue Execute"
{
    ...
    procedure GetFileName(TaskLine: Record "Task Line"): Text[1024]
    var
    ...
    procedure GetFilePath(TaskLine: Record "Task Line"): Text[1024]
    var
        NewFileName: Text[1024];
        NewFilePath: Text[1024];
    begin
        ...
        while (StrPos(NewFilePath, '{') <> 0) and (Counter < 10) do begin
          ...
          case UpperCase(FieldValue) of
            'CURRENTDATETIME':
              begin
                case true of
                  (UseFormatNumber) :       TextToAdd := Format(CurrentDateTime, FormatLength, FormatNumber);
                  (FormatStr <> '') :       TextToAdd := Format(CurrentDateTime, FormatLength, FormatStr);
                  (FormatLenghtTxt <> '') : TextToAdd := Format(CurrentDateTime, FormatLength);
                  else                      TextToAdd := Format(CurrentDateTime);
                end;
              end;
            'TODAY':
              begin
                case true of
                  (UseFormatNumber) :       TextToAdd := Format(Today, FormatLength, FormatNumber);
                  (FormatStr <> '') :       TextToAdd := Format(Today, FormatLength, FormatStr);
                  (FormatLenghtTxt <> '') : TextToAdd := Format(Today, FormatLength);
                  else                      TextToAdd := Format(Today);
                end;
              end;
            'TIME':
              begin
                case true of
                  (UseFormatNumber) :       TextToAdd := Format(Time, FormatLength, FormatNumber);
                  (FormatStr <> '') :       TextToAdd := Format(Time, FormatLength, FormatStr);
                  (FormatLenghtTxt <> '') : TextToAdd := Format(Time, FormatLength);
                  else                      TextToAdd := Format(Time);
                end;
              end;
            'USERID':
              begin
                case true of
                  (UseFormatNumber) :       TextToAdd := Format(UserId, FormatLength, FormatNumber);
                  (FormatStr <> '') :       TextToAdd := Format(UserId, FormatLength, FormatStr);
                  (FormatLenghtTxt <> '') : TextToAdd := Format(UserId, FormatLength);
                  else                      TextToAdd := Format(UserId);
                end;
              end;
          end;
    ...
    local procedure Suffix(TaskLine: Record "Task Line"): Text
    begin
        //-TQ1.29
