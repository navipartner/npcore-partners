table 6014402 "Payment Type POS"
{
    ...
    fields
    {
        ...
        field(4;"G/L Account No.";Code[20])
        {
            ...
            trigger OnValidate()
            begin
                if RetailSetup.Get then if RetailSetup."Payment Type By Register" then
                  Register.SetRange("Register No.",PaymentTypePOS."Register No.");

                if Register.Find('-') then begin
                  repeat
                    case "Processing Type" of
                      "Processing Type"::Cash      : Register.Account := "G/L Account No.";
                      "Processing Type"::"Gift Voucher"     : Register."Gift Voucher Account" := "G/L Account No.";
                      "Processing Type"::"Credit Voucher" : Register."Credit Voucher Account" := "G/L Account No.";
                    end;
                    Register.Modify;
        ...
        field(25;"Via Terminal";Boolean)
        {
            ...
            begin
                if "Via Terminal" then begin
                  case "Processing Type" of
                    "Processing Type"::Cash : Error(Trans0001);
                    "Processing Type"::"Credit Voucher" : Error(Trans0003);
                    "Processing Type"::EFT : Error(Trans0004);
                  end;
                end;
        ...
        {
            CalcFormula = Sum("Audit Roll"."Amount Including VAT" WHERE ("Register No."=FIELD("Register Filter"),
        ...
        {
            CalcFormula = Count("Audit Roll" WHERE ("Register No."=FIELD("Register Filter"),
                                                    "Sales Ticket No."=FIELD("Receipt Filter"),
                                                    Type=CONST(Item),
        ...
        {
            CalcFormula = Sum("Audit Roll".Quantity WHERE ("Register No."=FIELD("Register Filter"),
                                                           "Sales Ticket No."=FIELD("Receipt Filter"),
                                                           "Sale Date"=FIELD("Date Filter"),
                                                           Type=CONST(Item),
                                                           "Salesperson Code"=FIELD("Salesperson Filter"),
        ...
        {
            CalcFormula = Sum("Audit Roll".Cost WHERE ("Register No."=FIELD("Register Filter"),
                                                       "Sales Ticket No."=FIELD("Receipt Filter"),
                                                       "Sale Date"=FIELD("Date Filter"),
                                                       Type=CONST(Item),
        ...
        {
            CalcFormula = Sum("Audit Roll".Amount WHERE ("Sale Date"=FIELD("Date Filter"),
                                                         "Register No."=FIELD("Register Filter"),
                                                         "Sale Type"=CONST(Sale),
                                                         Type=CONST(Item),
                                                         "Salesperson Code"=FIELD("Salesperson Filter"),
        ...
        {
            CalcFormula = Sum("Audit Roll".Cost WHERE ("Sale Date"=FIELD("Date Filter"),
                                                       "Register No."=FIELD("Register Filter"),
                                                       "Sale Type"=CONST("Debit Sale"),
                                                       Type=CONST(Item),
                                                       "Gift voucher ref."=FILTER(=''),
        ...
        {
            CalcFormula = Sum("Payment Type - Detailed".Amount WHERE ("Payment No."=FIELD("No."),
                                                                      "Register No."=FIELD("Register Filter")));
            Caption = 'Counted';
            Editable = false;
            FieldClass = FlowField;
