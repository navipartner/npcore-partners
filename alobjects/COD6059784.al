codeunit 6059784 "TM Ticket Management"
{
    ...
    begin
        ...
        if TicketType.Get(Item."Ticket Type") and TicketType."Is Ticket" then begin
    ...
    begin
        ...
        if TicketType.Get(Item."Ticket Type") and TicketType."Is Ticket" then begin
          for TicketNo := 1 to TicketQty do begin
            ...
            //-NPR4.16
            Ticket."Sales Receipt No."  := SalesHeader."Sales Ticket No.";
            //+NPR4.16
            // Ticket.INSERT(TRUE);
          end;
        end;
    ...
    procedure IssueTicketsFromAuditRoll(var Auditroll: Record "Audit Roll")
    var
        Token: Text[100];
    ...
    local procedure PrintTicketUsingFormater(var Ticket: Record "TM Ticket";PrintObjectType: Option;PrintObjectId: Integer;PrintTemplateCode: Code[20]) Printed: Boolean
    var
        TicketType: Record "TM Ticket Type";
    ...
    procedure PrintSingleTicket(var Ticket: Record "TM Ticket") Printed: Boolean
    var
        TicketType: Record "TM Ticket Type";
        StdCodeunitCode: Codeunit "Std. Codeunit Code";
        ObjectOutputMgt: Codeunit "Object Output Mgt.";
    ...
    begin
        ...
        exit(CODEUNIT::"TM Ticket Management");
    end;

    [EventSubscriber(ObjectType::Codeunit, 6150705, 'OnFinishSale', '', true, true)]
    local procedure PrintTicketsOnSale(POSSalesWorkflowStep: Record "POS Sales Workflow Step";SalePOS: Record "Sale POS")
    begin
        //-#302779 [302779]
    ...
    var
        ...
        ReservationSchEntry: Record "TM Admission Schedule Entry";
        Ticket: Record "TM Ticket";
        ReservationAccessEntry: Record "TM Det. Ticket Access Entry";
    ...
    var
        ...
        TicketAccessEntry: Record "TM Ticket Access Entry";
        DetailedTicketAccessEntry: Record "TM Det. Ticket Access Entry";
        DepartureAccessEntry: Record "TM Det. Ticket Access Entry";
    begin
    ...
    procedure SetTicketProperties(var Ticket: Record "TM Ticket";ValidFromDate: Date)
    var
        TicketType: Record "TM Ticket Type";
    begin
    ...
    procedure GetTicketAccessEntryValidDateBoundery(Ticket: Record "TM Ticket";var LowDate: Date;var HighDate: Date)
    var
    ...
    procedure CreateAdmissionAccessEntry(FailWithError: Boolean;Ticket: Record "TM Ticket";TicketQty: Integer;AdmissionCode: Code[20];AdmissionSchEntry: Record "TM Admission Schedule Entry";var ResponseMessage: Text) ResponseCode: Integer
    var
        TicketAccessEntry: Record "TM Ticket Access Entry";
        Item: Record Item;
        TicketType: Record "TM Ticket Type";
    ...
    var
        AdmissionBOM: Record "TM Ticket Admission BOM";
        TicketAccessEntry: Record "TM Ticket Access Entry";
        Item: Record Item;
        TicketType: Record "TM Ticket Type";
        Admission: Record "TM Admission";
    ...
    procedure CreatePaymentEntryType(Ticket: Record "TM Ticket";PaymentType: Option PAYMENT,PREPAID,POSPAID;PaymentReferenceNo: Code[20];CustomerNo: Code[20])
    var
        AdmissionBOM: Record "TM Ticket Admission BOM";
        TicketAccessEntry: Record "TM Ticket Access Entry";
        Item: Record Item;
    ...
    procedure RegisterDefaultAdmissionArrivalOnPosSales(FailWithError: Boolean;Ticket: Record "TM Ticket";AdmissionCode: Code[20];ResponseMessage: Text)
    var
        Admission: Record "TM Admission";
    ...
    var
        TicketType: Record "TM Ticket Type";
        TicketTypeForm: Page "TM Ticket Type";
    begin
    ...
    var
        ...
        ClauseString: Codeunit "String Library";
        Clauses: Integer;
        Clause: Integer;
    ...
    var
        TicketAccessEntry: Record "TM Ticket Access Entry";
        ReservationTicketAccessEntry: Record "TM Det. Ticket Access Entry";
        DetailedTicketAccessEntry: Record "TM Det. Ticket Access Entry";
        AdmissionScheduleEntry: Record "TM Admission Schedule Entry";
        Admission: Record "TM Admission";
    begin
    ...
    var
        TicketAccessEntry: Record "TM Ticket Access Entry";
        DepartureTicketAccessEntry: Record "TM Det. Ticket Access Entry";
    begin
        //-NPR4.16 - New Function
    ...
    var
        TicketAccessEntry: Record "TM Ticket Access Entry";
        PaymentTicketAccessEntry: Record "TM Det. Ticket Access Entry";
    begin
        ...
        case PaymentType of
          gAccesEntryPaymentType::PAYMENT : PaymentTicketAccessEntry.Type := PaymentTicketAccessEntry.Type::PAYMENT;
          gAccesEntryPaymentType::PREPAID : PaymentTicketAccessEntry.Type := PaymentTicketAccessEntry.Type::PREPAID;
    ...
    var
        TicketAccessEntry: Record "TM Ticket Access Entry";
        CancelTicketAccessEntry: Record "TM Det. Ticket Access Entry";
        OpenTicketAccessEntry: Record "TM Det. Ticket Access Entry";
        InitialTicketAccessEntry: Record "TM Det. Ticket Access Entry";
        AdmittedTicketAccessEntry: Record "TM Det. Ticket Access Entry";
        ReservationTicketAccessEntry: Record "TM Det. Ticket Access Entry";
        QtyToCancel: Integer;
    ...
    local procedure CloseInitialEntry(var ClosedByAccessEntry: Record "TM Det. Ticket Access Entry"): Boolean
    var
        DetailedTicketAccessEntry: Record "TM Det. Ticket Access Entry";
    begin
        //-NPR4.16 - New Function
    ...
    local procedure CloseArrivalEntry(var ClosedByAccessEntry: Record "TM Det. Ticket Access Entry"): Boolean
    var
        DetailedTicketAccessEntry: Record "TM Det. Ticket Access Entry";
    begin
        //-NPR4.16 - New Function
    ...
    local procedure ClosePostpaidEntry(var ClosedByAccessEntry: Record "TM Det. Ticket Access Entry"): Boolean
    var
        DetailedTicketAccessEntry: Record "TM Det. Ticket Access Entry";
    begin
    ...
    local procedure GetReservationEntry(TicketAccessEntryNo: BigInteger;var DetailedTicketAccessEntry: Record "TM Det. Ticket Access Entry") Closed: Boolean
    begin
        //-NPR4.16 - New Function
    ...
    local procedure GetAdmScheduleEntry(AdmissionCode: Code[20];AdmissionDate: Date;AdmissionTime: Time;var AdmissionSchEntry: Record "TM Admission Schedule Entry";WithCreate: Boolean) Found: Boolean
    var
    ...
    local procedure GetTicket(TicketIdentifierType: Option INTERNAL_TICKET_NO,EXTERNAL_TICKET_NO,PRINTED_TICKET_NO;TicketIdentifier: Text[50];var Ticket: Record "TM Ticket"): Boolean
    begin
        //-NPR4.16 - New Function
            ...
            begin
              ...
              if ((CapacityExceeded) and (FirstAccessDate = LastAccessDate)) then begin
                case TicketBOM."Allow Rescan Within (Sec.)" of
                  TicketBOM."Allow Rescan Within (Sec.)"::"15"  : CapacityExceeded := ((LastAccessTime - FirstAccessTime) >  15000);
                  TicketBOM."Allow Rescan Within (Sec.)"::"30"  : CapacityExceeded := (LastAccessTime - FirstAccessTime) >  30000;
                  TicketBOM."Allow Rescan Within (Sec.)"::"60"  : CapacityExceeded := (LastAccessTime - FirstAccessTime) >  60000;
                  TicketBOM."Allow Rescan Within (Sec.)"::"120" : CapacityExceeded := (LastAccessTime - FirstAccessTime) > 120000;
                end;
    ...
    var
        ...
        TicketType: Record "TM Ticket Type";
        TicketAccessEntry: Record "TM Ticket Access Entry";
        ScheduleEntry: Record "TM Admission Schedule Entry";
        TicketBOM: Record "TM Ticket Admission BOM";
    begin
    ...
    var
        ...
        CM: Codeunit "Calendar Management";
        CalendarDesc: Text;
    begin
        ...
        if (TicketBOM."Ticket Base Calendar Code" <> '') then begin
          NonWorking := CM.CheckCustomizedDateStatus (CustomizedCalendar."Source Type"::Service, AdmissionCode, '', TicketBOM."Ticket Base Calendar Code", AdmissionDate, CalendarDesc);

          if (not NonWorking) then
            NonWorking := CM.CheckCustomizedDateStatus (CustomizedCalendar."Source Type"::Service, AdmissionCode, ItemNo, TicketBOM."Ticket Base Calendar Code", AdmissionDate, CalendarDesc);
    ...
    local procedure GetEntryValidationConstraints(TicketType: Record "TM Ticket Type";TicketBom: Record "TM Ticket Admission BOM";var EntryValidationType: Option UNDEFINED,SINGLE,SAME_DAY,MULTIPLE;var MaxEntryCount: Integer;var TimespanFormula: DateFormula)
    begin
        ...
        if (TicketType."Ticket Configuration Source" = TicketType."Ticket Configuration Source"::TICKET_BOM) then begin
          case TicketBom."Admission Entry Validation" of
            TicketBom."Admission Entry Validation"::SINGLE : EntryValidationType := EntryValidationType::SINGLE;
            TicketBom."Admission Entry Validation"::SAME_DAY : EntryValidationType := EntryValidationType::SAME_DAY;
            TicketBom."Admission Entry Validation"::MULTIPLE : EntryValidationType := EntryValidationType::MULTIPLE;
            else
        ...
        if (TicketType."Ticket Configuration Source" = TicketType."Ticket Configuration Source"::TICKET_TYPE) then begin
          case TicketType."Ticket Entry Validation" of
            TicketType."Ticket Entry Validation"::SINGLE : EntryValidationType := EntryValidationType::SINGLE;
            TicketType."Ticket Entry Validation"::SAME_DAY : EntryValidationType := EntryValidationType::SAME_DAY;
            TicketType."Ticket Entry Validation"::MULTIPLE : EntryValidationType := EntryValidationType::MULTIPLE;
            else
              EntryValidationType := EntryValidationType::UNDEFINED;
          end;
          MaxEntryCount := TicketBom."Max No. Of Entries";
    ...
    var
        TmpTicket: Record "TM Ticket" temporary;
        TmpAggregatedPerRequest: Record "TM Ticket Access Entry" temporary;
        TmpAdmissionPerDate: Record "TM Det. Ticket Access Entry" temporary;
        TmpDetailedAccessEntries: Record "TM Det. Ticket Access Entry" temporary;
    ...
    local procedure CollectUnhandledPostpaidTickets(ShowDialog: Boolean;var TmpPostpaidTickets: Record "TM Ticket" temporary;var TmpDetailedAccessEntries: Record "TM Det. Ticket Access Entry" temporary)
    var
        DetTicketAccessEntry: Record "TM Det. Ticket Access Entry";
    ...
    local procedure AggregatePaymentEntries(ShowDialog: Boolean;var TmpPostpaidTickets: Record "TM Ticket" temporary;var TmpAggregatedPerRequest: Record "TM Ticket Access Entry" temporary;var TmpAdmissionsPerDate: Record "TM Det. Ticket Access Entry" temporary)
    var
        TicketReservationRequest: Record "TM Ticket Reservation Request";
        TicketAccessEntry: Record "TM Ticket Access Entry";
        EntryNo: Integer;
    ...
    local procedure CreatePostpaidTicketInvoice(ShowDialog: Boolean;var TmpAggregatedPerRequest: Record "TM Ticket Access Entry" temporary;var TmpAdmissionsPerDate: Record "TM Det. Ticket Access Entry" temporary)
    var
        TicketReservationRequest: Record "TM Ticket Reservation Request";
        SalesHeader: Record "Sales Header";
    ...
    begin
        ...
        repeat
          if (TicketReservationRequest.Get (TmpAggregatedPerRequest."Entry No.")) then begin
            ...
            SalesHeader."External Order No." := TicketReservationRequest."External Order No.";
    ...
    local procedure MarkPostpaidTicketAsInvoiced(ShowDialog: Boolean;var TmpDetailedAccessEntries: Record "TM Det. Ticket Access Entry" temporary;var TmpAggregatedPerRequest: Record "TM Ticket Access Entry" temporary;var TmpTicket: Record "TM Ticket" temporary)
    var
        DetTicketAccessEntry: Record "TM Det. Ticket Access Entry";
        MaxCount: Integer;
        Index: Integer;
