codeunit 6150627 "POS Workshift Checkpoint"
{
    ...
    var
        ...
        POSWorkshiftCheckpointPage: Page "POS Workshift Checkpoint Card";
    ...
    var
        ...
        UnittoBinRelation: Record "POS Unit to Bin Relation";
        POSWorkshiftCheckpoint: Record "POS Workshift Checkpoint";
        POSUnit: Record "POS Unit";
    ...
    begin
        ...
        if (POSUnit."Default POS Payment Bin" <> '') then begin
          case EntrySourceMethod of
            EntrySourceMethodOption::AUDITROLL : PaymentBinCheckpoint.CreateAuditRollBinCheckpoint (POSUnit."No.", POSUnit."Default POS Payment Bin", CheckPointEntryNo);
            EntrySourceMethodOption::BINENTRY  : PaymentBinCheckpoint.CreatePosEntryBinCheckpoint (POSUnit."No.", POSUnit."Default POS Payment Bin", CheckPointEntryNo);
          end;
        end;
        ...
        if (UnittoBinRelation.FindSet ()) then begin
          repeat
            ...
            case EntrySourceMethod of
              EntrySourceMethodOption::AUDITROLL : PaymentBinCheckpoint.CreateAuditRollBinCheckpoint (UnittoBinRelation."POS Unit No.", UnittoBinRelation."POS Payment Bin No.", CheckPointEntryNo);
              EntrySourceMethodOption::BINENTRY  : PaymentBinCheckpoint.CreatePosEntryBinCheckpoint (UnittoBinRelation."POS Unit No.", UnittoBinRelation."POS Payment Bin No.", CheckPointEntryNo);
            end;
    ...
    begin
        ...
        // Refactored code, moved implementation to local worker functions
        case WithPosting of
          true  : Mode := EodWorkshiftMode::ZREPORT;
          false : Mode := EodWorkshiftMode::XREPORT;
        end;
    ...
    begin
        ...
        // Select Default UI
        case Mode of
          EodWorkshiftMode::ZREPORT        : UIOption := EndOfDayUIOption::SUMMARY;
          EodWorkshiftMode::XREPORT        : UIOption := EndOfDayUIOption::SUMMARY;
          EodWorkshiftMode::CLOSEWORKSHIFT : UIOption := EndOfDayUIOption::NONE;
        end;
        BlindCount := false;
            ...
            EodWorkshiftMode::ZREPORT :
              with POSEndofDayProfile do
                case POSEndofDayProfile."Z-Report UI" of
                  "Z-Report UI"::SUMMARY_BALANCING : UIOption := EndOfDayUIOption::SUMMARY;
                  "Z-Report UI"::BALANCING         : UIOption := EndOfDayUIOption::BALANCING;
                end;

            EodWorkshiftMode::XREPORT :
              with POSEndofDayProfile do
                case POSEndofDayProfile."X-Report UI" of
                  "X-Report UI"::SUMMARY_PRINT : UIOption := EndOfDayUIOption::SUMMARY;
                  "X-Report UI"::PRINT         : UIOption := EndOfDayUIOption::NONE;
                end;
        ...
        if (UIOption = EndOfDayUIOption::SUMMARY) then begin
          ...
          if (not POSPaymentBinCheckpoint.IsEmpty ()) then begin
            ...
            case Mode of
              EodWorkshiftMode::XREPORT : POSWorkshiftCheckpointPage.SetCheckpointMode (0);
              EodWorkshiftMode::ZREPORT : POSWorkshiftCheckpointPage.SetCheckpointMode (1);
            end;
        ...
        if (UIOption = EndOfDayUIOption::BALANCING) then begin
          ...
          if (not POSPaymentBinCheckpoint.IsEmpty ()) then begin
            ...
            case Mode of
              EodWorkshiftMode::XREPORT : PaymentBinCheckpointPage.SetCheckpointMode (0);
              EodWorkshiftMode::ZREPORT : PaymentBinCheckpointPage.SetCheckpointMode (1);
            end;
    ...
    begin
        ...
        case GetEntrySourceMethod () of
          EntrySourceMethodOption::AUDITROLL : CheckPointEntryNo := CreateEndWorkshiftCheckpoint_AuditRoll (UnitNo);
          EntrySourceMethodOption::BINENTRY  : CheckPointEntryNo := CreateEndWorkshiftCheckpoint_POSEntry (UnitNo);
        end;

        POSWorkshiftCheckpoint.Get (CheckPointEntryNo);
        case Mode of
          EodWorkshiftMode::ZREPORT        : POSWorkshiftCheckpoint.Type := POSWorkshiftCheckpoint.Type::ZREPORT;
          EodWorkshiftMode::XREPORT        : POSWorkshiftCheckpoint.Type := POSWorkshiftCheckpoint.Type::XREPORT;
          EodWorkshiftMode::CLOSEWORKSHIFT : POSWorkshiftCheckpoint.Type := POSWorkshiftCheckpoint.Type::XREPORT;
        end;
        ...
        POSPaymentBinCheckpoint.SetFilter ("Workshift Checkpoint Entry No.", '=%1', CheckPointEntryNo);
        case Mode of
          EodWorkshiftMode::ZREPORT        : POSPaymentBinCheckpoint.ModifyAll (Type, POSPaymentBinCheckpoint.Type::ZREPORT);
          EodWorkshiftMode::XREPORT        : POSPaymentBinCheckpoint.ModifyAll (Type, POSPaymentBinCheckpoint.Type::XREPORT);
          EodWorkshiftMode::CLOSEWORKSHIFT : POSPaymentBinCheckpoint.ModifyAll (Type, POSPaymentBinCheckpoint.Type::XREPORT);
        end;
    ...
    var
        POSWorkshiftCheckpoint: Record "POS Workshift Checkpoint";
        TargetPOSWorkshiftCheckpoint: Record "POS Workshift Checkpoint";
    begin
        //-NPR5.49 [348458]
    ...
    var
        TargetWorkshiftTaxCheckpoint: Record "POS Workshift Tax Checkpoint";
    ...
    var
        ...
        POSPaymentBinDenomination: Record "POS Payment Bin Denomination";
        POSPaymentBinCheckpoint: Record "POS Payment Bin Checkpoint";
    begin
    ...
    local procedure CalculateCheckpointStatistics(POSUnitNo: Code[20];var POSWorkshiftCheckpoint: Record "POS Workshift Checkpoint")
    var
        PreviousUnitCheckpoint: Record "POS Workshift Checkpoint";
        FromEntryNo: Integer;
        EntriesToBalance: Record "POS Entry";
    begin
    ...
    local procedure CalculateWorkshiftSummary(POSUnitNo: Code[20];var POSWorkshiftCheckpoint: Record "POS Workshift Checkpoint";FromPosEntryNo: Integer)
    var
          ...
          if (POSEntry."Entry Type" = POSEntry."Entry Type"::"Credit Sale") then begin
            ...
            //+NPR5.49 [348458]
              ...
              case (POSEntry."Sales Document Type") of
                POSEntry."Sales Document Type"::Invoice       : POSWorkshiftCheckpoint."Credit Real. Sale Amt. (LCY)" += "Amount Excl. VAT (LCY)";
                POSEntry."Sales Document Type"::Order         : POSWorkshiftCheckpoint."Credit Real. Sale Amt. (LCY)" += "Amount Excl. VAT (LCY)";
                POSEntry."Sales Document Type"::"Credit Memo" : POSWorkshiftCheckpoint."Credit Real. Return Amt. (LCY)" += "Amount Excl. VAT (LCY)";
                POSEntry."Sales Document Type"::"Return Order": POSWorkshiftCheckpoint."Credit Real. Return Amt. (LCY)" += "Amount Excl. VAT (LCY)";
              end;
    ...
    local procedure SetDiscounts(var POSWorkshiftCheckpoint: Record "POS Workshift Checkpoint";POSSalesLine: Record "POS Sales Line")
    begin
        ...
        with POSSalesLine do begin
          ...
          case "Discount Type" of
            "Discount Type"::"BOM List" : POSWorkshiftCheckpoint."BOM Discount (LCY)" += "Line Dsc. Amt. Incl. VAT (LCY)";
            "Discount Type"::Campaign   : POSWorkshiftCheckpoint."Campaign Discount (LCY)" += "Line Dsc. Amt. Incl. VAT (LCY)";
            "Discount Type"::Customer   : POSWorkshiftCheckpoint."Customer Discount (LCY)" += "Line Dsc. Amt. Incl. VAT (LCY)";
            "Discount Type"::Manual     : POSWorkshiftCheckpoint."Custom Discount (LCY)" += "Line Dsc. Amt. Incl. VAT (LCY)";
            "Discount Type"::Mix        : POSWorkshiftCheckpoint."Mix Discount (LCY)" += "Line Dsc. Amt. Incl. VAT (LCY)";
            "Discount Type"::Quantity   : POSWorkshiftCheckpoint."Quantity Discount (LCY)" += "Line Dsc. Amt. Incl. VAT (LCY)";
            else
    ...
    local procedure SetPayments(var POSWorkshiftCheckpoint: Record "POS Workshift Checkpoint";POSPaymentLine: Record "POS Payment Line";LCYCode: Code[10])
    var
    ...
    begin
        ...
        with POSPaymentLine do begin
          ...
          case POSPaymentMethod."Processing Type" of
            ...
            POSPaymentMethod."Processing Type"::CUSTOMER : POSWorkshiftCheckpoint."Debtor Payment (LCY)" += "Amount (LCY)";
            POSPaymentMethod."Processing Type"::EFT : POSWorkshiftCheckpoint."EFT (LCY)" += "Amount (LCY)";
            POSPaymentMethod."Processing Type"::PAYOUT : ; // PAYOUT is recorded on Sales Line POSWorkshiftCheckpoint."GL Payment (LCY)" += "Amount (LCY)";
            POSPaymentMethod."Processing Type"::VOUCHER : POSWorkshiftCheckpoint."Redeemed Vouchers (LCY)" += "Amount (LCY)";
    ...
    local procedure FinalizeCheckpoint(var POSWorkshiftCheckpoint: Record "POS Workshift Checkpoint")
    begin
    ...
    var
        ...
        PeriodWorkshiftTaxCheckpoint: Record "POS Workshift Tax Checkpoint";
        POSAuditLogMgt: Codeunit "POS Audit Log Mgt.";
        POSAuditLog: Record "POS Audit Log";
    begin
    ...
    local procedure AggregateTaxCheckpoint(var TmpPeriodTaxWorkshiftCheckpoint: Record "POS Workshift Tax Checkpoint" temporary;PeriodEntryNo: Integer;ZReportEntryNo: Integer;var TempEntryNo: Integer)
    var
        POSWorkshiftTaxCheckpoint: Record "POS Workshift Tax Checkpoint";
    begin
    ...
    local procedure BalanceRegister_AR(RegNo: Code[20];var POSWorkshiftCheckpoint: Record "POS Workshift Checkpoint")
    var
    ...
    begin
        ...
        if AuditRoll.FindSet then repeat
          ...
          end else if (AuditRoll."Sales Ticket No." <> LastReceiptNo) then
    ...
    procedure CopyCheckpointToPeriode_AR(CheckpointEntryNo: Integer;var Sale: Record "Sale POS";BalanceDate: Date;BalanceTime: Time;Balanced: Boolean)
    var
    ...
    local procedure AggregateVat_AR(WorkshiftCheckpointEntryNo: Integer;var AuditRoll: Record "Audit Roll")
    var
        TmpPOSWorkshiftTaxCheckpoint: Record "POS Workshift Tax Checkpoint" temporary;
        POSWorkshiftTaxCheckpoint: Record "POS Workshift Tax Checkpoint";
        TempEntryNo: Integer;
    begin
