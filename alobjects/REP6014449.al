report 6014449 "Vendor trn. by Item group"
{
    ...
    dataset
    {
        dataitem(vg;"Item Group")
        {
            ...
            dataitem(Vendor;Vendor)
            {
                ...
                DataItemLink = "Item Group Filter"=FIELD("No."),"Date Filter"=FIELD("Date Filter"),"Global Dimension 1 Filter"=FIELD("Global Dimension 1 Filter"),"No."=FIELD("Vendor Filter");
                ...
                column(DB_vgvendor;"Sales (LCY)"-"COGS (LCY)")
                {
                }
                ...
                begin
                    if (vg.GetFilter("Vendor Filter") = '') and ("Sales (LCY)"=0) then CurrReport.Skip;

                    Clear(dg);
                    if "Sales (LCY)" <> 0 then
                     dg:=(("Sales (LCY)"-"COGS (LCY)")/"Sales (LCY)")*100;

                    vareposter.SetRange("Vendor No.","No.");
                ...
                begin
                    vareposter.SetCurrentKey("Entry Type","Posting Date","Item Group No.","Vendor No.");
                    vareposter.SetRange("Entry Type", vareposter."Entry Type"::Sale);
                    vareposter.SetRange("Item Group No.",vg."No.");
    ...
    var
        ...
        totalsalg: Decimal;
        pcttot: Decimal;
        varegruppefjor: Record "Item Group";
        startdato: Date;
        slutdato: Date;
