codeunit 6150663 "NPRE Waiter Pad Management"
{
    ...
    procedure DuplicateWaiterPadHdr(FromWaiterPad: Record "NPRE Waiter Pad";var NewWaiterPad: Record "NPRE Waiter Pad")
    var
        SeatingWaiterPadLink: Record "NPRE Seating - Waiter Pad Link";
        SeatingWaiterPadLink2: Record "NPRE Seating - Waiter Pad Link";
    begin
        //-NPR5.55 [399170]
        NewWaiterPad."No." := '';
    ...
    procedure CloseWaiterPad(var WaiterPad: Record "NPRE Waiter Pad";ForceClose: Boolean)
    var
        SetupProxy: Codeunit "NPRE Restaurant Setup Proxy";
        Handled: Boolean;
        OK: Boolean;
    ...
    procedure CloseWaiterPadSeatings(var WaiterPad: Record "NPRE Waiter Pad";SetupProxy: Codeunit "NPRE Restaurant Setup Proxy")
    var
        SeatingWaiterPadLink: Record "NPRE Seating - Waiter Pad Link";
        SeatingWaiterPadLink2: Record "NPRE Seating - Waiter Pad Link";
    begin
    ...
    local procedure CloseWaiterPadSeatingLink(SeatingWaiterPadLink: Record "NPRE Seating - Waiter Pad Link";SetupProxy: Codeunit "NPRE Restaurant Setup Proxy")
    var
        SeatingWaiterPadLink2: Record "NPRE Seating - Waiter Pad Link";
        SeatingMgt: Codeunit "NPRE Seating Management";
    begin
    ...
    local procedure CleanupWaiterPad(var WaiterPad: Record "NPRE Waiter Pad")
    var
        WaiterPadLine: Record "NPRE Waiter Pad Line";
    ...
    local procedure WaiterPadCanBeClosed(WaiterPad: Record "NPRE Waiter Pad";SetupProxy: Codeunit "NPRE Restaurant Setup Proxy"): Boolean
    var
    ...
    local procedure WaiterPadSeatingsCanBeClosed(WaiterPad: Record "NPRE Waiter Pad";SetupProxy: Codeunit "NPRE Restaurant Setup Proxy"): Boolean
    var
        ServiceFlowProfile: Record "NPRE Service Flow Profile";
    begin
        //-NPR5.55 [399170]/[384923]
    ...
    local procedure WPIsPaid(WaiterPad: Record "NPRE Waiter Pad"): Boolean
    var
        WaiterPadLine: Record "NPRE Waiter Pad Line";
    begin
        //-NPR5.55 [399170]
    ...
    local procedure WPIsServed(WaiterPad: Record "NPRE Waiter Pad";SetupProxy: Codeunit "NPRE Restaurant Setup Proxy"): Boolean
    var
        ...
        RestPrint: Codeunit "NPRE Restaurant Print";
    begin
        //-#NPR5.55 [384923]
    ...
    procedure MoveNumberOfGuests(var FromWaiterPad: Record "NPRE Waiter Pad";var ToWaiterPad: Record "NPRE Waiter Pad";NumberOfGuests: Integer)
    begin
        //-NPR5.55 [399170]
    ...
    procedure FilterAssignedPrintCategories(AppliesToRecID: RecordID;var AssignedPrintCategory: Record "NPRE Assigned Print Category")
    begin
        //-NPR5.55 [382428]
    ...
    procedure AssignedFlowStatusesAsFilterString(AppliesToRecID: RecordID;StatusObject: Option;var AssignedFlowStatus: Record "NPRE Assigned Flow Status"): Text
    var
    ...
    procedure FilterAssignedFlowStatuses(AppliesToRecID: RecordID;StatusObject: Option;var AssignedFlowStatus: Record "NPRE Assigned Flow Status")
    begin
        //-NPR5.55 [382428]
    ...
    var
        AssignedFlowStatus: Record "NPRE Assigned Flow Status";
        AssignedFlowStatusTmp: Record "NPRE Assigned Flow Status" temporary;
        AssignedPrintCategory: Record "NPRE Assigned Print Category";
        PrintCategory: Record "NPRE Print/Prod. Category";
        PrintCategoryList: Page "NPRE Select Print Categories";
        Handled: Boolean;
    begin
    ...
    var
        ...
        Item: Record Item;
        ItemRoutingProfile: Record "NPRE Item Routing Profile";
        NewAssignedPrintCategory: Record "NPRE Assigned Print Category";
        PrintCategory: Record "NPRE Print/Prod. Category";
    ...
    begin
        ...
        case SetupProxy.ServingStepDiscoveryMethod() of
          RestSetup."Serving Step Discovery Method"::"Legacy (using print tags)": begin
          ...
          RestSetup."Serving Step Discovery Method"::"Item Routing Profiles": begin
            if not (Item.Get(WaiterPadLine."No.") and (Item."NPRE Item Routing Profile" <> '')) then
              exit;
            ItemRoutingProfile.Get(Item."NPRE Item Routing Profile");
    ...
    procedure AddAssignedPrintCategory(AppliesToRecID: RecordID;PrintCategory: Record "NPRE Print/Prod. Category";var NewAssignedPrintCategory: Record "NPRE Assigned Print Category")
    begin
        //-NPR5.55 [399170]/[382428]
        //(Moved from CU6150660)
    ...
    procedure SelectFlowStatuses(AppliesToRecID: RecordID;StatusObject: Option;var AssignedFlowStatus: Record "NPRE Assigned Flow Status")
    var
        FlowStatus: Record "NPRE Flow Status";
        FlowStatusList: Page "NPRE Select Flow Status";
        Handled: Boolean;
    begin
        //-NPR5.55 [382428]
        FlowStatus.Reset;
        ...
        case StatusObject of
          FlowStatus."Status Object"::WaiterPadLineMealFlow: begin
            FilterAssignedFlowStatuses(AppliesToRecID, StatusObject, AssignedFlowStatus);
            if AssignedFlowStatus.FindSet then
              repeat
                if FlowStatus.Get(AssignedFlowStatus."Flow Status Code", AssignedFlowStatus."Flow Status Object") then
    ...
    procedure AssignWPadLineServingStepsFromPrintCategories(WaiterPadLine: Record "NPRE Waiter Pad Line";RemoveExisting: Boolean)
    var
        AssignedPrintCategory: Record "NPRE Assigned Print Category";
        FlowStatus: Record "NPRE Flow Status";
        Handled: Boolean;
    ...
    procedure AddAssignedFlowStatus(AppliesToRecID: RecordID;FlowStatus: Record "NPRE Flow Status")
    var
        AssignedFlowStatus: Record "NPRE Assigned Flow Status";
    begin
        //-NPR5.55 [382428]
    ...
    procedure OnBeforeSelectPrintCategories(AppliesToRecID: RecordID;var PrintCategory: Record "NPRE Print/Prod. Category";var Handled: Boolean)
    begin
        //NPR5.55 [360258]
    end;

    [IntegrationEvent(false, false)]
    procedure OnAfterSelectPrintCategories(AppliesToRecID: RecordID;var PrintCategory: Record "NPRE Print/Prod. Category";var Handled: Boolean)
    begin
        //NPR5.55 [360258]
    end;

    [IntegrationEvent(false, false)]
    procedure OnBeforeSelectFlowStatuses(AppliesToRecID: RecordID;StatusObject: Option;var FlowStatus: Record "NPRE Flow Status";var Handled: Boolean)
    begin
        //NPR5.55 [382428]
    end;

    [IntegrationEvent(false, false)]
    procedure OnAfterSelectFlowStatuses(AppliesToRecID: RecordID;StatusObject: Option;var FlowStatus: Record "NPRE Flow Status";var Handled: Boolean)
    begin
        //NPR5.55 [382428]
    end;

    [IntegrationEvent(false, false)]
    procedure OnBeforeCloseWaiterPad(var WaiterPad: Record "NPRE Waiter Pad";SetupProxy: Codeunit "NPRE Restaurant Setup Proxy";var ForceClose: Boolean;var Handled: Boolean)
    begin
        //NPR5.55 [399170]
    end;

    [IntegrationEvent(false, false)]
    procedure OnAfterCloseWaiterPad(var WaiterPad: Record "NPRE Waiter Pad")
    begin
        //NPR5.55 [399170]
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterWaiterPadCleanup(var WaiterPad: Record "NPRE Waiter Pad")
    begin
        //NPR5.55 [399170]
    end;
}
