codeunit 6014545 "RP Blaster CPL Device Library"
{
    ...
    begin
        ...
        if DeviceSettings.FindSet then repeat
          case DeviceSettings.Name of
            'MEDIA_SIZE' : InitializePrinter(DeviceSettings.Value);
            else
    ...
    begin
        ...
        else if CopyStr(FontType,1,4) = 'UPCA' then
        ...
        end else if CopyStr(FontType,1,6) = 'STRING' then begin
          StringLib.Construct(FontType);
          FontParam := StringLib.SelectStringSep(2,' ');
          String(FontParam,X,Y,TextIn)
        end else if CopyStr(FontType,1,4) = 'TEXT' then begin
          StringLib.Construct(FontType);
          FontParam := StringLib.SelectStringSep(2,' ');
          Text(FontParam,X,Y,TextIn)
        end else if CopyStr(FontType,1,9) = 'ULTRAFONT' then begin
          PrintUltraFont(FontType,X,Y,TextIn)
        end else if CopyStr(FontType,1,4) = 'LINE' then begin
          DrawLine(X,Y,Width);
        end else if CopyStr(FontType,1,5) = 'SETUP' then begin
    ...
    begin
        ...
        case true of
          StrLen(TextIn) = 13 : FontCode := 'EAN13';
          StrLen(TextIn) = 12 : FontCode := 'UPCA';
          StrLen(TextIn) < 12  : FontCode := 'CODE39';
        end;
    ...
    begin
        ...
        if RetailList.RunModal = ACTION::LookupOK then begin
          ...
          case tmpDeviceSetting.Name of
            'MEDIA_SIZE' : tmpDeviceSetting."Data Type" := tmpDeviceSetting."Data Type"::Text;
          end;
