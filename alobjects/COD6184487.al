codeunit 6184487 "Pepper Library Transcendence"
{
    ...
    // NPR5.46/MMV /20180924 CASE 290734 EFT refactoring
    ...
    begin
        ...
        //-NPR5.46 [290734]
        //PepperSetupInitialized := FindTerminalSetupFromRegister(RegisterNo, PepperTerminal, PepperInstance, PepperConfiguration, PepperVersion);
    ...
    local procedure MakeDeviceRequest(EFTTransactionRequest: Record "EFT Transaction Request"): Boolean
    begin
          ...
          case "Processing Type" of
            ...
            "Processing Type"::OTHER :
              case "Pepper Trans. Subtype Code" of
                ...
                '20' : TrxVoidPaymentOfGoods (EFTTransactionRequest);
                '60' : TrxRefund (EFTTransactionRequest);
                else
                  exit (false);
              end;
    ...
    local procedure AutomaticallyBeginWorkshift(EFTTransactionRequest: Record "EFT Transaction Request"): Boolean
    var
        EftBeginWorkshiftRequest: Record "EFT Transaction Request";
    begin
    ...
    local procedure BeginWorkshift(EFTTransactionRequest: Record "EFT Transaction Request";ForceDownloadLicense: Boolean)
    var
        PepperConfigManagement: Codeunit "Pepper Config. Management";
        PepperBeginWorkshift: Codeunit "Pepper Begin Workshift TSD";
    ...
    local procedure BeginWorkshiftResponse(PepperBeginWorkshift: Codeunit "Pepper Begin Workshift TSD";EFTTransactionRequest: Record "EFT Transaction Request")
    var
        PepperConfigManagement: Codeunit "Pepper Config. Management";
        LicenseString: Text;
        EftRetryTransactionRequest: Record "EFT Transaction Request";
        EftRecoverTransactionRequest: Record "EFT Transaction Request";
    begin
    ...
    local procedure BeginWorkshiftResponse_MOCK(EFTTransactionRequest: Record "EFT Transaction Request")
    begin
    ...
    local procedure EndWorkshift(EFTTransactionRequest: Record "EFT Transaction Request")
    var
        PepperEndWorkshift: Codeunit "Pepper End Workshift TSD";
    ...
    local procedure EndWorkshiftResponse(PepperEndWorkshift: Codeunit "Pepper End Workshift TSD";EFTTransactionRequest: Record "EFT Transaction Request")
    begin
    ...
    local procedure EndWorkshiftResponse_MOCK(EFTTransactionRequest: Record "EFT Transaction Request")
    begin
    ...
    local procedure TrxRecoverTransaction(EFTTransactionRequest: Record "EFT Transaction Request")
    var
        PepperTrxTransaction: Codeunit "Pepper Trx Transaction TSD";
    ...
    local procedure TrxPaymentOfGoods(EFTTransactionRequest: Record "EFT Transaction Request";MbxPosReference: Text[20])
    var
        PepperTrxTransaction: Codeunit "Pepper Trx Transaction TSD";
        ActivateOffline: Boolean;
        ResultCode: Integer;
    ...
    local procedure TrxVoidPaymentOfGoods(EFTTransactionRequest: Record "EFT Transaction Request")
    var
        PepperTrxTransaction: Codeunit "Pepper Trx Transaction TSD";
        ActivateOffline: Boolean;
        ResultCode: Integer;
    ...
    local procedure TrxRefund(EFTTransactionRequest: Record "EFT Transaction Request")
    var
        PepperTrxTransaction: Codeunit "Pepper Trx Transaction TSD";
        ActivateOffline: Boolean;
        ResultCode: Integer;
    ...
    local procedure TrxResponse(PepperTrxTransaction: Codeunit "Pepper Trx Transaction TSD";EFTTransactionRequest: Record "EFT Transaction Request")
    var
        EftBeginWorkshiftTransaction: Record "EFT Transaction Request";
        EftRetryTransactionRequest: Record "EFT Transaction Request";
    ...
    begin
        ...
        //+NPR5.46 [290734]
          ...
          if ("Result Code" <> -50) then begin
            ...
            //-NPR5.46 [290734]
        //    "POS Payment Type Code" := GetPaymentTypePOS ("Card Type");
    ...
    local procedure TrxResponse_MOCK(EFTTransactionRequest: Record "EFT Transaction Request")
    begin
    ...
    local procedure HandleRecoveryOfTransaction(EFTTransactionRequest: Record "EFT Transaction Request"): Boolean
    var
        EftRefundTransaction: Record "EFT Transaction Request";
    begin
    ...
    local procedure AuxFunctionRequest(EFTTransactionRequest: Record "EFT Transaction Request")
    var
        PepperAuxFunctions: Codeunit "Pepper Aux Functions TSD";
        AuxFunction: Option;
    begin
        ...
        case AuxFunction of
          SupportedAuxFunction::ABORT      :          PepperAuxFunctions.SetAbort();
          SupportedAuxFunction::CUSTOMMENU :          PepperAuxFunctions.SetShowCustomMenu ();
          SupportedAuxFunction::DIAGNOSTICS :         PepperAuxFunctions.SetGetDiagnostics (false); // setup required ?
          SupportedAuxFunction::DISPWITHNUMINPUT :    PepperAuxFunctions.SetDisplayShowText ('TEXT'); // custom text;
          SupportedAuxFunction::PANSUPPRESSIONOFF :   PepperAuxFunctions.SetPanSuppressionOff ();
          SupportedAuxFunction::PANSUPPRESSIONON :    PepperAuxFunctions.SetPanSuppressionOn ();
          SupportedAuxFunction::SUMMARYREPORT :       PepperAuxFunctions.SetGetSummaryReport (false); //setup required ?
          SupportedAuxFunction::SYSTEMINFO :          PepperAuxFunctions.SetGetSystemInfoTicket (false); // setup required ?
          SupportedAuxFunction::TICKETREPRINT :       PepperAuxFunctions.SetReprintLastTicket (true);
          SupportedAuxFunction::TINAACTIVATION :      PepperAuxFunctions.SetTinaActivation (''); // Setup required
          SupportedAuxFunction::TINADEACTIVATION :    PepperAuxFunctions.SetTinaDeactivation (''); // Setup required
          SupportedAuxFunction::TINAQUERY :           PepperAuxFunctions.SetTinaQuery (''); // Setup Required
          SupportedAuxFunction::SHOWCUSTOMMENU :      PepperAuxFunctions.SetShowCustomMenu ();
        end;
    ...
    local procedure AuxFunctionResponse(PepperAuxFunctions: Codeunit "Pepper Aux Functions TSD";EFTTransactionRequest: Record "EFT Transaction Request")
    var
        AuxFunction: Option;
        EftBeginWorkshiftTransaction: Record "EFT Transaction Request";
    begin
        ...
        with EFTTransactionRequest do begin
          ...
          case AuxFunction of
            ...
            SupportedAuxFunction::TICKETREPRINT: begin
            ...
            SupportedAuxFunction::TINAQUERY : begin
    ...
    local procedure InstallPepperRequest(EFTTransactionRequest: Record "EFT Transaction Request")
    var
        PepperFileMgmtFunctions: Codeunit "Pepper FileMgmt. Functions TSD";
        AuxFunction: Option;
    begin
    ...
    local procedure InstallPepperResponse(EFTTransactionRequest: Record "EFT Transaction Request")
    var
        PepperFileMgmtFunctions: Codeunit "Pepper FileMgmt. Functions TSD";
    begin
    ...
    local procedure CreateBeginWorkshiftRequest(RegisterNo: Code[20];var EFTTransactionRequest: Record "EFT Transaction Request"): Boolean
    begin
    ...
    local procedure CreateEndWorkshiftRequest(RegisterNo: Code[20];SalesReceiptNo: Code[20];var EFTTransactionRequest: Record "EFT Transaction Request"): Boolean
    begin
    ...
    local procedure CreatePaymentOfGoodsRequest(RegisterNo: Code[20];SalesReceiptNo: Code[20];CurrencyCode: Code[10];Amount: Decimal;var EFTTransactionRequest: Record "EFT Transaction Request"): Boolean
    var
    ...
    begin
        ...
        with EFTTransactionRequest do begin
          ...
          "Amount Input" := Amount;
          //-NPR5.46 [290734]
          //"Cashback Amount" := PepperProtocol.CalcCashBackAmount(Amount,RegisterNo,"Sales Ticket No.");
          //+NPR5.46 [290734]
    ...
    local procedure CreateRecoveryTransactionRequest(RegisterNo: Code[20];SalesReceiptNo: Code[20];CurrencyCode: Code[10];Amount: Decimal;var EFTTransactionRequest: Record "EFT Transaction Request"): Boolean
    var
        GLSetup: Record "General Ledger Setup";
    begin
    ...
    local procedure CreateRefundRequest(RegisterNo: Code[20];SalesReceiptNo: Code[20];CurrencyCode: Code[10];Amount: Decimal;ReferenceNumber: Code[12];var EFTTransactionRequest: Record "EFT Transaction Request"): Boolean
    var
        GLSetup: Record "General Ledger Setup";
    begin
    ...
    local procedure CreateVoidPaymentOfGoodsRequest(RegisterNo: Code[20];SalesReceiptNo: Code[20];CurrencyCode: Code[10];Amount: Decimal;ReferenceNumber: Code[12];var EFTTransactionRequest: Record "EFT Transaction Request"): Boolean
    var
        GLSetup: Record "General Ledger Setup";
    begin
    ...
    local procedure CreateAuxRequest(RegisterNo: Code[20];AuxFunction: Option;var EFTTransactionRequest: Record "EFT Transaction Request"): Boolean
    begin
    ...
    local procedure CreateInstallRequest(RegisterNo: Code[20];var EFTTransactionRequest: Record "EFT Transaction Request"): Boolean
    begin
    ...
    local procedure InitializeGenericRequest(RegisterNo: Code[20];var EFTTransactionRequest: Record "EFT Transaction Request")
    begin
    ...
    procedure SetTrxProcessingType(TransactionSubtypeCode: Code[10];RegisterNo: Code[10];var EFTTransactionRequest: Record "EFT Transaction Request")
    begin
    ...
    local procedure FindTerminalSetupFromRegister(RegisterNo: Code[10];var VarPepperTerminal: Record "Pepper Terminal";var VarPepperInstance: Record "Pepper Instance";var VarPepperConfiguration: Record "Pepper Configuration";var VarPepperVersion: Record "Pepper Version";var VarEFTSetup: Record "EFT Setup"): Boolean
    begin
    ...
    local procedure CheckTestMode(PepperConfiguration: Record "Pepper Configuration"): Boolean
    var
    ...
    var
        PepperTransactionType: Record "Pepper EFT Transaction Type";
        PepperConfiguration: Record "Pepper Configuration";
    begin
    ...
    local procedure GetPepperReceiptEncoding(PepperTerminal: Record "Pepper Terminal"): Code[50]
    begin
    ...
    local procedure GetNavReceiptEncoding(PepperTerminal: Record "Pepper Terminal"): Code[50]
    begin
    ...
    var
        EFTTransactReqComment: Record "EFT Transact. Req. Comment";
    begin
    ...
    begin
        ...
        //EFTTransactReqComment.SETFILTER ("Entry No.", '=%1', EftRequestEntryNo);
        //IF (EFTTransactReqComment.FINDLAST) THEN
        //  I := EFTTransactReqComment."Line No.";
        ...
        repeat
          ...
          if CommentText[I] <> '' then begin
            ...
            EFTTransactReqComment."Entry No." := EftRequestEntryNo;
            //-NPR5.38 [298176]
            //EFTTransactReqComment."Line No." := I;
    ...
    local procedure AddReceipt(var EFTTransactionRequest: Record "EFT Transaction Request";ReceiptText: Text): Boolean
    var
        OStream: OutStream;
    begin
    ...
    local procedure AttemptOpenAndRetry(EFTTransactionRequest: Record "EFT Transaction Request"): Boolean
    var
        EFTResultCode: Record "Pepper EFT Result Code";
    begin
    ...
    procedure SetTerminalToOfflineMode(Register: Record Register;CommandType: Option Activate,Deactivate) Success: Boolean
    var
        PepperTerminal: Record "Pepper Terminal";
    begin
    ...
    local procedure OnEftDeviceRequest(EftTransactionRequest: Record "EFT Transaction Request";var Handled: Boolean)
    begin
    ...
    begin
        //-NPR5.46 [290734]
        // IF (NOT IsForMe (Register, Handled)) THEN
        //  EXIT;

        // CLEAR (EFTTransactionRequest);

        // Handled := CreateBeginWorkshiftRequest (Register."Register No.", EFTTransactionRequest);
    ...
    begin
        ...
        // Handled := CreateEndWorkshiftRequest (Register."Register No.", SalesReceiptNo, EFTTransactionRequest);
    ...
    begin
        ...
        // Handled := CreatePaymentOfGoodsRequest (Register."Register No.", SalesReceiptNo, CurrencyCode, AmountToCapture, EFTTransactionRequest);
    ...
    local procedure OnCreateRefundOfGoodsRequest(var EftTransactionRequest: Record "EFT Transaction Request";var Handled: Boolean)
    var
    ...
    begin
        ...
        // CLEAR (EFTTransactionRequest);
        // IF (OriginalTransactionRequest.GET (OriginalRequestEntryNo)) THEN ;

        // Handled := CreateRefundRequest (Register."Register No.", SalesReceiptNo, CurrencyCode, AmountToRefund, OriginalTransactionRequest."Reference Number Output", EFTTransactionRequest);
        ...
        if (Handled) then begin
          //-NPR5.46 [290734]
          //IF (OriginalTransactionRequest.GET (OriginalRequestEntryNo)) THEN BEGIN
    ...
    local procedure OnCreateVoidPaymentOfGoodsRequest(var EftTransactionRequest: Record "EFT Transaction Request";var Handled: Boolean)
    var
        OriginalTransactionRequest: Record "EFT Transaction Request";
    begin
        //-NPR5.46 [290734]
        ...
        //  Handled := CreateVoidPaymentOfGoodsRequest (Register."Register No.", SalesReceiptNo, "Currency Code", "Amount Output", "Reference Number Output", EFTTransactionRequest);
        ...
        if (Handled) then begin
          //-NPR5.46 [290734]
          //IF (OriginalTransactionRequest.GET (PaymentTransactonRequestEntryNoToVoid)) THEN BEGIN
          if (OriginalTransactionRequest.Get (EftTransactionRequest."Processed Entry No.")) then begin
          //+NPR5.46 [290734]
            OriginalTransactionRequest.Reversed := true;
            OriginalTransactionRequest."Reversed by Entry No." := EftTransactionRequest."Entry No.";
    ...
    begin
        ...
        // Handled := CreateAuxRequest (Register."Register No.", AuxFunction, EFTTransactionRequest);
    ...
    local procedure OnCreateInstallRequest(var EftTransactionRequest: Record "EFT Transaction Request";var Handled: Boolean)
    begin
        //-NPR5.46 [290734]
        // IF (NOT IsForMe (Register, Handled)) THEN
        //  EXIT;
        ...
        // Handled := CreateInstallRequest (Register."Register No.", EFTTransactionRequest);
    ...
    var
        AlternativeTransactionRequest: Record "EFT Transaction Request";
    begin
        //-NPR5.46 [290734]
        if not EftTransactionRequest.IsType(GetIntegrationType()) then
          exit;
    ...
    local procedure OnDisplayReceipt(EFTTransactionRequest: Record "EFT Transaction Request";ReceiptNo: Integer;var Handled: Boolean)
    var
        PepperConfigManagement: Codeunit "Pepper Config. Management";
    begin
        //-NPR5.46 [290734]
    ...
    begin
        ...
        tmpEFTIntegrationType."Codeunit ID" := CODEUNIT::"Pepper Library Transcendence";
    ...
    local procedure OnAfterEFTSetupInsert(var Rec: Record "EFT Setup";RunTrigger: Boolean)
    var
        EFTSetup: Record "EFT Setup";
    begin
        //-NPR5.46 [290734]
    ...
    local procedure OnAfterEFTSetupRename(var Rec: Record "EFT Setup";var xRec: Record "EFT Setup";RunTrigger: Boolean)
    var
        EFTSetup: Record "EFT Setup";
    begin
        //-NPR5.46 [290734]
    ...
    local procedure OnBeginWorkshiftResponse(var Sender: Codeunit "Pepper Begin Workshift TSD";EFTPaymentRequestID: Integer)
    var
    ...
    local procedure OnTransactionResponse(var Sender: Codeunit "Pepper Trx Transaction TSD";EFTPaymentRequestID: Integer)
    var
        EFTTransactionRequest: Record "EFT Transaction Request";
    begin
    ...
    local procedure OnEndWorkshiftResponse(var Sender: Codeunit "Pepper End Workshift TSD";EFTPaymentRequestID: Integer)
    var
        EFTTransactionRequest: Record "EFT Transaction Request";
    begin
    ...
    local procedure OnAuxResponse(var Sender: Codeunit "Pepper Aux Functions TSD";EFTPaymentRequestID: Integer)
    var
        EFTTransactionRequest: Record "EFT Transaction Request";
    begin
