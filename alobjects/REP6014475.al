report 6014475 "Item Replenishment by Store"
{
                  ...
                  repeat
                    if ItemVariant.FindSet then
                      repeat
                        ...
                        SearchItemReplenishSetupAndInsert(Location."Store Group Code","No.",ItemVariant.Code,Location.Code);
                      until ItemVariant.Next = 0
                    else begin
                      CalcQtyAndInsert("No.",'',Location.Code);
                      SearchItemReplenishSetupAndInsert(Location."Store Group Code","No.",'',Location.Code);
                    end;
    ...
    begin
        ...
        if ItemReplenishByStore.Get(StoreGroupHere,ItemNoHere,VariantCodeHere) then begin
          ...
          MakeTotals();
          ItemLedgEntryTemp.Modify;
        end else if ShowItems = ShowItems::"With Replenish. Setup" then
    ...
    begin
        ...
        if ItemLedgEntryTemp."Document No." <> '' then //if reorder point has been set check if it's greater then projected inventory, else leave new projected inventory from previous code block
          ...
          end else if ItemLedgEntryTemp.Description <> '' then //if max. inventory has been set then check if new inventory will be greater, else leave new projected inventory from previous code block
