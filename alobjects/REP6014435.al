report 6014435 "Vendor/Item Group"
{
    ...
    dataset
    {
        ...
        {
            CalcFields = "COGS (LCY)","Sales (LCY)","Purchases (LCY)";
            PrintOnlyIfDetail = true;
            RequestFilterFields = "No.","Date Filter","Global Dimension 1 Filter","Item Group Filter";
            ...
            column(SalesLCY_Vendor;"Sales (LCY)")
            {
            }
            column(SalesQty_Vendor;"Sales (Qty.)")
            {
            }
            column(LY_SalesLCY_Vendor;Kreditorsidsteaar."Sales (LCY)")
            {
            }
            column(LY_SalesQty_Vendor;Kreditorsidsteaar."Sales (Qty.)")
            {
            }
            column(pctaendring_Vendor;PctaendringVen)
            {
            }
            column(DB_Vendor;"Sales (LCY)"-"COGS (LCY)")
            {
            }
            column(dg_Vendor;DG)
            {
            ...
            dataitem("Item Group";"Item Group")
            {
                ...
                begin
                    ...
                    if Sortervaregruppe then begin
                      if Vendor."Sales (LCY)" <> 0 then
                        Pctoms := "Sales (LCY)" / Vendor."Sales (LCY)" * 100
                      else
                        Clear(Pctoms);
                      ...
                      case Sorterefter of
                        Sorterefter::antal      : TempNPRBuffer."Decimal 1" := "Sales (Qty.)";
                        Sorterefter::omsaetning : TempNPRBuffer."Decimal 1" := "Sales (LCY)";
                        Sorterefter::db         : TempNPRBuffer."Decimal 1" := ("Sales (LCY)"-"Consumption (Amount)");
                      end;
                ...
                begin
                    ...
                    //+NPR5.39
                    if Vendor.GetFilter("Item Group Filter") <> '' then
                      SetFilter("No.", Vendor.GetFilter("Item Group Filter"));
                    if Vendor.GetFilter("Global Dimension 1 Filter") <> '' then
            ...
            dataitem(ItemGrpSum;"Integer")
            {
                ...
                begin
                    ...
                    VendorPctoms := (SaleLCYSum / Vendor."Sales (LCY)") * 100;
                end;
            }
            ...
            begin
                ...
                SumCost := 0;
                //+NPR5.36
                CalcFields("Sales (Qty.)", "Purchases (LCY)","Sales (LCY)");
                ...
                Kreditorsidsteaar.CalcFields("Sales (LCY)","Sales (Qty.)", "Purchases (LCY)");
                ...
                if Kreditorsidsteaar."Sales (LCY)" <> 0 then
                  PctaendringVen := ((Vendor."Sales (LCY)" - Kreditorsidsteaar."Sales (LCY)") / (Kreditorsidsteaar."Sales (LCY)")) * 100
                else
                  Clear(PctaendringVen);

                TotalVendorSalesLCY += "Sales (LCY)";
            end;
    ...
    begin
        ...
        case Sorter of
         Sorter::Mindste : TempNPRBuffer.Ascending(false);
        end;
        //-NPR5.25
    end;
    ...
    var
        ...
        Varegrupperec: Record "Item Group";
        Totalantal: Decimal;
