page 6014633 "GCP Ticket Options"
{
    ...
    begin
        case Attribute of
          ...
          'duplex'           : ShowValue := 'type';
          'page_orientation' : ShowValue := 'type';
          'media_size'       : ShowValue := 'vendor_id'
          else
            exit(false)
        end;
    ...
    begin
        case Attribute of
          'color'            : exit(Property in ['vendor_id', 'type']);
          'duplex'           : exit(Property in ['type']);
          'page_orientation' : exit(Property in ['type']);
          'media_size'       : exit(Property in ['width_microns', 'height_microns', 'is_continuous_feed', 'vendor_id']);
        end;
    end;
}
