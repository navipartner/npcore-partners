codeunit 6014542 "RP Zebra ZPL Device Library"
{
    ...
    begin
        ...
        if DeviceSettings.FindSet then repeat
          case DeviceSettings.Name of
            'PRINT_RATE' : Setup('PR', DeviceSettings.Value);
            'PRINT_WIDTH' : Setup('PW', DeviceSettings.Value);
            'SET_DARKNESS' : Setup('SD', DeviceSettings.Value);
            'MEDIA_DARKNESS' : Setup('MD', DeviceSettings.Value);
            'LABEL_LENGTH' : Setup('LL', DeviceSettings.Value);
    ...
    begin
        ...
        case true of
          StrLen(TextIn) = 13       : FontCode := 'E';        // EAN13
          StrLen(TextIn) = 12       : FontCode := 'U';        // UPC-A
          StrLen(TextIn) in [9..11] : FontCode := '1';        // Text
          StrLen(TextIn) < 9        : FontCode := '3';        // Code39
        end;
    ...
    begin
        ...
        case Type of
          '3' : TempPattern := '^B%1%2,%6,%3,%4,%5'; //CODE39
          'E' : TempPattern := '^B%1%2,%3,%4,%5';    //EAN13
          'U' : TempPattern := '^B%1%2,%3,%4,%5,%6'; //UPC-A
          'C' : TempPattern := '^B%1%2,%3,%4,%5,%6,A'; //CODE128 in automatic subset mode
        end;
    ...
    begin
        ...
        if Align > 0 then begin
          ...
          case Align of
            1 :
              case Rotate of
                0,2 : HorzStart := X - (Round((StrLength / 2),1,'=') div 1);
                1,3 : HorzStart := Y - (Round((StrLength / 2),1,'=') div 1);
              end;
            2 :
              case Rotate of
                0,2 : HorzStart := X - StrLength;
                1,3 : HorzStart := Y - StrLength;
              end;
          end;
    ...
    begin
        ...
        case Setting of
          'PR' : PrintRate(StringLib.SelectStringSep(1,','),StringLib.SelectStringSep(2,','),StringLib.SelectStringSep(3,','));
          'PW' : if Evaluate(IntegerValue, Value) then PrintWidth(IntegerValue);
          'SD' : if Evaluate(IntegerValue, Value) then SetDarkness(IntegerValue);
          'MD' : if Evaluate(IntegerValue, Value) then MediaDarkness(IntegerValue);
          'LL' : if Evaluate(IntegerValue, Value) then LabelLength(IntegerValue);
          'MT' : MediaType(CopyStr(Value,1,1));
          'PO' : PrintOrientation(CopyStr(Value,1,1));
          'LH' : if Evaluate(IntegerValue,  StringLib.SelectStringSep(1,',')) and
    ...
    begin
        ...
        if Align > 0 then begin
          ...
          case Align of
            1 :
              case Rotate of
                0,2 : HorzStart := Round(X - (StrLength / 2),1,'=') div 1;
                1,3 : HorzStart := Round(Y - (StrLength / 2),1,'=') div 1;
              end;
            2 :
              case Rotate of
                0,2 : HorzStart := X - StrLength;
    ...
    begin
        //-NPR5.48 [327107]
        case Encoding of
          Encoding::"Windows-1252" : exit('Windows-1252');
          Encoding::"UTF-8" : exit('utf-8');
        end;
        //+NPR5.48 [327107]
    end;
    ...
    begin
        ...
        if RetailList.RunModal = ACTION::LookupOK then begin
          ...
          case tmpDeviceSetting.Name of
            'LABEL_HOME' : tmpDeviceSetting."Data Type" := tmpDeviceSetting."Data Type"::Text;
            'LABEL_LENGTH' : tmpDeviceSetting."Data Type" := tmpDeviceSetting."Data Type"::Integer;
            'MEDIA_DARKNESS' : tmpDeviceSetting."Data Type" := tmpDeviceSetting."Data Type"::Integer;
            'MEDIA_TYPE' :
              begin
            ...
            'PRINT_RATE' : tmpDeviceSetting."Data Type" := tmpDeviceSetting."Data Type"::Text;
            'PRINT_WIDTH' : tmpDeviceSetting."Data Type" := tmpDeviceSetting."Data Type"::Integer;
            'SET_DARKNESS' : tmpDeviceSetting."Data Type" := tmpDeviceSetting."Data Type"::Integer;
        //-NPR5.48 [327107]
            'RFID_EPC_MEMORY' : tmpDeviceSetting."Data Type" := tmpDeviceSetting."Data Type"::Text;
            'ENCODING' :
              begin
                tmpDeviceSetting."Data Type" := tmpDeviceSetting."Data Type"::Option;
