table 6014425 "Retail Document Header"
{
    ...
    LookupPageID = "Retail Document List";
    ...
    fields
    {
        ...
        field(2;"Customer No.";Code[20])
        {
            ...
            TableRelation = IF ("Customer Type"=CONST(Kontant)) Contact
                            ELSE IF ("Customer Type"=CONST(Alm)) Customer;
        ...
        field(11;"Salesperson Code";Code[10])
        {
            ...
            trigger OnValidate()
            begin
                SalespersonPurchaser.SetRange("Register Password","Salesperson Code");
        ...
        field(15;"Rent Salesperson";Code[10])
        {
            ...
            begin
                ...
                "Salesperson Code" := SalespersonPurchaser."Register Password";
            end;
        }
        ...
        field(17;"Rent Sales Ticket";Code[20])
        {
            ...
            TableRelation = "Audit Roll"."Sales Ticket No.";
        }
        ...
        {
            CalcFormula = Sum("Retail Document Lines"."Total Rental Amount incl. VAT" WHERE ("Document Type"=FIELD("Document Type"),
        ...
        {
            CalcFormula = Sum("Retail Document Lines"."Amount Including VAT" WHERE ("Document Type"=FIELD("Document Type"),
                                                                                    "Document No."=FIELD("No.")));
            Caption = 'Total Price';
            Editable = false;
            FieldClass = FlowField;
        ...
        field(69;Factor;Integer)
        {
            ...
            begin
                  ...
                  "Invoicing Period"::"1 Month": Factor := 1;
                  "Invoicing Period"::"2 Month": Factor := 2;
                  "Invoicing Period"::Quater: Factor := 3;
                  "Invoicing Period"::"Semi Annual": Factor := 6;
                  "Invoicing Period"::Annual: Factor := 12;
                  "Invoicing Period"::None: Factor := 0;
                  end;
            end;
        }
        ...
        {
            CalcFormula = Exist("Retail Comment" WHERE ("Table ID"=CONST(6014425),
                                                        "No."=FIELD("No."),
        ...
        field(106;"Customer Type";Option)
        {
            ...
            var
                ...
                Contact: Record Contact;
                Customer: Record Customer;
                TableCode: Codeunit "Retail Table Code";
            begin
        ...
        {
            CalcFormula = Sum("Retail Document Lines".Quantity WHERE ("Document Type"=CONST("Retail Order"),
                                                                      "Document No."=FIELD("No.")));
            Caption = 'Quantity';
            Editable = false;
            FieldClass = FlowField;
        ...
        {
            CalcFormula = Sum("Retail Document Lines"."Quantity in order" WHERE ("Document Type"=CONST("Retail Order"),
                                                                                 "Document No."=FIELD("No.")));
        ...
        {
            CalcFormula = Sum("Retail Document Lines"."Quantity received" WHERE ("Document Type"=CONST("Retail Order"),
                                                                                 "Document No."=FIELD("No.")));
        ...
        {
            CalcFormula = Sum("Retail Document Lines"."Quantity Shipped" WHERE ("Document Type"=CONST("Retail Order"),
                                                                                "Document No."=FIELD("No.")));
        ...
        {
            CalcFormula = Sum("Retail Document Lines".Amount WHERE ("Document Type"=FIELD("Document Type"),
                                                                    "Document No."=FIELD("No.")));
        ...
        field(122;"Invoice No.";Code[20])
        {
            ...
            TableRelation = IF ("Invoice Document Type"=CONST(Quote)) "Sales Header"."No." WHERE ("Document Type"=CONST(Quote),
                                                                                                  "No."=FIELD("Invoice No."))
                                                                                                  ELSE IF ("Invoice Document Type"=CONST(Order)) "Sales Header"."No." WHERE ("Document Type"=CONST(Order),
                                                                                                                                                                             "No."=FIELD("Invoice No."))
                                                                                                                                                                             ELSE IF ("Invoice Document Type"=CONST(Invoice)) "Sales Header"."No." WHERE ("Document Type"=CONST(Invoice),
                                                                                                                                                                                                                                                          "No."=FIELD("Invoice No."))
                                                                                                                                                                                                                                                          ELSE IF ("Invoice Document Type"=CONST("Credit Memo")) "Sales Header"."No." WHERE ("Document Type"=CONST("Credit Memo"),
                                                                                                                                                                                                                                                                                                                                             "No."=FIELD("Invoice No."));
        }
        ...
        {
            CalcFormula = Sum("Retail Document Lines"."VAT Amount" WHERE ("Document Type"=FIELD("Document Type"),
                                                                          "Document No."=FIELD("No.")));
        ...
        field(157;"VAT Base Amount";Decimal)
        {
            ...
            AutoFormatType = 1;
            CalcFormula = Sum("Retail Document Lines"."VAT Base Amount" WHERE ("Document Type"=FIELD("Document Type"),
                                                                               "Document No."=FIELD("No.")));
        ...
        field(1000;Outstanding;Boolean)
        {
            ...
            trigger OnValidate()
            var
                Linie: Record "Retail Document Lines";
            begin
                  ...
                  if RetailDocumentLine.FindFirst then begin
                    RecalculatePrice :=
                      Confirm(
                        ...
                        true);
                    if RecalculatePrice then repeat
        ...
        field(1025;"POS Entry No.";Integer)
        {
            ...
            TableRelation = "POS Entry"."Entry No.";
        }
    }
    ...
    var
        ...
        RetailDocumentLine: Record "Retail Document Lines";
    ...
    begin
        ...
        case "Document Type" of
          "Document Type"::"Selection Contract" : exit(RetailSetup."Selection No. Series");
          "Document Type"::"Retail Order"       : exit(RetailSetup."Order  No. Series");
          "Document Type"::"Rental contract"    : exit(RetailSetup."Rental Contract  No. Series");
          "Document Type"::"Purchase contract"  : exit(RetailSetup."Purchase Contract  No. Series");
          "Document Type"::Customization        : exit(RetailSetup."Customization  No. Series");
          "Document Type"::Quote                : exit(RetailSetup."Quote  No. Series");
        end;
    end;
    ...
    begin
        RetailSetup.Get;
        case "Document Type" of
          "Document Type"::"Selection Contract" : RetailSetup.TestField("Selection No. Series");
          "Document Type"::"Retail Order"       : RetailSetup.TestField("Order  No. Series");
          "Document Type"::"Rental contract"    : RetailSetup.TestField("Rental Contract  No. Series");
          "Document Type"::"Purchase contract"  : RetailSetup.TestField("Purchase Contract  No. Series");
          "Document Type"::Customization        : RetailSetup.TestField("Customization  No. Series");
          "Document Type"::Quote                : RetailSetup.TestField("Quote  No. Series");
        end;
    end;
    ...
    var
        ...
        RetailDocLine: Record "Retail Document Lines";
        LineNo: Integer;
    ...
    begin
        ...
        if RetailDocLine.Find('-') then repeat
          ...
          if (PurchOrderQty > 0) then begin
            ...
            if ItemHere."Group sale"=false then begin
              ...
              if PurchLine2.Find('-') then repeat
                if PurchLine2."Outstanding Quantity" - PurchLine2."Procure Quantity"> PurchOrderQty then begin
                  if Confirm(StrSubstNo(Trans0002,
                                      PurchLine2."Document No.", RetailDocLine.Description,
                                      PurchLine2."Outstanding Quantity" - PurchLine2."Procure Quantity")) then begin
                    PurchLine2."Procure Quantity" := PurchLine2."Procure Quantity" - PurchOrderQty;
                    PurchLine2.Modify;
        ...
        /*Der findes en mankoliste til leverand�ren med den samme vare*/
            if CreateManko then begin
              ...
              PurchLine2.SetRange("No.", RetailDocLine."No.");
              PurchLine2.SetRange("Buy-from Vendor No.", ItemHere."Vendor No.");
              PurchLine2.SetRange("Compaign Order", false);
        ...
        /*Der tilf�jes en linie til manko-listen*/
                ...
                PurchLine."Procure Quantity" := PurchOrderQty;
                PurchLine.Insert;
                RetailDocLine."Quantity in order":=RetailDocLine."Quantity in order"+PurchOrderQty;
                RetailDocLine.Modify;
                Message(Trans0001,
    ...
    procedure CashFromSale(var Sale: Record "Sale POS")
    var
        RetailDocHeader: Record "Retail Document Header";
    begin
        //CashFromSale()
        with Sale do begin
    ...
    begin
        DoCalc := true;
        case "Invoicing Period" of
          "Invoicing Period"::"1 Month": Factor2 := 12;
          "Invoicing Period"::"2 Month": Factor2 := 6;
          "Invoicing Period"::Quater: Factor2 := 4;
          "Invoicing Period"::"Semi Annual": Factor2 := 2;
          "Invoicing Period"::Annual: Factor2 := 1;
          "Invoicing Period"::None: DoCalc := false;
        end;
        if DoCalc then
    ...
    var
        ReportSelectionRetail1: Record "Report Selection Retail";
        ReportSelectionRetail2: Record "Report Selection - Contract";
        RetailDocHeaderHere: Record "Retail Document Header";
    ...
    begin
        ...
        if (ReportSelectionRetail1.GetFilters <> '') and ReportSelectionRetail1.Find('-') then repeat
          case true of
        ...
        if (ReportSelectionRetail2.GetFilters <> '') and ReportSelectionRetail2.Find('-') then repeat
          case true of
    ...
    procedure AnyPaymentDebit(var AuditRoll: Record "Audit Roll"): Boolean
    begin
        //AnyPaymentDebit
