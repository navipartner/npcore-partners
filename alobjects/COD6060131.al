codeunit 6060131 "MM Member Retail Integration"
{
    ...
    var
        ...
        GlobalExternalCardNo: Text[100];
        GlobalMemberInfoEntryNo: Integer;
    ...
    begin
        ...
        //+MM1.36 [335828]
        case InputMode of
          InputMode::NO_PROMPT : ; // Prevalidated
          InputMode::CARD_SCAN : ExternalMemberCardNo := UI.SearchBox (MEMBER_CARD_NO, MEMBER_MANAGEMENT, 100);
          InputMode::FACIAL_RECOGNITION : begin
    ...
    begin
        ...
        case MembershipSetup."POS Print Action" of
          MembershipSetup."POS Print Action"::DIRECT : PrintMemberAccountCardWorker (Member, MembershipSetup);
          MembershipSetup."POS Print Action"::OFFLINE : MemberManagement.PrintOffline (MemberInfoCapture."Information Context"::PRINT_ACCOUNT, Member."Entry No.");
        end;
    end;
    ...
    procedure IssueMembershipFromPOSEntrySalesLine(POSSalesLine: Record "POS Sales Line")
    var
    ...
    begin
        ...
        //-MM1.36 [307440]
        if (not POSEntry.Get (POSSalesLine."POS Entry No.")) then
          exit;

        MemberInfoCapture.SetCurrentKey ("Receipt No.", "Line No.");
        MemberInfoCapture.SetFilter ("Receipt No.", '=%1', POSSalesLine."Document No.");
        MemberInfoCapture.SetFilter ("Line No.", '=%1', POSSalesLine."Line No.");
        if (not MemberInfoCapture.FindSet ()) then
          exit;

        with POSSalesLine do
          IssueMembershipFromEndOfSaleWorker ("Document No.", "Line No.",  POSEntry."Document Date", "Unit Price", "Amount Excl. VAT (LCY)", "Amount Incl. VAT (LCY)", Description, Quantity);
        //+MM1.36 [307440]
    end;

    procedure IssueMembershipExternal(ReceiptNo: Code[20];ReceiptLine: Integer;SalesDate: Date;UnitPrice: Decimal;Amount_LCY: Decimal;AmountInclVat_LCY: Decimal;Description: Text;Quantity: Decimal)
    begin

        //-MM1.36 [307440]
        IssueMembershipFromEndOfSaleWorker (ReceiptNo, ReceiptLine, SalesDate, UnitPrice, Amount_LCY, AmountInclVat_LCY, Description, Quantity);
        //+MM1.36 [307440]
    end;
    ...
    begin
        ...
        case MemberInfoCapture."Information Context" of
          ...
          MemberInfoCapture."Information Context"::NEW : begin
    ...
    begin
        ...
        case MembershipSalesSetup."Business Flow Type" of
          MembershipSalesSetup."Business Flow Type"::MEMBERSHIP : MemberManagement.DeleteMembership (MemberInfoCapture."Membership Entry No.", true);
        end;
    end;
    ...
    begin
        ...
        ErrorText := '';
        case MsgNo of
          -1100 : exit (gLastMessage);
          -1101 : exit (MSG_1101);
          -1102 : exit (MSG_1102);
          -1103 : exit (MSG_1103);
          -1104 : exit (CARD_BLOCKED_NOT_FOUND);
          -1105 : exit (MSG_1105);
        end;
        //+MM1.05
    end;
