codeunit 6151161 "MM Loyalty Points Mgr (Server)"
{
    ...
    var
        LoyaltyStoreLedger: Record "MM Loyalty Ledger Entry (Srvr)";
    ...
    procedure ReservePoints(var TmpAuthorizationIn: Record "MM Loyalty Ledger Entry (Srvr)" temporary;var TmpReserveLinesIn: Record "MM Register Sales Buffer" temporary;var TmpPointsOut: Record "MM Loyalty Ledger Entry (Srvr)";var ResponseMessage: Text;var ResponseMessageId: Text): Boolean
    var
        LoyaltyStoreLedger: Record "MM Loyalty Ledger Entry (Srvr)";
        MembershipPointsEntry: Record "MM Membership Points Entry";
        Membership: Record "MM Membership";
    ...
    procedure GetLoyaltySetup(var TmpAuthorizationIn: Record "MM Loyalty Ledger Entry (Srvr)" temporary;var TmpLoyaltySetup: Record "MM Loyalty Setup" temporary;var ResponseMessage: Text;var ResponseMessageId: Text): Boolean
    var
        Membership: Record "MM Membership";
        MembershipSetup: Record "MM Membership Setup";
        LoyaltySetup: Record "MM Loyalty Setup";
    ...
    local procedure ValidateAuthorizarion(BasicCheck: Boolean;var TmpAuthorizationIn: Record "MM Loyalty Ledger Entry (Srvr)" temporary;var MembershipEntryNo: Integer;var ResponseMessage: Text;var ResponseMessageId: Text): Boolean
    var
        LoyaltyServerStoreLedger: Record "MM Loyalty Ledger Entry (Srvr)";
        StoreSetup: Record "MM Loyalty Store Setup";
        Membership: Record "MM Membership";
        LoyaltySetup: Record "MM Loyalty Setup";
        MembershipSetup: Record "MM Membership Setup";
    ...
    local procedure ValidateRegisterSales(var TmpSaleLines: Record "MM Register Sales Buffer" temporary;var TmpPaymentLines: Record "MM Register Sales Buffer" temporary;var ResponseMessage: Text;var ResponseMessageId: Text): Boolean
    var
        LoyaltyServerStoreLedger: Record "MM Loyalty Ledger Entry (Srvr)";
        ...
        if (TmpSaleLines.FindSet ()) then begin
          ...
          repeat
            ...
            case TmpSaleLines.Type of
              ...
              begin
                ResponseMessage := StrSubstNo (TYPE_INCORRECT, StrSubstNo ('[%1,%2]', TmpSaleLines.Type::SALES, TmpSaleLines.Type::RETURN));
        ...
        if (TmpPaymentLines.FindSet()) then begin
          repeat
            ...
            case TmpPaymentLines.Type of
              ...
              begin
                ResponseMessage := StrSubstNo (TYPE_INCORRECT, StrSubstNo ('[%1,%2]', TmpPaymentLines.Type::PAYMENT, TmpPaymentLines.Type::REFUND));
    ...
    local procedure ValidateReservePoints(var TmpReserveLines: Record "MM Register Sales Buffer" temporary;MembershipEntryNo: Integer;var ResponseMessage: Text;var ResponseMessageId: Text): Boolean
    var
        Membership: Record "MM Membership";
        LoyaltyServerStoreLedger: Record "MM Loyalty Ledger Entry (Srvr)";
    ...
    local procedure CreateSalesEntry(LoyaltySetup: Record "MM Loyalty Setup";var LoyaltyStoreLedger: Record "MM Loyalty Ledger Entry (Srvr)";Membership: Record "MM Membership";TmpBuffer: Record "MM Register Sales Buffer" temporary;var TotalEarnAmount: Decimal)
    var
        MembershipPointsEntry: Record "MM Membership Points Entry";
        LoyaltyPointManagement: Codeunit "MM Loyalty Point Management";
    begin
    ...
    local procedure CreateCaptureEntry(LoyaltySetup: Record "MM Loyalty Setup";var LoyaltyStoreLedger: Record "MM Loyalty Ledger Entry (Srvr)";Membership: Record "MM Membership";TmpBuffer: Record "MM Register Sales Buffer" temporary;var TotalBurnAmount: Decimal)
    var
        MembershipPointsEntry: Record "MM Membership Points Entry";
        LoyaltyPointManagement: Codeunit "MM Loyalty Point Management";
    begin
    ...
    local procedure CreateReserveEntry(LoyaltySetup: Record "MM Loyalty Setup";var LoyaltyStoreLedger: Record "MM Loyalty Ledger Entry (Srvr)";Membership: Record "MM Membership";TmpBuffer: Record "MM Register Sales Buffer" temporary)
    var
        MembershipPointsEntry: Record "MM Membership Points Entry";
        LoyaltyPointManagement: Codeunit "MM Loyalty Point Management";
    begin
    ...
    local procedure CreateNotEligibleEntry(LoyaltySetup: Record "MM Loyalty Setup";var LoyaltyStoreLedger: Record "MM Loyalty Ledger Entry (Srvr)";Membership: Record "MM Membership";TotalEarnAmount: Decimal;TotalBurnAmount: Decimal)
    var
        MembershipPointsEntry: Record "MM Membership Points Entry";
        LoyaltyPointManagement: Codeunit "MM Loyalty Point Management";
        EarnRatio: Decimal;
    ...
    procedure InvoiceOneStorePoints(LoyaltyStoreSetup: Record "MM Loyalty Store Setup")
    var
        GenJournalBatch: Record "Gen. Journal Batch";
        GenJournalLine: Record "Gen. Journal Line";
        NoSeriesManagement: Codeunit NoSeriesManagement;
    ...
    local procedure InvoiceStoreWorker(JournalTemplateName: Code[20];JournalBatchName: Code[20];DocumentNo: Code[20];LoyaltyStoreSetup: Record "MM Loyalty Store Setup")
    var
        GenJournalLine: Record "Gen. Journal Line";
        NoSeriesManagement: Codeunit NoSeriesManagement;
        UntilDate: Date;
    ...
    begin
        ...
        case LoyaltyStoreSetup."Reconciliation Period" of
          LoyaltyStoreSetup."Reconciliation Period"::PREVIOUS_MONTH : UntilDate := CalcDate ('<-CM-1D>', Today);
          LoyaltyStoreSetup."Reconciliation Period"::TODAY : UntilDate := Today;
        end;
        ...
        MakeJournalLine (GenJournalLine,
          ...
          StrSubstNo ('Period ..%1, Earn: %2, Burn: %3', UntilDate, EarnPoints, BurnPoints),
    ...
    local procedure ReconcilePoints(LoyaltyStoreSetup: Record "MM Loyalty Store Setup";UntilDate: Date;ReconciliationReference: Code[20];var EarnPoints: Integer;var EarnAmount: Decimal;var BurnPoints: Integer;var BurnAmount: Decimal)
    var
        LoyaltySetup: Record "MM Loyalty Setup";
        LoyaltyLedgerEntry: Record "MM Loyalty Ledger Entry (Srvr)";
    begin
