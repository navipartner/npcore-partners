codeunit 6014558 "RP Data Join Buffer Mgt."
{
    ...
    trigger OnRun()
    var
        Buffer2: Record "RP Data Join Buffer" temporary;
        ParentDataItem: Record "RP Data Items";
    ...
    var
        Buffer: Record "RP Data Join Buffer" temporary;
        RecIDBuffer: Record "RP Data Join Record ID Buffer" temporary;
        FieldMap: Record "RP Data Join Field Map" temporary;
    ...
    procedure GetBuffer(var BufferOut: Record "RP Data Join Buffer" temporary)
    begin
    ...
    begin
        ...
        else if (UpperBound = 0) and (LowerBound > 0) then
          Buffer.SetFilter("Unique Record No.", '>=%1', LowerBound)
        else if (UpperBound > 0) then
    ...
    local procedure ProcessDataItem(ParentDataItem: Record "RP Data Items";var ParentRecRef: RecordRef) ContinueProcessing: Boolean
    var
    ...
    begin
        ...
        //+NPR5.50 [353588]
            ...
            if ChildDataItems.FindSet then repeat
    ...
    local procedure FillRecord(var RecRef: RecordRef;ParentDataItem: Record "RP Data Items")
    var
        FieldRef: FieldRef;
    begin
            ...
            if FieldMap."Data Item Field No." > 0 then begin
              ...
              if UpperCase(Format(FieldRef.Type)) = 'DECIMAL' then
                case DecimalRounding of
                  DecimalRounding::"2" : Buffer.Value := Format(FieldRef.Value,0,'<Precision,2:2><Standard Format,2>');
                  DecimalRounding::"3" : Buffer.Value := Format(FieldRef.Value,0,'<Precision,3:3><Standard Format,2>');
                  DecimalRounding::"4" : Buffer.Value := Format(FieldRef.Value,0,'<Precision,4:4><Standard Format,2>');
                  DecimalRounding::"5" : Buffer.Value := Format(FieldRef.Value,0,'<Precision,5:5><Standard Format,2>');
                end
              else
    ...
    local procedure SetupTableJoin(ParentRecord: Record "RP Data Items";ChildRecord: Record "RP Data Items";var ParentRecRef: RecordRef;var ChildRecRef: RecordRef)
    var
        DataItemLinks: Record "RP Data Item Links";
    ...
    begin
        ...
        if DataItemLinks.FindSet then repeat
    ...
    local procedure TestConstraint(DataItem: Record "RP Data Items";var RecRefIn: RecordRef) Result: Boolean
    var
        RecRef: RecordRef;
        FieldRef: FieldRef;
    ...
    begin
        ...
        if DataItemConstraint.FindSet then repeat
    ...
    local procedure TestDistinct(DataItem: Record "RP Data Items";var RecRefIn: RecordRef;var tmpDistinctValueList: Record "Retail List" temporary): Boolean
    var
        FieldRef: FieldRef;
    begin
        //-NPR5.50 [353588]
        ...
        case UpperCase(Format(ParentFieldRef.Class)) of
          'FLOWFIELD' :
            begin
              ...
              case LinkType of
                LinkType::"=" : ChildFieldRef.SetFilter('=%1', ParentFieldRef.Value);
                LinkType::">" : ChildFieldRef.SetFilter('>%1', ParentFieldRef.Value);
                LinkType::"<" : ChildFieldRef.SetFilter('<%1', ParentFieldRef.Value);
                LinkType::"<>" : ChildFieldRef.SetFilter('<>%1', ParentFieldRef.Value);
              end;
    ...
    local procedure SetFieldMapping(Template: Code[20];ParentDataItem: Record "RP Data Items")
    var
    ...
    local procedure ShouldProcessingContinue(DataItem: Record "RP Data Items";DataFound: Boolean): Boolean
    begin
