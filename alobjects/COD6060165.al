codeunit 6060165 "Event Plan. Line Grouping Mgt."
{
    ...
    begin
        ...
        JobPlanningLine.SetRange("Group Source Line No.",Rec."Line No.");
    ...
    var
        ...
        EventPlanLineBuffer: Record "Event Planning Line Buffer" temporary;
        Date: Record Date;
        EventPeriodDistrDlg: Page "Event Period Distr. Dialog";
        EventPlanLineBufferList: Page "Event Planning Line Buffer";
    ...
    begin
        ...
        if PAGE.RunModal(PAGE::"Event Planning Line Buffer",EventPlanLineBuffer) = ACTION::LookupOK then begin
    ...
    local procedure InsertBuffer(var JobPlanningLine: Record "Job Planning Line";LineNo: Integer;var EventPlanLineBuffer: Record "Event Planning Line Buffer")
    begin
        ...
        EventPlanLineBuffer."Starting Time" := JobPlanningLine."Starting Time";
        EventPlanLineBuffer."Ending Time" := JobPlanningLine."Ending Time";
    ...
    local procedure CheckCapAndTimeAvailability(var JobPlanningLine: Record "Job Planning Line";var EventPlanLineBuffer: Record "Event Planning Line Buffer")
    var
    ...
    begin
        ...
        if ReturnMsg <> '' then begin
          ...
          case OverCapacitateSetup of
            JobsSetup."Over Capacitate Resource"::Disallow:
              begin
            ...
            JobsSetup."Over Capacitate Resource"::Warn:
              begin
    ...
    procedure CheckCapAndTimeAvailabilityOnDemand(var EventPlanLineBuffer: Record "Event Planning Line Buffer";WithModify: Boolean)
    var
        JobPlanningLineTemp: Record "Job Planning Line" temporary;
    begin
        ...
        JobPlanningLineTemp."Starting Time" := EventPlanLineBuffer."Starting Time";
        JobPlanningLineTemp."Ending Time" := EventPlanLineBuffer."Ending Time";
    ...
    begin
        ...
        Rec.TestField("Starting Time",0T);
        Rec.TestField("Ending Time",0T);
        Rec.TestField(Quantity,0);
        Rec.TestField("Qty. Invoiced",0);

        if Rec."Group Line" then begin
          JobPlanningLine.SetRange("Job No.",Rec."Job No.");
          JobPlanningLine.SetRange("Job Task No.",Rec."Job Task No.");
          JobPlanningLine.SetRange("Group Source Line No.",Rec."Line No.");
    ...
    local procedure CreateJobPlanningLineFromBuffer(var Rec: Record "Job Planning Line";var EventPlanLineBuffer: Record "Event Planning Line Buffer")
    var
        Job: Record Job;
          ...
          repeat
            ...
            if EventPlanLineBuffer.FindSet then
              repeat
                ...
                JobPlanningLine2."Skip Cap./Avail. Check" := EventPlanLineBuffer."Status Checked";
                JobPlanningLine2."Group Source Line No." := Rec."Line No.";
                JobPlanningLine2."Group Line" := true;
                ...
                JobPlanningLine2.Validate("Starting Time",EventPlanLineBuffer."Starting Time");
                JobPlanningLine2.Validate("Ending Time",EventPlanLineBuffer."Ending Time");
                JobPlanningLine2.Validate(Quantity,EventPlanLineBuffer.Quantity);
                JobPlanningLine2."Skip Cap./Avail. Check" := false;
