codeunit 6014540 "RP Escape Code Library"
{
    // Escape Code Library.
    ...
    begin
        ...
        for Itt := 1 to String.CountOccurences(' ') + 1 do begin
          ...
          case Token of
            'NUL'   : Text += Format(cNUL);
            '01'    : Text += Format(c01);
            '02'    : Text += Format(c02);
            '03'    : Text += Format(c03);
            'EOT'   : Text += Format(cEOT);
            'ENQ'   : Text += Format(cENQ);
            'ACK'   : Text += Format(cACK);
            '07'    : Text += Format(c07);
            '08'    : Text += Format(c08);
            'HT'    : Text += Format(cHT);
            'LF'    : Text += Format(cLF);
            ...
            '15'    : Text += Format(c15);
            '16'    : Text += Format(c16);
            'DLE'   : Text += Format(cDLE);
            'XON'   : Text += Format(cXON);
            'XOFF'  : Text += Format(cXOFF);
            'DC4'   : Text += Format(cDC4);
            'NAK'   : Text += Format(cNAK);
            '22'    : Text += Format(c22);
            '23'    : Text += Format(c23);
            'CAN'   : Text += Format(cCAN);
            '25'    : Text += Format(c25);
            '26'    : Text += Format(c26);
            'ESC'   : Text += Format(cESC);
            'FS'    : Text += Format(cFS);
            'GS'    : Text += Format(cGS);
            'RS'    : Text += Format(cRS);
            '31'    : Text += Format(c31);
    ...
    begin
        ...
        for Itt := 1 to String.CountOccurences(' ') + 1 do begin
          Token := String.SelectStringSep(Itt,' ');
          case Token of
            'NUL'   : ReturnSequence +=  NUL;
            '01'    : ReturnSequence += "01";
            '02'    : ReturnSequence += "02";
            '03'    : ReturnSequence += "03";
            'EOT'   : ReturnSequence +=  EOT;
            'ENQ'   : ReturnSequence +=  ENQ;
            'ACK'   : ReturnSequence +=  ACK;
            '07'    : ReturnSequence += "07";
            '08'    : ReturnSequence += "08";
            'HT'    : ReturnSequence +=  HT;
            'LF'    : ReturnSequence +=  LF;
            '11'    : ReturnSequence += "11";
            'FF'    : ReturnSequence +=  FF;
            'CR'    : ReturnSequence +=  CR;
            '14'    : ReturnSequence += "14";
            '15'    : ReturnSequence += "15";
            '16'    : ReturnSequence += "16";
            'DLE'   : ReturnSequence +=  DLE;
            'XON'   : ReturnSequence +=  XON;
            'XOFF'  : ReturnSequence +=  XOFF;
            'DC4'   : ReturnSequence +=  DC4;
            'NAK'   : ReturnSequence +=  NAK;
            '22'    : ReturnSequence += "22";
            '23'    : ReturnSequence += "23";
            'CAN'   : ReturnSequence +=  CAN;
            '25'    : ReturnSequence += "25";
            '26'    : ReturnSequence += "26";
            'ESC'   : ReturnSequence +=  ESC;
            'FS'    : ReturnSequence +=  FS;
            'GS'    : ReturnSequence +=  GS;
            'RS'    : ReturnSequence +=  RS;
            '31'    : ReturnSequence += "31";
            'SP'    : ReturnSequence +=  SP;
          else
