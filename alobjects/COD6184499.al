codeunit 6184499 "EFT Framework Mgt."
{
    ...
    begin
        ...
        EFTTransactionRequest."Processing Type" := EFTTransactionRequest."Processing Type"::xLookup;
        EFTInterface.OnCreateLookupTransactionRequest(EFTTransactionRequest, Handled);
        CheckHandled(EFTSetup."EFT Integration Type", EFTTransactionRequest, Format(EFTTransactionRequest."Processing Type"::xLookup), Handled);
    end;
    ...
    begin
        if not POSSession.IsActiveSession(POSFrontEnd) then
            Error(ERROR_OUTSIDE_POS, Format(EFTTransactionRequest."Processing Type"::xLookup));
        POSFrontEnd.GetSession(POSSession);
        POSSession.GetSale(POSSale);
        POSSale.GetCurrentSale(SalePOS);
        if SalePOS."Register No." <> EFTTransactionRequest."Register No." then
            Error(ERROR_SAME_POS, Format(EFTTransactionRequest."Processing Type"::xLookup));
