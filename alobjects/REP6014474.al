report 6014474 "Item/Sales Person Top"
{
    ...
    dataset
    {
        dataitem(Item;Item)
        {
            ...
            dataitem("Salesperson/Purchaser";"Salesperson/Purchaser")
            {
                ...
                DataItemLink = "Item Filter"=FIELD("No.");
                ...
                column(SalesQty_SalespersonPurchaser;"Salesperson/Purchaser"."Sales (Qty.)")
                {
                }

                trigger OnAfterGetRecord()
                begin
                    if "Sales (LCY)" = 0 then CurrReport.Skip;

                    if Sorting then begin
                      ItemAmount."Item No.":=Code;

                      case SortBy of
                        SortBy::Turnover : begin
                          ItemAmount.Amount := Abs("Sales (LCY)");
                          ItemAmount."Amount 2" := Abs("Sales (Qty.)");
                        end;
                        SortBy::"Qty." : begin
                          ItemAmount.Amount := Abs("Sales (Qty.)");
                          ItemAmount."Amount 2" := Abs("Sales (LCY)");
                        end;
                      end;
                      ItemAmount.Insert;
                    end;
    ...
    begin
        ...
        if Sorting then begin
          ...
          case Key of
            Key::stigende : ItemAmount.Ascending(false);
            Key::faldende : ItemAmount.Ascending(true);
          end;
        end;
    end;
