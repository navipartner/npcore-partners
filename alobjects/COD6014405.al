codeunit 6014405 "Create Item Group Structure"
{
    ...
    begin
        ...
        if Rec.IsTemporary then
          exit;
        if Rec."Table ID" <> DATABASE::"Item Group" then
          exit;
    ...
    local procedure OnInsertItemGroup(var Rec: Record "Item Group";RunTrigger: Boolean)
    begin
        //-NPR5.30 [262977]
    ...
    local procedure OnModifyItemGroup(var Rec: Record "Item Group";var xRec: Record "Item Group";RunTrigger: Boolean)
    begin
        //-NPR5.30 [262977]
        if Rec.IsTemporary then
          exit;
    ...
    begin
        ...
        if DefaultDimension.IsTemporary then
          exit;
        if DefaultDimension."Table ID" <> DATABASE::"Item Group" then
          exit;
        ...
        repeat
          if DeleteDimension then begin
            if DefaultDimension2.Get(DATABASE::"Item Group",ItemGroup."No.",DefaultDimension."Dimension Code") then
              DefaultDimension2.Delete(true);
          end else begin
            if not DefaultDimension2.Get(DATABASE::"Item Group",ItemGroup."No.",DefaultDimension."Dimension Code") then begin
    ...
    local procedure CopyParentItemGroupDimensions(ItemGroup: Record "Item Group";"Query": Boolean)
    var
    ...
    local procedure CopyParentItemGroupSetups(ItemGroup: Record "Item Group";"Query": Boolean)
    var
