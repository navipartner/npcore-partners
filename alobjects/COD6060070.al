codeunit 6060070 "Pre-map Incoming Item Lines"
{
    ...
    begin
        with IntermediateDataImport do begin
          ...
          //  EXIT(FALSE);
          //+NPR5.29 [264077]
          if FindEntry(EntryNo,DATABASE::"Purchase Line",PurchaseLine.FieldNo(Size),HeaderNo,RecordNo) then
            Size := Value;
          if FindEntry(EntryNo,DATABASE::"Purchase Line",PurchaseLine.FieldNo(Color),HeaderNo,RecordNo) then
            Color := Value;
          ...
          if ItemVariant.IsEmpty then
            exit(false);
          if StrPos(Item."Variety 1",'SIZE') <> 0 then
            ItemVariant.SetRange("Variety 1 Value",Size)
          else
            if StrPos(Item."Variety 1",'COLO') <> 0 then
              ItemVariant.SetRange("Variety 1 Value",Color);
          if StrPos(Item."Variety 2",'SIZE') <> 0 then
            ItemVariant.SetRange("Variety 2 Value",Size)
          else
            if StrPos(Item."Variety 2",'COLO') <> 0 then
              ItemVariant.SetRange("Variety 2 Value",Color);
          if StrPos(Item."Variety 3",'SIZE') <> 0 then
            ItemVariant.SetRange("Variety 3 Value",Size)
          else
            if StrPos(Item."Variety 3",'COLO') <> 0 then
              ItemVariant.SetRange("Variety 3 Value",Color);
          if StrPos(Item."Variety 4",'SIZE') <> 0 then
            ItemVariant.SetRange("Variety 4 Value",Size)
          else
            if StrPos(Item."Variety 4",'COLO') <> 0 then
              ItemVariant.SetRange("Variety 4 Value",Color);
    ...
    var
        ...
        ItemWkshtDocExchange: Codeunit "Item Wksht. Doc. Exchange";
    ...
    begin
        ...
        with IntermediateDataImport do begin
          if FindEntry(EntryNo,DATABASE::"Purchase Line",PurchaseLine.FieldNo("Cross-Reference No."),HeaderRecordNo,RecordNo) then begin
            ...
            IntermediateDataImport2.SetRange("Table ID",DATABASE::"Item Worksheet Line");
            IntermediateDataImport2.SetRange("Parent Record No.",HeaderRecordNo);
            IntermediateDataImport2.SetRange("Record No.",RecordNo);
            if IntermediateDataImport2.FindSet then repeat
    ...
    local procedure CreateVariantFromSizeAndColor(EntryNo: Integer;HeaderNo: Integer;RecordNo: Integer;ItemWorksheetLine: Record "Item Worksheet Line"): Boolean
    var
        IntermediateDataImport: Record "Intermediate Data Import";
        PurchaseLine: Record "Purchase Line";
        Item: Record Item;
