codeunit 6060144 "MM Member Limitation Mgr."
{
        ...
        if (RuleNo <> 0) then begin
          ...
          if (ClientType = ClientType::POS) then begin
            case MembershipLimitationSetup."POS Response Action" of
              MembershipLimitationSetup."POS Response Action"::USER_ERROR : asserterror Error (''); // This will rollback our successful transaction - whatever it did!!
        ...
        if (RuleNo <> 0) then begin
          ...
          if (ClientType = ClientType::POS) then begin
            case MembershipLimitationSetup."POS Response Action" of
              ...
              MembershipLimitationSetup."POS Response Action"::USER_ERROR : Commit;
              MembershipLimitationSetup."POS Response Action"::USER_CONFIRM : Commit;
            end;
          end;
          ...
          if (MembershipLimitationSetup.FindSet ()) then begin
            repeat
              ...
              if (LimitFound) then begin
                ...
                case ClientType of
                  ClientType::WS  : ResponseMessage := MembershipLimitationSetup."WS Deny Message";
                  ClientType::POS : ResponseMessage := MembershipLimitationSetup."POS Response Message"
    ...
    begin
        ...
        with MembershipLimitationSetup do begin
          ...
          case "Constraint Source" of
            "Constraint Source"::MEMBER :     MemberArrivalLogEntry.SetFilter ("External Member No.", '=%1', CopyStr (KeyValue, 1, MaxStrLen (MemberArrivalLogEntry."External Member No.")));
            "Constraint Source"::MEMBERCARD : MemberArrivalLogEntry.SetFilter ("External Card No.", '=%1', CopyStr (KeyValue, 1, MaxStrLen (MemberArrivalLogEntry."External Card No.")));
            "Constraint Source"::MEMBERSHIP : MemberArrivalLogEntry.SetFilter ("External Membership No.", '=%1', CopyStr (KeyValue, 1, MaxStrLen (MemberArrivalLogEntry."External Membership No.")));
            ...
            "Constraint Source"::GDPR_REJECTED : exit (MemberHasRejectedGDPRRequest (CopyStr (KeyValue, 1, MaxStrLen (MemberArrivalLogEntry."External Card No."))));
            //+MM1.29 [313795]
          end;

          case "Event Type" of
            "Event Type"::SUCCESS_COUNT : MemberArrivalLogEntry.SetFilter ("Response Type", '=%1', MemberArrivalLogEntry."Response Type"::SUCCESS);
            "Event Type"::FAIL_COUNT :    MemberArrivalLogEntry.SetFilter ("Response Type", '<>%1', MemberArrivalLogEntry."Response Type"::SUCCESS);
          end;
          ...
          if (MemberArrivalLogEntry.FindFirst()) then begin
            case "Constraint Type" of
              "Constraint Type"::RELATIVE_TIME : ContraintText := StrSubstNo ('%1', MembershipLimitationSetup."Constraint Seconds" - Round ((CurrentDateTime - MemberArrivalLogEntry."Created At") / 1000,1));
              "Constraint Type"::FIXED_TIME    : ContraintText := StrSubstNo ('%1 - %', MemberArrivalLogEntry."Local Date", MemberArrivalLogEntry."Local Time");
              "Constraint Type"::DATEFORMULA   : ContraintText := StrSubstNo ('%1 - %', MemberArrivalLogEntry."Local Date", MemberArrivalLogEntry."Local Time");
             end;
          end else begin
            case "Constraint Type" of
              "Constraint Type"::RELATIVE_TIME : ContraintText := StrSubstNo ('%1', MembershipLimitationSetup."Constraint Seconds");
              "Constraint Type"::FIXED_TIME    : ContraintText := StrSubstNo ('%1 - %2', "Constraint From Time", "Constraint Until Time");
              "Constraint Type"::DATEFORMULA   : ContraintText := StrSubstNo ('%1', Today);
            end;
          end;

          if ("Event Limit" = 0) then begin
            case "Constraint Type" of
              "Constraint Type"::RELATIVE_TIME : exit (true);
              "Constraint Type"::FIXED_TIME    : exit ((Time > "Constraint From Time") and (Time < "Constraint Until Time"));
              "Constraint Type"::DATEFORMULA   : exit (CalcDate ("Constraint Dateformula", Today) = Today);
            end;
          end;
