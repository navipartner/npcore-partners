codeunit 6014437 "Phone Lookup"
{
    ...
    procedure RunPhoneLookup(PhoneNo: Text[30];var PhoneLookupBuffer: Record "Phone Lookup Buffer" temporary): Boolean
    begin
        //-NPR5.26 [252881]
        ...
        exit(PAGE.RunModal(PAGE::"Phone Number Lookup",PhoneLookupBuffer) = ACTION::LookupOK);
        //+NPR5.26 [252881]
    end;
    ...
    var
        ...
        PhoneLookupBuffer: Record "Phone Lookup Buffer" temporary;
    begin
        if xRec."No." <> '' then
          exit;
        if (Rec."No." = '') then
    ...
    [EventSubscriber(ObjectType::Page, 5052, 'OnAfterActionEvent', 'PhoneLookup', false, false)]
    local procedure ContactListOnAfterAction(var Rec: Record Contact)
    var
        PhoneNoLookup: Page "Phone No lookup";
    ...
    [EventSubscriber(ObjectType::Page, 22, 'OnAfterActionEvent', 'PhoneLookup', false, false)]
    ...
    [EventSubscriber(ObjectType::Page, 27, 'OnAfterActionEvent', 'PhoneLookup', false, false)]
    ...
    local procedure MemberInfoCapOnValidatePhone(var Rec: Record "MM Member Info Capture";var xRec: Record "MM Member Info Capture";CurrFieldNo: Integer)
    var
        PhoneLookupBuffer: Record "Phone Lookup Buffer" temporary;
    begin
        //-NPR5.31 [268879]
    ...
    procedure UpdateCont(var Cont: Record Contact;PhoneLookupBuf: Record "Phone Lookup Buffer" temporary)
    begin
        //-NPR5.26 [252881]
        with Cont do begin
    ...
    procedure UpdateCust(var Cust: Record Customer;PhoneLookupBuf: Record "Phone Lookup Buffer" temporary)
    begin
        //-NPR5.23
        with Cust do begin
          Validate(Name, CopyStr(PhoneLookupBuf.Name, 1, MaxStrLen(Name)));
    ...
    procedure UpdateVend(var Vend: Record Vendor;PhoneLookupBuf: Record "Phone Lookup Buffer" temporary)
    begin
        //-NPR5.25 [247278]
        with Vend do begin
          Validate(Name, CopyStr(PhoneLookupBuf.Name, 1, MaxStrLen(Name)));
    ...
    procedure UpdateMemberInfoCapture(var MMMemberInfoCapture: Record "MM Member Info Capture";PhoneLookupBuf: Record "Phone Lookup Buffer" temporary)
    begin
        //-NPR5.31 [268879]
    ...
    procedure Creation(var TDCNamesNumbersBuffer: Record "Phone Lookup Buffer" temporary)
    begin
        //-NPR5.23
    ...
    local procedure CreateCustomer(var TDCNamesNumbersBuffer: Record "Phone Lookup Buffer" temporary)
    var
    ...
    local procedure CreateContact(TDCNamesNumbersBuffer: Record "Phone Lookup Buffer" temporary)
    var
    ...
    local procedure CreateContactBusinessRel(TDCNamesNumbersBuffer: Record "Phone Lookup Buffer" temporary;LinkToTable: Option " ",Customer,Vendor,"Bank Account")
    var
    ...
    local procedure CreateVendor(TDCNamesNumbersBuffer: Record "Phone Lookup Buffer" temporary)
    var
