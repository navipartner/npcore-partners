table 6059902 "Task Line"
{
    ...
    fields
    {
        ...
        field(9;Enabled;Boolean)
        {
            ...
            var
                ...
                RunTask: Codeunit "Task Queue Processor";
        ...
        field(20;Recurrence;Option)
        {
            ...
            begin
                case Recurrence of
                  ...
                  Recurrence::Hourly: "Recurrence Interval" := 1000*60*60;
                  Recurrence::Daily:  "Recurrence Interval" := 1000*60*60*24;
                  Recurrence::Weekly: "Recurrence Interval" := 1000*60*60*24*7;
                end;

                case Recurrence of
                  Recurrence::" ":    "Recurrence Calc. Interval" := 0;
                  Recurrence::Hourly: "Recurrence Calc. Interval" := 1000*60;
                  Recurrence::Daily:  "Recurrence Calc. Interval" := 1000*60*60;
                  Recurrence::Weekly: "Recurrence Calc. Interval" := 1000*60*60*24;
                  Recurrence::Custom: "Recurrence Calc. Interval" := 1000*60;
                end;
            end;
        }
        ...
        {
            CalcFormula = Count("Task Line Parameters" WHERE ("Journal Template Name"=FIELD("Journal Template Name"),
    ...
    begin
        ...
        if TaskLog.FindSet(true, true) then repeat
        ...
        if TaskOutputLog.FindSet(true, true) then repeat
    ...
    procedure SetUpNewLine(LastTaskLine: Record "Task Line")
    var
    ...
    begin
        ...
        else if TaskTemplate.Get("Journal Template Name") and (TaskTemplate."Task Worker Group" <> '') then
    ...
    begin
        ...
        if TaskLog.FindFirst then repeat
    ...
    var
        TaskQueueAdd2Log: Codeunit "Task Queue - Single Instance";
    begin
        //-TQ1.29
    ...
    var
        TaskQueueExecute: Codeunit "Task Queue Execute";
    begin
        TestField("File Path");
        TestField("File Name");
    ...
    begin
        ...
        case Date2DWY(Today,1) of
          1: if not "Run on Monday" then exit(false);
          2: if not "Run on Tuesday" then exit(false);
          3: if not "Run on Wednesday" then exit(false);
          4: if not "Run on Thursday" then exit(false);
          5: if not "Run on Friday" then exit(false);
          6: if not "Run on Saturday" then exit(false);
          7: if not "Run on Sunday" then exit(false);
        end;
