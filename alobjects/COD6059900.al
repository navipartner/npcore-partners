codeunit 6059900 "Task Jnl. Management"
{
    ...
    procedure TemplateSelection(PageID: Integer;PageTemplate: Option General,NaviPartner;var TaskJnlLine: Record "Task Line";var JnlSelected: Boolean)
    var
        TaskJnlTemplate: Record "Task Template";
    begin
        JnlSelected := true;
    ...
    procedure OpenJnl(var CurrentJnlBatchName: Code[10];var TaskJnlLine: Record "Task Line")
    begin
    ...
    var
        TaskJnlBatch: Record "Task Batch";
    begin
    ...
    procedure CheckName(CurrentJnlBatchName: Code[10];var TaskJnlLine: Record "Task Line")
    var
        TaskJnlBatch: Record "Task Batch";
    begin
    ...
    procedure SetName(CurrentJnlBatchName: Code[10];var TaskJnlLine: Record "Task Line")
    begin
        TaskJnlLine.FilterGroup := 2;
        TaskJnlLine.SetRange("Journal Batch Name",CurrentJnlBatchName);
        TaskJnlLine.FilterGroup := 0;
    ...
    procedure LookupName(var CurrentJnlBatchName: Code[10];var TaskJnlLine: Record "Task Line"): Boolean
    var
        TaskJnlBatch: Record "Task Batch";
    begin
        Commit;
    ...
    procedure GetNames(var TaskJnlLine: Record "Task Line";var JobDescription: Text[50];var AccName: Text[50])
    begin
        /*
    ...
    procedure SyncroniseCompanies(TaskLine: Record "Task Line";ChangeType: Option Insert,Modify,Delete,Rename)
    var
        TaskBatch: Record "Task Batch";
        Comp: Record Company;
        TaskBatch2: Record "Task Batch";
        TaskLine2: Record "Task Line";
        SyncNextExecuteTime: Boolean;
        SyncParms: Boolean;
        TaskQueue: Record "Task Queue";
        TaskQueue2: Record "Task Queue";
        TaskLog2: Record "Task Log (Task)";
        TaskOutputLog2: Record "Task Output Log";
        TaskLineParm2: Record "Task Line Parameters";
            ...
            case ChangeType of
              ...
              ChangeType::Modify:
                begin
                  ...
                  if SyncParms then begin
                    ...
                    if TaskLineParm.FindSet then repeat
              ...
              ChangeType::Delete:
                begin
                  ...
                  if TaskLog2.FindSet(true, true) then repeat
                  ...
                  if TaskOutputLog2.FindSet(true, true) then repeat
    ...
    procedure SetupCommonBatch(TaskBatch: Record "Task Batch")
    var
        TMPConfigSel: Record "Config. Selection" temporary;
        Comp: Record Company;
        TaskBatch2: Record "Task Batch";
        TaskTemplate2: Record "Task Template";
        NASGroup2: Record "Task Worker Group";
        NASGroup: Record "Task Worker Group";
    ...
    begin
        ...
        if TMPConfigSel.FindSet then repeat
          ...
          if TaskLine.FindSet(false, false) then repeat
