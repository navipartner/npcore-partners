codeunit 6014630 "Touch - Sale POS (Web)"
{
    ...
    // NPR5.54/TJ  /20200302 CASE 393478 Recoding for removed field "Item remarks"
    ...
    begin
        ...
        with This do begin
          ...
          //+NPR5.20
            ...
            case FuncStr of
               /*----- LOGIN -----*/
               'PRINT_LAST_RECEIPT'    :    TouchScreenFunctions.PrintLastReceipt(This,0);
               'PRINT_LAST_RECEIPT_A4' :    TouchScreenFunctions.PrintLastReceipt(This,1);
               'PRINT_LAST_RECEIPT_DEBIT' : TouchScreenFunctions.PrintLastReceipt(This,2);
               'PRINT_LAST'            :    TouchScreenFunctions.PrintLastReceipt(This, 3);
               'REGISTER_LOCK'         : begin
                                           SetScreenView(ViewType.Locked);
                                         end;
               'REGISTER_CHANGE'       : begin
                                           PushRegisterChange();
                                           exit;
                                         end;
               'RECEIPT2NPORDER'       : RetailFormCode.ReceiptToRetailOrder(This, 0, true,"Retail Document Type"::"Retail Order");
               'RECEIPT2CUSTIMIZATION' : RetailFormCode.ReceiptToRetailOrder(This, 0, true,"Retail Document Type"::Customization);
               'SHIPMENT2NPORDER'      : RetailFormCode.ShipmentToRetailOrder(This, true);
               ...
               'FUNCTIONS_SALE'        : Functions('');
               'FUNCTIONS_DISCOUNT'    : begin
              ...
              'TYPE_SALE_RETURN'      : ReverseQtyOnSaleLines(This);
              'OUT_PAYMENT'           : if PAGE.RunModal(PAGE::"Touch Screen - G/L Accounts", Account) = ACTION::LookupOK then begin
              ...
              'ITEM_LEDGERENTRIES'    : begin
               ...
               /* Retail Quote */
               'QUOTE_SEND',
               'QUOTE_GET'             : RetailDocumentHandling(FuncStr);
               ...
               'SCANNER_GET_SALE'      : cuSkanner.initSale( This );
        
               //-#TM1.09
               'TM_SCAN_TICKET'        : begin
               ...
               'MM_SCAN_CARD'          : begin
                                           ...
                                           case (MM_PrePushResult) of
                                             0 : ; // do nothing
                                             1 : EnterPush (); // Uses value from validering
                                           else
                                           ...
                                           case (MM_PostPushResult) of
                                             0 : ; // do nothing
                                             1 : EnterHit (MM_PushAction); // Uses value from validering
                                           else
               ...
               'SERIAL_NUMBER_ARB'     : begin
                                           SaleLinePOSObject.GETRECORD(SaleLinePOS);
                                           exit(TouchScreenFunctions.SetSerialNumber(SaleLinePOS, true));
                                         end;
               'COMMENT_INSERT'        : begin
                                           Validering := '*';
                                           ButtonDefault;
                                         end;
               'COMMENT_EDIT'          : RetailFormCode.EditComments( This );
               'REPAIR_GET'            : begin
                                           RetailFormCode.FetchRepair( This );
                                           Validering := "Customer No.";
                                           case "Customer Type" of
                                             "Customer Type"::Ord : SetCustomer();
                                             "Customer Type"::Cash : SetContact();
                                           end;
                                         end;
               'RETURN_SALE'           : TouchScreenFunctions.ReturnSale( This );
               'ITEM_INVENTORY'        : begin
                                           SaleLinePOSObject.GETRECORD(SaleLinePOS);
              ...
              'TERMINAL_UNLOCK'       : TouchScreenFunctions.CallTerminal(This, 'UNLOCK',0);
              //+NPR5.20
              //-NPR5.22
              'TERMINAL_OFFLINE'      : begin
                                          Register.TestField("Register No.");
                                          This."Register No." := Register."Register No.";
                                          if not Evaluate (Int,MenuLines1.Parametre) then
                                            Marshaller.DisplayError(Text10600200,Text10600215,true);
              ...
              'CUSTOMER_ILE'          : TouchScreenFunctions.ItemLedgerEntries( This."Customer Type", This."Customer No." );
              'CUSTOMER_CRM'          : SetContact();
              'CUSTOMER_STD'          : begin
                                          SetCustomer();
                                          //-NPR4.14
                                          Commit;
              ...
              /*----- SALE - DISCOUNT -----*/
              'LINE_AMOUNT'           : SetLineAmount(0);
              'LINE_UNITPRICE'        : begin
                                          PushLineUnitPrice();
                                          exit;
                                        end;
              'TOTAL_AMOUNT'          : begin
              ...
              'TOTAL_DISCOUNTPCT_ABS' : begin
                                          PushDiscountPct(false);
                                          exit;
                                        end;
              'TOTAL_DISCOUNTPCT_REL' : begin
                                          PushDiscountPct(true);
                                          exit;
                                        end;
              'DISCOUNTPCT_CR'        : begin
                                          SaleLinePOSObject.GETRECORD(SaleLinePOS);
                                          TouchScreenFunctions.DiscountCR( SaleLinePOS );
                                        end;
              'LINE_DISCOUNTPCT_ABS'  : begin
                                          PushLineDiscountPct(false);
                                          exit;
                                        end;
              'LINE_DISCOUNTPCT_REL'  : begin
                                          PushLineDiscountPct(true);
                                          exit;
                                        end;
              'LINE_DISCOUNT_BLOCK'   : begin
                                          SaleLinePOSObject.GETRECORD(SaleLinePOS);
                                          TouchScreenFunctions.DiscountBlockLine(SaleLinePOS);
                                        end;
              'LINE_DISCOUNT_AMOUNT'  : begin
                                          PushLineDiscountAmount();
                                          exit;
                                        end;
              'SALE_INIT'             : SaleInit(true,true);
               //-NPR5.32 [270885]
              'SALE_INIT_ORDER'       : begin
              ...
              //+NPR5.32 [270885]
              //-NPR5.32 [274462]
              'TOTAL_DISCOUNTPCT_VAR' : begin
                                          PushTotalDiscountPctVar();
                                          exit;
                                        end;
              'LINE_DISCOUNTPCT_VAR'  : begin
             ...
             'PRINT_ITEM_LABEL'         : begin
                                            SaleLinePOSObject.GETRECORD(SaleLinePOS);
                                            TouchScreenFunctions.PrintLabel(SaleLinePOS);
                                          end;
             'PRINT_ITEM_LABEL_ALL'     : begin
                                            SaleLinePOSObject.GETRECORD(SaleLinePOS);
              ...
              'PAYMENT_GIFTVOUCHER'   : PaymentType(PaymentTypePOS."Processing Type"::"Gift Voucher", '');
              'PAYMENT_CREDITVOUCHER' : PaymentType(PaymentTypePOS."Processing Type"::"Credit Voucher", '');
              'CREDITVOUCHER_CREATE'  : begin
                                          CreateCreditVoucher();
              ...
              //-NPR4.02
              'HIDE_INPUT_DIALOG'     : begin
                                          HideInputDialog := true;
                                        end;
              'SHOW_INPUT_DIALOG'     : begin
              ...
              /*----- GENERAL -----*/
              'ENTERPUSH'             : EnterPush;
              'FUNCTIONS_ITEM'        : Functions('ITEMFUNCTIONS');
              'SALE_QUIT'             : QueryClose;
              'REGISTER_OPEN'         : begin
        ...
        //+NPR5.28
               ...
               'INFO'                 : ShowLineInformation();
               'ZOOM'                 : ShowSaleLineDetails();
               //-NPR5.27
               //'DIMS_SALE'            : ShowDimensions;
               'DIMS_SALE'            : This.ShowDocDim;
              //+NPR5.27
               'TAX_FREE'             : begin
                                          //-NPR4.21
               ...
               'TERMINAL_PAY'           : //-NPR4.11
                                          ...
                                          //+NPR4.11
              //-NPR5.22
              'CUST_LOCATION_EXPORT'    : if POSCustLocMgt.SaveSaleToLoc(MenuLines1.Parametre, This) then begin
                                            NewSale();
              ...
              'CUST_LOCATION_IMPORT'    : if POSCustLocMgt.GetSaleFromLoc(MenuLines1.Parametre, This, POSCustLocReceipt) then
                                            LoadSavedSale(POSCustLocReceipt);
              'CUST_LOCATION_LIST'      : POSCustLocMgt.List(false, true);
              'CUST_LOCATION_PRINT'     : POSCustLocMgt.Print(0,This,MenuLines1.Parametre);
              //-NPR5.31 [264112]
              'CUST_LOCATION_STAMP_IMPORT' : if POSCustLocMgt.StampSaleAndGetFromLoc(MenuLines1.Parametre, This, POSCustLocReceipt) then
                                               LoadSavedSale(POSCustLocReceipt);
              //+NPR5.31 [264112]
              //-NPR5.26 OSFI
              ...
              //+NPR5.26 OSFI
                ...
                if RetailSalesDocMgt.Run(This) or (GetLastErrorText <> '') then begin
                  if (GetLastErrorText <> '') then
                    ...
                    //+NPR5.38 [300126]
                end else if RetailSalesDocImpMgt.Run(This) or (GetLastErrorText <> '') then begin
                  if (GetLastErrorText <> '') then
                    //-NPR5.38 [300126]
                    //ERROR(GETLASTERRORTEXT);
                    Message(GetLastErrorText);
        ...
        with This do begin
          ...
          if CopyStr(Validering,1,1) in ['�', '$', '?'] then begin
            ...
            case UpperCase(CopyValidering) of
              'USERID'     : Validering := UserId;
              'R'          : Validering := ValideringLast;
              'OBJECTID'   : begin
                               Error('');
                             end;
              'VER'        :
    ...
    begin
        ...
        with This do begin
          ...
          //+NPR5.20
            ...
            if SaleLinePOSTest.FindSet then repeat
          ...
          if (str = 'EKSPEDITION') then begin
            ...
            if (strpos1 < strlen1) and (strlen1 > 1) and (strpos1 > 0) then begin
              ...
              if CopyValidering <> '' then begin
                ...
                end else if (MenuLines1.Parametre <> '') and UI.TryParseDecimal(Qty,MenuLines1.Parametre) then begin
                ...
                if (SaleLinePOS.Type = SaleLinePOS.Type::Item) then begin
                  if ItemComment.Get(SaleLinePOS."No.") then begin
                    ...
                    if ItemComment.Comment then begin
                    //+NPR5.54 [393478]
                      Commit;
                    end;
                  end;
            ...
            */
            ...
            if CopyValidering <> '' then begin
              ...
              if not SaleLinePOSObject.InsertItemLine(CopyValidering, This) then begin
                 SaleLinePOSObject.SetValidation('');
                 Marshaller.Error_Protocol(Text10600200, StrSubstNo(Text10600107, CopyValidering),true);
              end else if MenuLines1.Parametre <> '' then begin
    ...
    begin
        ...
        if theseFunctions = '' then begin
          case true of
            State.IsCurrentView(ViewType.Login):   theseFunctions := 'LOGIN';
            State.IsCurrentView(ViewType.Sale):    theseFunctions := 'EKSPEDITION';
            State.IsCurrentView(ViewType.Payment): theseFunctions := 'PAYMENT';
          end;
        end;

        case theseFunctions of
            'LOGIN'         : Heading := t001;
            'EKSPEDITION'   : Heading := t003;
            'PAYMENT'       : Heading := t002;
            'USERS'         : Heading := t004;
            'ITEM'          : Heading := t005;
            'BETALINGSVALG' : Heading := t006;
            'FORSIKRING'    : Heading := t008;
            'KEYBOARD'      : Heading := t009;
            'DISCOUNT'      : Heading := t010;
            'PRINTS'        : Heading := t011;
            'CUSTOMER'      : Heading := t012;
            'ITEMFUNCTIONS' : Heading := t013;
            'DEBITPRINT'    : Heading := t007;
            'DEBITSALEPOST' : ;
        end;
        Heading := '  ' + Heading;
        ...
        MenuLine.Reset;
        case theseFunctions of
            'LOGIN'           : MenuLine.SetRange(Type,MenuLine.Type::Login);
            'EKSPEDITION'     : MenuLine.SetRange(Type,MenuLine.Type::"Sale Functions");
            'PAYMENT'         : MenuLine.SetRange(Type,MenuLine.Type::"Payment Functions");
            'KASSEAFSLUTNING' : MenuLine.SetRange(Type,MenuLine.Type::"Sale Form");
            'USERS'           : MenuLine.SetRange(Type,MenuLine.Type::User);
            'ITEM'            : MenuLine.SetRange(Type,MenuLine.Type::Item);
            'BETALINGSVALG'   : MenuLine.SetRange(Type,MenuLine.Type::PaymentType);
            'CUSTOMER'        : MenuLine.SetRange(Type,MenuLine.Type::"Customer Functions");
            'FINANS'          : MenuLine.SetRange(Type,MenuLine.Type::"G/L Account");
            'FORSIKRING'      : MenuLine.SetRange(Type,MenuLine.Type::Insurance);
            'KEYBOARD'        : MenuLine.SetRange(Type,MenuLine.Type::Keyboard);
            'DISCOUNT'        : MenuLine.SetRange(Type,MenuLine.Type::Discount);
            'PRINTS'          : MenuLine.SetRange(Type,MenuLine.Type::Prints);
            'REPORTS'         : MenuLine.SetRange(Type,MenuLine.Type::Reports);
            'ITEMFUNCTIONS'   : MenuLine.SetRange(Type,MenuLine.Type::"Item Functions");
            'DEBITPRINT'      : ;
            'DEBTISALEPOST'   : ;
            else
    ...
    begin
        ...
        with This do begin
          ...
          case RetailSetup."Show saved expeditions" of
            ...
            RetailSetup."Show saved expeditions"::Register  : Ekspeditionshoved.SetRange("Register No.", "Register No.");
            RetailSetup."Show saved expeditions"::Salesperson : Ekspeditionshoved.SetRange("Salesperson Code", "Salesperson Code");
    ...
    begin
        ...
        with This do begin
          ...
          if "Sale Line POS Saved".Find('-') then repeat
          ...
          if (DialogType = DialogType::Direct) or (DialogType = DialogType::DirectCustCash) then begin
            ...
            //-NPR5.20
              ...
              if (This."Register No." <> '') and (This."Sales Ticket No." <> '') then begin
                ...
                if AuditRollCusCash.FindSet then repeat
            ...
            State.IsCurrentView(ViewType.Sale) :
              begin
                ...
                case SaleLinePOS."Sale Type" of
                  ...
                  SaleLinePOS."Sale Type"::"Out payment" : exit;
                end;
                SaleLinePOSObject.GETRECORD(SaleLinePOS);
    ...
    begin
        ...
        if MenuLines1.Count = 1 then begin
           case MenuLines1.Type of
                ...
                MenuLines1.Type::PaymentType : begin
                               MenuLines2 := MenuLines1;
                               MenuLines2.Next;
                               if MenuLines2.Level > MenuLines1.Level then begin
                                  UpdateTouchScreenButtons(MenuLines2);
                ...
                MenuLines1.Type::Insurance : begin
                               MenuLines2 := MenuLines1;
                               MenuLines2.Next;
                               if MenuLines2.Level > MenuLines1.Level then begin
                                  UpdateTouchScreenButtons(MenuLines2);
                ...
                MenuLines1.Type::Keyboard : begin
                               MenuLines2 := MenuLines1;
                               MenuLines2.Next;
                               if MenuLines2.Level > MenuLines1.Level then begin
                                  UpdateTouchScreenButtons(MenuLines2);
    ...
    begin
        ...
        with This do begin
          ...
          case TypeStr of
              'LOGIN' :           Setuplinie.SetFilter(Type, '=%1', Setuplinie.Type::Login);
              'EKSPEDITION' :     Setuplinie.SetFilter(Type, '=%1', Setuplinie.Type::"Sale Functions");
              'PAYMENT' :         Setuplinie.SetFilter(Type, '=%1', Setuplinie.Type::"Payment Functions");
              'SALESMENU' :       Setuplinie.SetFilter(Type, '=%1', Setuplinie.Type::"Sale Form");
              'USERS' :           Setuplinie.SetFilter(Type, '=%1', Setuplinie.Type::User);
              'ITEM'  :           Setuplinie.SetFilter(Type, '=%1', Setuplinie.Type::Item);
              'BETALINGSVALG'  :  Setuplinie.SetFilter(Type, '=%1', Setuplinie.Type::PaymentType);
              'CUSTOMER'    :     Setuplinie.SetFilter(Type, '=%1', Setuplinie.Type::"Customer Functions");
              'FINANS'      :     Setuplinie.SetFilter(Type, '=%1', Setuplinie.Type::"G/L Account");
              'FORSIKRING'  :     Setuplinie.SetFilter(Type, '=%1', Setuplinie.Type::Insurance);
              'KEYBOARD'    :     Setuplinie.SetFilter(Type, '=%1', Setuplinie.Type::Keyboard);
              'DISCOUNT'    :     Setuplinie.SetFilter(Type, '=%1', Setuplinie.Type::Discount);
              'PRINTS'      :     Setuplinie.SetFilter(Type, '=%1', Setuplinie.Type::Prints);
              'REPORTS'     :     Setuplinie.SetFilter(Type, '=%1', Setuplinie.Type::Reports);
              'ITEMFUNCTIONS' :   Setuplinie.SetFilter(Type, '=%1', Setuplinie.Type::"Item Functions");
              'DEBITPRINT' : ;
              'DEBTISALEPOST'  : ;
              else
    ...
    begin
        ...
        if MetaTriggers.FindSet then repeat
          int := MetaTriggers.ID;
          ...
          if int > 0 then
            case MetaTriggers."Line Type" of
              MetaTriggers."Line Type"::Report :
                begin
                  case MetaTriggers."Var Parameter" of
                    MetaTriggers."Var Parameter"::" "       : REPORT.RunModal(int, false, false);
                    MetaTriggers."Var Parameter"::Sale      : REPORT.RunModal(int, false, false, Sale);
                    MetaTriggers."Var Parameter"::SalesLine : REPORT.RunModal(int, false, false, "Sales Line");
                  end;
                end;
              MetaTriggers."Line Type"::Page   :
                begin
                  case MetaTriggers."Var Parameter" of
                    MetaTriggers."Var Parameter"::" "       : PAGE.RunModal(int);
                    MetaTriggers."Var Parameter"::Sale      : PAGE.RunModal(int, Sale);
                    MetaTriggers."Var Parameter"::SalesLine : PAGE.RunModal(int, "Sales Line");
                  end;
                end;
              ...
              MetaTriggers."Line Type"::Codeunit :
                  case MetaTriggers."Var Parameter" of
                    MetaTriggers."Var Parameter"::" "       : CODEUNIT.Run(int);
                    MetaTriggers."Var Parameter"::Sale      : CODEUNIT.Run(int, Sale);
                    MetaTriggers."Var Parameter"::SalesLine : CODEUNIT.Run(int, "Sales Line");
                  end;
            end;
    ...
    begin
        case Content.Context.MethodName of
          ...
          MethodName_PushLineDiscountPct:     Complete_PushLineDiscountPct(UI.ParseDecimal(Content.Text),Content.Context.MethodArguments.Item('KeepPrev'));
          MethodName_PushTotalAmount:         Complete_PushTotalAmount(UI.ParseDecimal(Content.Text));
          MethodName_PushTotalDiscount:       Complete_PushTotalDiscount(UI.ParseDecimal(Content.Text));
          MethodName_PushDiscountPct:         Complete_PushDiscountPct(UI.ParseDecimal(Content.Text),Content.Context.MethodArguments.Item('KeepPrev'));
          MethodName_PushLineDiscountAmount:  Complete_PushLineDiscountAmount(UI.ParseDecimal(Content.Text));
          MethodName_PushLineUnitPrice:       Complete_PushLineUnitPrice(UI.ParseDecimal(Content.Text));
          MethodName_PushRegisterChange:      Complete_PushRegisterChange(Content.Text);
          MethodName_PushSaleReverse:         Complete_PushSaleReverse(Content.Text);
          MethodName_PushSaleAnnull:          Complete_PushSaleAnnull(Content.Text);
          MethodName_PushRegisterOpen:        Complete_PushRegisterOpen(Content.Text);
        end;
