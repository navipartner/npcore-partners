codeunit 6151285 "SS Action - Item AddOn"
{
    // 
    ...
    begin
        ...
        case WorkflowStep of
          'GetSalesLineAddonConfigJson' : FrontEnd.WorkflowResponse (GetAddonConfigJson (POSSession, Context));
          'SetItemAddons' : UpdateOrder (POSSession, Context);
        end;

        POSSession.RequestRefreshData();
    end;

    procedure GetAddonConfigJson(POSSession: Codeunit "POS Session";Context: Codeunit "POS JSON Management") JsonText: Text
    var
    ...
    local procedure GenerateAddonConfigJson(POSSale: Codeunit "POS Sale";MasterSaleLinePOS: Record "Sale Line POS";ItemAddOn: Record "NpIa Item AddOn") JsonString: Text
    var
        ItemAddOnLine: Record "NpIa Item AddOn Line";
        ItemAddOnLineOption: Record "NpIa Item AddOn Line Option";
        SaleLinePOS: Record "Sale Line POS";
        CommentText: Text;
        Quantity: Decimal;
        Value: Text;
          ...
          repeat
            ...
            if (ItemAddOnLine.Type = ItemAddOnLine.Type::Quantity) then begin
              ...
              case (ItemAddOnLine."Fixed Quantity") of
                false : Append (JsonString, StrSubstNo ('{"type":"plusminus","id":"%1","caption":"%2","value":%3}', ItemAddOnLine."Line No.", FormatJson (ItemAddOnLine.Description), Quantity));
                true  : Append (JsonString, StrSubstNo ('{"type":"switch","id":"%1","caption":"%2","value":%3}', ItemAddOnLine."Line No.", FormatJson (ItemAddOnLine.Description), 'false'));
              end;
    ...
    procedure UpdateOrder(POSSession: Codeunit "POS Session";Context: Codeunit "POS JSON Management")
    var
        ItemAddOn: Record "NpIa Item AddOn";
        ItemAddOnLine: Record "NpIa Item AddOn Line";
        ItemAddOnLineOption: Record "NpIa Item AddOn Line Option";
        ...
        if (ItemAddOnLine.FindSet ()) then begin
          repeat
            ...
            case ItemAddOnLine.Type of
              ItemAddOnLine.Type::Quantity : ApplyUserQuantity (UserValue, ItemAddOnLine, MasterLineNumber, POSSale, POSSaleLine);
              ItemAddOnLine.Type::Select   : ApplyUserItem (UserValue, ItemAddOnLine, MasterLineNumber, POSSale, POSSaleLine);
            end;
    ...
    local procedure ApplyUserQuantity(Value: Text;ItemAddOnLine: Record "NpIa Item AddOn Line";MasterLineNumber: Integer;POSSale: Codeunit "POS Sale";POSSaleLine: Codeunit "POS Sale Line")
    var
        SaleLinePOS: Record "Sale Line POS";
        DecVal: Decimal;
    ...
    local procedure ApplyUserItem(Value: Text;NpIaItemAddOnLine: Record "NpIa Item AddOn Line";MasterLineNumber: Integer;POSSale: Codeunit "POS Sale";POSSaleLine: Codeunit "POS Sale Line")
    var
        SaleLinePOS: Record "Sale Line POS";
        ItemAddOnLineOption: Record "NpIa Item AddOn Line Option";
        SelectionValue: DotNet npNetJToken;
        ItemNo: Code[20];
        VariantCode: Code[10];
    begin
    ...
    local procedure ApplyUserComment(Comment: Text;NpIaItemAddOn: Record "NpIa Item AddOn";POSSaleLine: Codeunit "POS Sale Line")
    var
    ...
    local procedure FindSaleLine(POSSale: Codeunit "POS Sale";AppliesToLineNo: Integer;NpIaItemAddOnLine: Record "NpIa Item AddOn Line";var SaleLinePOS: Record "Sale Line POS"): Boolean
    var
        SalePOS: Record "Sale POS";
    ...
    local procedure InsertAddOn(SaleLinePOS: Record "Sale Line POS";NpIaItemAddOnLine: Record "NpIa Item AddOn Line";AppliesToLineNo: Integer)
    var
        SaleLinePOSAddOn: Record "NpIa Sale Line POS AddOn";
    begin
