table 6014401 Register
{
    ...
    begin
        ...
        //+NPR4.21
        DimMgt.DeleteDefaultDim(DATABASE::Register,"Register No.");
    end;
    ...
    var
        ...
        Text10600005: Label '%1 is not implemented yet!';
        ...
        case RetailSetup."Get register no. using" of
          RetailSetup."Get register no. using"::USERPROFILE :
            begin
              Fil.TextMode(true);
              RetailSetup.Get;
              if RetailSetup."Use WIN User Profile" then begin
                if Fil.Create(Environment.ClientEnvironment(Format(RetailSetup."Get register no. using")) + Text10600002) then begin
                  Evaluate( Int, RegNo );
                  TextLinie := StrSubstNo('%1',(Int - 2) * 3.5);
                  Fil.Write( TextLinie );
                  Fil.Close;
                end else
                  Error(Text10600003);
              end else begin
                if Fil.Create( RetailSetup."Path Filename to User Profile" ) then begin
                  Evaluate( Int, RegNo );
                  TextLinie := StrSubstNo('%1',(Int - 2) * 3.5);
                  Fil.Write( TextLinie );
                  Fil.Close;
          ...
          RetailSetup."Get register no. using"::COMPUTERNAME :
            begin
              Register.Get(RegNo);
              Register."Logon-User Name" := Environment.ClientEnvironment('COMPUTERNAME');
              Register.Modify;
            end;
          RetailSetup."Get register no. using"::CLIENTNAME :
            begin
              Register.Get(RegNo);
              Register."Logon-User Name" := Environment.ClientEnvironment('CLIENTNAME');
              Register.Modify;
            end;
          RetailSetup."Get register no. using"::SESSIONNAME :
            begin
              Register.Get(RegNo);
              Register."Logon-User Name" := Environment.ClientEnvironment('SESSIONNAME');
              Register.Modify;
            end;
          RetailSetup."Get register no. using"::USERNAME :
            begin
              Register.Get(RegNo);
              Register."Logon-User Name" := Environment.ClientEnvironment('USERNAME');
              Register.Modify;
            end;
          ...
          RetailSetup."Get register no. using"::USERDOMAINID :
            Error(Text10600005, RetailSetup."Get register no. using"::USERDOMAINID);
          RetailSetup."Get register no. using"::"USER SETUP TABLE" :
            begin
              if UserSetup.Get(UserId) then begin
                UserSetup."Backoffice Register No." := RegNo;
                UserSetup.Modify;
              end else begin
                UserSetup.Init;
