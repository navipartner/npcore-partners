report 6060042 "Import Excel Item Worksheet"
{
          ...
          repeat
            ...
            //+NPR5.22
              ...
              if MappingFound then begin
                if GetColumnMapping(DATABASE::"Item Worksheet Line",ItemWorksheetLine.FieldNo("Item No."),MappedColumnNo,FieldLength) then
                ...
                //+NPR5.25[246088]
                if GetColumnMapping(DATABASE::"Item Worksheet Line",ItemWorksheetLine.FieldNo("Item Group"),MappedColumnNo,FieldLength) then
                  if ExcelBuf2.Get(ExcelBuf."Row No.",MappedColumnNo) then
                     ItemWorksheetLine.Validate("Item Group",CopyStr(ExcelBuf2."Cell Value as Text",1,FieldLength));
                if GetColumnMapping(DATABASE::"Item Worksheet Line",ItemWorksheetLine.FieldNo("Vendor No."),MappedColumnNo,FieldLength) then
                  if ExcelBuf2.Get(ExcelBuf."Row No.",MappedColumnNo) then
                ...
                if GetColumnMapping(DATABASE::"Item Worksheet Line",ItemWorksheetLine.FieldNo("Vendor Item No."),MappedColumnNo,FieldLength) then
                  if ExcelBuf2.Get(ExcelBuf."Row No.",MappedColumnNo) then
                    ItemWorksheetLine.Validate("Vendor Item No.",CopyStr(ExcelBuf2."Cell Value as Text",1,FieldLength));
                //+NPR5.25[246088]
                if GetColumnMapping(DATABASE::"Item Worksheet Line",ItemWorksheetLine.FieldNo(Description),MappedColumnNo,FieldLength) then
                  if ExcelBuf2.Get(ExcelBuf."Row No.",MappedColumnNo) then
                     ItemWorksheetLine.Validate(Description,CopyStr(ExcelBuf2."Cell Value as Text",1,FieldLength));
                if GetColumnMapping(DATABASE::"Item Worksheet Line",ItemWorksheetLine.FieldNo("Direct Unit Cost"),MappedColumnNo,FieldLength) then
                  if ExcelBuf2.Get(ExcelBuf."Row No.",MappedColumnNo) then
                    if Evaluate(TempDec,ExcelBuf2."Cell Value as Text") then
                      ItemWorksheetLine.Validate("Direct Unit Cost",TempDec);
                if GetColumnMapping(DATABASE::"Item Worksheet Line",ItemWorksheetLine.FieldNo("Sales Price"),MappedColumnNo,FieldLength) then
                  if ExcelBuf2.Get(ExcelBuf."Row No.",MappedColumnNo) then
                    if Evaluate(TempDec,ExcelBuf2."Cell Value as Text") then
                      ItemWorksheetLine.Validate("Sales Price",TempDec);
                if GetColumnMapping(DATABASE::"Item Worksheet Line",ItemWorksheetLine.FieldNo("Sales Price Currency Code"),MappedColumnNo,FieldLength) then
                  if ExcelBuf2.Get(ExcelBuf."Row No.",MappedColumnNo) then
                ...
                if GetColumnMapping(DATABASE::"Item Worksheet Line",ItemWorksheetLine.FieldNo("Purchase Price Currency Code"),MappedColumnNo,FieldLength) then
                  if ExcelBuf2.Get(ExcelBuf."Row No.",MappedColumnNo) then
                    if ExcelBuf2."Cell Value as Text" <> GLSetup."LCY Code" then
                      ItemWorksheetLine.Validate("Purchase Price Currency Code",CopyStr(ExcelBuf2."Cell Value as Text",1,FieldLength));
                //-NPR5.38 [297587]
                if GetColumnMapping(DATABASE::"Item Worksheet Line",ItemWorksheetLine.FieldNo("Sales Price Start Date"),MappedColumnNo,FieldLength) then
                  if ExcelBuf2.Get(ExcelBuf."Row No.",MappedColumnNo) then
                    if Evaluate(TempDate,ExcelBuf2."Cell Value as Text") then
                      ItemWorksheetLine.Validate("Sales Price Start Date",TempDate);
                if GetColumnMapping(DATABASE::"Item Worksheet Line",ItemWorksheetLine.FieldNo("Purchase Price Start Date"),MappedColumnNo,FieldLength) then
                  if ExcelBuf2.Get(ExcelBuf."Row No.",MappedColumnNo) then
                    if Evaluate(TempDate,ExcelBuf2."Cell Value as Text") then
                      ItemWorksheetLine.Validate("Purchase Price Start Date",TempDate);
                //+NPR5.38 [297587]
                if GetColumnMapping(DATABASE::"Item Worksheet Line",ItemWorksheetLine.FieldNo("Tariff No."),MappedColumnNo,FieldLength) then
                  if ExcelBuf2.Get(ExcelBuf."Row No.",MappedColumnNo) then
                     ItemWorksheetLine.Validate("Tariff No.",CopyStr(ExcelBuf2."Cell Value as Text",1,FieldLength));
                if GetColumnMapping(DATABASE::"Item Worksheet Line",ItemWorksheetLine.FieldNo("Variety Group"),MappedColumnNo,FieldLength) then
                  if ExcelBuf2.Get(ExcelBuf."Row No.",MappedColumnNo) then
                     ItemWorksheetLine.Validate("Variety Group",CopyStr(ExcelBuf2."Cell Value as Text",1,FieldLength));
                if GetColumnMapping(DATABASE::"Item Worksheet Line",ItemWorksheetLine.FieldNo("Item Category Code"),MappedColumnNo,FieldLength) then
                  if ExcelBuf2.Get(ExcelBuf."Row No.",MappedColumnNo) then
                     ItemWorksheetLine."Item Category Code" := CopyStr(ExcelBuf2."Cell Value as Text",1,FieldLength);
                if GetColumnMapping(DATABASE::"Item Worksheet Line",ItemWorksheetLine.FieldNo("Product Group Code"),MappedColumnNo,FieldLength) then
                  if ExcelBuf2.Get(ExcelBuf."Row No.",MappedColumnNo) then
                     ItemWorksheetLine."Product Group Code" := CopyStr(ExcelBuf2."Cell Value as Text",1,FieldLength);
                if GetColumnMapping(DATABASE::"Item Worksheet Line",ItemWorksheetLine.FieldNo("Recommended Retail Price"),MappedColumnNo,FieldLength) then
                  if ExcelBuf2.Get(ExcelBuf."Row No.",MappedColumnNo) then
                    if Evaluate(TempDec,ExcelBuf2."Cell Value as Text") then
                      ItemWorksheetLine.Validate("Recommended Retail Price",TempDec);
                //-NPR5.23 [242329]
                if GetColumnMapping(DATABASE::"Item Worksheet Line",ItemWorksheetLine.FieldNo("Vendors Bar Code"),MappedColumnNo,FieldLength) then
                  if ExcelBuf2.Get(ExcelBuf."Row No.",MappedColumnNo) then
                     ItemWorksheetLine.Validate("Vendors Bar Code",CopyStr(ExcelBuf2."Cell Value as Text",1,FieldLength));
                if GetColumnMapping(DATABASE::"Item Worksheet Line",ItemWorksheetLine.FieldNo("Internal Bar Code"),MappedColumnNo,FieldLength) then
                  if ExcelBuf2.Get(ExcelBuf."Row No.",MappedColumnNo) then
                     ItemWorksheetLine.Validate("Internal Bar Code",CopyStr(ExcelBuf2."Cell Value as Text",1,FieldLength));
                //+NPR5.23 [242329]
                //-NPR5.23 [242498]
                if GetColumnMapping(DATABASE::"Item Worksheet Line",ItemWorksheetLine.FieldNo("Net Weight"),MappedColumnNo,FieldLength) then
                  if ExcelBuf2.Get(ExcelBuf."Row No.",MappedColumnNo) then
                    if Evaluate(TempDec,ExcelBuf2."Cell Value as Text") then
                      ItemWorksheetLine.Validate("Net Weight",TempDec);
                if GetColumnMapping(DATABASE::"Item Worksheet Line",ItemWorksheetLine.FieldNo("Gross Weight"),MappedColumnNo,FieldLength) then
                  if ExcelBuf2.Get(ExcelBuf."Row No.",MappedColumnNo) then
                    if Evaluate(TempDec,ExcelBuf2."Cell Value as Text") then
                ...
                ProcessColumnMapping(DATABASE::"Item Worksheet Line",'',ExcludeColumnsFilter);
                ...
                //+NPR5.25 [246088]
                GetAttributeMapping(DATABASE::"Item Worksheet Line");
              end else begin
              //+NPR5.22
              ...
              //-NPR5.22
                ...
                repeat
                  ...
                  //+NPR5.23 [242498]
                    NPRAttrManagement.SetWorksheetLineAttributeValue (DATABASE::"Item Worksheet Line", AttributeNo,
              ...
              //-NPR5.22
              if MappingFound then begin
                if GetColumnMapping(DATABASE::"Item Worksheet Variant Line",ItemWorksheetVariantLine.FieldNo("Variety 1 Value"),MappedColumnNo,FieldLength) then
                  if ExcelBuf2.Get(ExcelBuf."Row No.",MappedColumnNo) then
                     ItemWorksheetVariantLine."Variety 1 Value" := CopyStr(ExcelBuf2."Cell Value as Text",1,FieldLength);
                if GetColumnMapping(DATABASE::"Item Worksheet Variant Line",ItemWorksheetVariantLine.FieldNo("Variety 2 Value"),MappedColumnNo,FieldLength) then
                  if ExcelBuf2.Get(ExcelBuf."Row No.",MappedColumnNo) then
                     ItemWorksheetVariantLine."Variety 2 Value" := CopyStr(ExcelBuf2."Cell Value as Text",1,FieldLength);
                if GetColumnMapping(DATABASE::"Item Worksheet Variant Line",ItemWorksheetVariantLine.FieldNo("Variety 3 Value"),MappedColumnNo,FieldLength) then
                  if ExcelBuf2.Get(ExcelBuf."Row No.",MappedColumnNo) then
                     ItemWorksheetVariantLine."Variety 3 Value" := CopyStr(ExcelBuf2."Cell Value as Text",1,FieldLength);
                if GetColumnMapping(DATABASE::"Item Worksheet Variant Line",ItemWorksheetVariantLine.FieldNo("Variety 4 Value"),MappedColumnNo,FieldLength) then
                  if ExcelBuf2.Get(ExcelBuf."Row No.",MappedColumnNo) then
                   //-NPR5.25
                   //ItemWorksheetVariantLine."Variety 3 Value" := COPYSTR(ExcelBuf2."Cell Value as Text",1,FieldLength);
                   ...
                   //+NPR5.25
                //-NPR5.23 [242329]
                if GetColumnMapping(DATABASE::"Item Worksheet Variant Line",ItemWorksheetVariantLine.FieldNo("Vendors Bar Code"),MappedColumnNo,FieldLength) then
                  if ExcelBuf2.Get(ExcelBuf."Row No.",MappedColumnNo) then
                     ItemWorksheetVariantLine.Validate("Vendors Bar Code",CopyStr(ExcelBuf2."Cell Value as Text",1,FieldLength));
                if GetColumnMapping(DATABASE::"Item Worksheet Variant Line",ItemWorksheetVariantLine.FieldNo("Internal Bar Code"),MappedColumnNo,FieldLength) then
                  if ExcelBuf2.Get(ExcelBuf."Row No.",MappedColumnNo) then
    ...
    local procedure AppendPrefix(ItemNoIn: Text;ItemWorksheetLineIn: Record "Item Worksheet Line"): Text
    var
        ItemWorksheetTemplate2: Record "Item Worksheet Template";
        ItemWorksheet2: Record "Item Worksheet";
        PrefixCode: Code[10];
    begin
        //-NPR5.36 [268786]
        PrefixCode := '';
